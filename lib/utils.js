...
module.exports = {
    ...
    ensureSaveSchema(schemas, ast, scope, getMeta = false) {
        return this.getMemorySchema(schemas, 'save', ast.table, getMeta).then((schema) => {
            if (schema) {
    ...
    getMemorySchema(schemaRetriever, name, table, getMeta = false) {
        return schemaRetriever.getMemorySchema(table, getMeta).then((schema) => {
            if (!schema)
                return null;

            let args, types, inOpt, inReq, out;
            if (name === 'save') {
                args = schema.args;
                types = schema.types;
                inReq = {};
                ...
                schema.args.forEach((arg, i) => {
                    inOpt[arg] = schema.types[i];
                });
            } else {
                args = ['table', 'principal'].concat(schema.args);
                types = [Type.Entity('tt:table'), Type.Entity('tt:contact')].concat(schema.types);
                inReq = {'table': Type.Entity('tt:table')};
                inOpt = {'principal': Type.Entity('tt:contact')};
                out = {};
                schema.args.forEach((arg, i) => {
