...
function convertToJS(type, value) {
    if (value === null)
        return null;
    if (type.isArray)
        return value.map((v) => convertToJS(type.elem, v));
    if (type.isMap) {
        if (value instanceof Immutable.Map) {
            return value.entrySeq().map(function(t) {
                var k = t[0];
                var v = t[1];
                return [convertToJS(type.key, k), convertToJS(type.value, v)];
            }).toArray();
        } else {
            return value;
        }
    ...
    if (type.isTuple) {
        if (type.schema !== null)
            return value.map((v, i) => convertToJS(type.schema[i], v));
        else
            return value;
    } else {
        return value;
