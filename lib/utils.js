...
module.exports = {
    ...
    getMemorySchema(schemaRetriever, name, table) {
        return schemaRetriever.getMemorySchema(table).then((schema) => {
            let args = ['table'].concat(schema.args);
            if (!schema)
                throw new Error('Failed to find table: ' + table);
            let types = [Type.String].concat(schema.types);
            let index = args.reduce((index, arg) => {index[arg] = Object.keys(index).length; return index}, {});
            let inOpt = {};
            let out = {};
            schema.args.forEach((arg, i) => {
                name === 'log.save'? inOpt[arg] = schema.types[i] : out[arg] = schema.types[i];
            });
            return new Ast.FunctionDef('global',
                args, // args
                types, // types
                index, // index
                {'table': Type.String}, // inReq
                inOpt, // inOpt
                out, // out
                '', // canonical
                '', // confirmation
