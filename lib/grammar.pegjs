...
class_def = is_abstract:('abstract' __ )? 'class' _ name:class_name _ extends_:('extends' __ full_class_name_list)? annotations:(_ annotation)* _ '{' _ members:class_member* _ '}' _ ';'? {
    ...
    return new Ast.ClassDef(name, extends_ !== null ? extends_[2] : null, { queries, actions, imports, macros }, { nl, impl }, options);
}
...
function_def = modifiers:query_modifiers? _ type:('query' / 'action') __ name:ident _ extends_:('extends' _ function_name_list _ )? args:function_param_decl_list _ annotations: ( _ annotation _)* ';' _ {
    ...
    const [nl_annotations, impl_annotations] = splitAnnotations(annotations);
    return new Ast.FunctionDef(type, null, name, extends_ !== null ? extends_[2] : null, { is_list, is_monitorable }, args, { nl:nl_annotations, impl:impl_annotations });
}
...
function_param_decl = direction:param_direction __ name:ident _ ':' _ type:type_ref annotations:( _ annotation _)* {
    const [nl, impl] = splitAnnotations(annotations);
    return new Ast.ArgumentDef(direction, name, type, { nl, impl });
}
...
long_date_value = ('makeDate' / 'new' __ 'Date') _ '(' year:literal_number _ ',' _ month:literal_number _ ',' _ day:literal_number _ ',' _ hours:literal_number _ ',' _ minutes:literal_number _ ',' _ seconds:literal_number _ ')' {
    var d = new Date;
    d.setFullYear(year);
    d.setMonth(month-1);
    d.setDate(day);
...
short_date_value = ('makeDate' / 'new' __ 'Date') _ '(' year:literal_number _ ',' _ month:literal_number _ ',' _ day:literal_number _ ')' {
    var d = new Date;
    d.setFullYear(year);
    d.setMonth(month-1);
    d.setDate(day);
...
unix_date_value = ('makeDate' / 'new' __ 'Date') _ '(' unix:literal_number _ ')' {
    var d = new Date;
    d.setTime(unix);
    return new Ast.Value.Date(d, '+', null);
}
...
compound_type_field = name:ident _ ':' _ type:type_ref annotations:( _ annotation _)* {
    const [nl, impl] = splitAnnotations(annotations);
    return new Ast.ArgumentDef(undefined, name, type, { nl, impl });
}
