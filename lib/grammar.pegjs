...
class_def = 'class' _ name:class_name _ extends_:('extends' __ full_class_name)? '{' _ members:class_member* _ '}' annotations:(_ annotation)* {
    const queries = {};
    const actions = {};
    const imports = [];
    ...
    return new Ast.ClassDef(name, extends_ !== null ? extends_[2] : null, queries, actions, imports, nl_annotations, impl_annotations);
}
...
example = 'stream'  _ params:(lambda_param_decl_list)? _ ':=' _ stream:stream _ utterances:example_annotation _ ';' {
    return new Ast.Example(-1, 'stream', params ? params : [], stream, utterances, []);
} / 'query'  _ params:(lambda_param_decl_list)? _ ':=' _ query:table _ utterances:example_annotation _ ';' {
    return new Ast.Example(-1, 'query', params ? params : [], query, utterances, []);
} / 'action'  _ params:(lambda_param_decl_list)? _ ':=' _ action:action _ utterances:example_annotation _ ';' {
    action = new Ast.Action.Invocation(action, action.schema);
    return new Ast.Example(-1, 'action', params ? params : [], action, utterances, []);
} / 'program'  _ params:(lambda_param_decl_list)? _ ':=' _ program:program _ utterances:example_annotation _ ';' {
    return new Ast.Example(-1, 'program', params ? params : [], program, utterances, []);
}
...
object_value = '{' _ first:( ident _ '=' _ value ) _ rest:(',' _ ident _ '=' _ value _ )* _ '}' {
    ...
    obj[first[0]] = first[4];
    if (rest.length !== 0)
        obj[take(rest, 2)] = take(rest, 6);
    return Ast.Value.Object(obj);
}


annotation = nl_annotation / impl_annotation
example_annotation = annotation:nl_annotation {
    if (annotation.name !== 'utterances') throw new Error('Only "utterances" annotation is allowed for examples');
    if (!annotation.value.isArray) throw new Error('An array is expected for "utterances" annotation');
    return annotation.value.value.map((v) => v.value);
}
