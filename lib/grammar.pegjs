...
log_query = log_aggregation / log
log =
    'query' _ in_params:input_param_list _ filters:filter_list out_params:output_param_list {
    return RulePart(Selector.Builtin, 'query', in_params, filters, out_params, null, null);
}
log_aggregation =
    'query' _ in_params:input_param_list _ filters:filter_list _ ',' _ agg:(agg_param / argm_param) {
    let out_params = agg.out_params;
    let aggregation = agg.aggregation;
    return RulePart(Selector.Builtin, 'query', in_params, filters, out_params, null, aggregation);
}
...
argm_lhs = '[' _ '*' _ ']' {return [];} / '[' _ names:argm_list _ ']' {return names;}
argm_rhs = type:argm _ '(' _ field:ident _ ', ' _ cols:argm_lhs _ ')' {return Aggregation(type, field, cols);}
argm_list = (first:ident rest:(', ' _ ident _)*) {return [first].concat(take(rest, 2));}
