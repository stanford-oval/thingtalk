...
} / 'attimer' _ '(' _ 'time' _ '=' _ '[' _ first:time_value _ rest:(',' _  time_value _)* ']' _ ')' {
  return new Ast.Stream.AtTimer([first].concat(take(rest, 2)), null);
}
...
action = fn:thingpedia_function_call {
  ...
  if (rule.isCommand)
    throw new Error('Invalid rule to invoke: a when clause is needed');
  return new Ast.Action.Rule(null, rule);
}
