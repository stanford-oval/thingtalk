...
primary_table =
  ...
  / argmin_max_expression / aggregate_expression / table_ref
...
window_expression = what:('window' / 'timeseries') __ base:primary_scalar_expression _ ',' _ delta:primary_scalar_expression _ 'of' __ stream:alias_stream {
  if (what === 'window')
    return new Ast.Table.Window(base, delta, stream, null);
  else
    return new Ast.Table.TimeSeries(base, delta, stream, null);
}
history_expression = what:('sequence' / 'history') __ base:primary_scalar_expression _ ',' _ delta:primary_scalar_expression _ 'of' __ table:alias_table {
  if (what === 'sequence')
...
argmin_max_expression = 'aggregate' __ op:('argmin' / 'argmax') _ base:primary_scalar_expression _ ',' _ limit:primary_scalar_expression _ field:ident _ 'of' __ table:alias_table {
...
primary_stream = timer / attimer / '(' _ stream:stream _ ')' { return stream; } / edge_trigger / monitor_stream / stream_projection / stream_ref
...
monitor_stream = 'monitor' __ table:alias_table {
