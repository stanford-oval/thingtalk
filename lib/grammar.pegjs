...
compound_measure_value = first:measure_value _ rest:('+' _ measure_value _)* {
  if (rest.length === 0)
    return first;
  return Value.CompoundMeasure([first].concat(take(rest, 2)));
}
...
edge_date_value = edge:('start_of' / 'end_of') _ '(' unit:time_unit _ ')' {
    ...
    return new Value.Date(DateEdge(edge, unit), null);
}
now = 'makeDate' _ '(' _ ')' {
...
relative_date_value = date:(long_date_value / short_date_value / unix_date_value / edge_date_value / now) _ '+' _ 'delta' _ '(' _ offset:compound_measure_value _ ')' {
    return Value.Date(date.value, offset);
}
