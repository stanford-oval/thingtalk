...
class_def = is_abstract:('abstract' __ )? 'class' _ name:class_name _ extends_:('extends' __ full_class_name_list)? annotations:(_ annotation)* _ '{' _ members:class_member* _ '}' _ ';'? {
    ...
    return new Ast.ClassDef(name, extends_ !== null ? extends_[2] : null, queries, actions, imports, nl_annotations, impl_annotations, !!is_abstract);
}
...
primary_table_expression =
  sliced_table / table_projection / window_expression / history_expression
  / aggregate_expression / computed_table / sort_expression / table_ref
...
top_level_statement = class_def / dataset / no_param_declaration / with_param_declaration / statement
statement = assignment / rule / on_input
...
no_param_declaration = 'let' __ 'stream' __ name:ident _ ':=' _ lambda:('\\' _ lambda_param_decl_list _ '->' _)? stream:stream _ ';' {
  ...
  return new Ast.Statement.Declaration(name, 'action', lambda !== null ? lambda[2] : {},
    new Ast.Action.Invocation(action, null));
} / 'let' __ type:('program' / 'procedure') __ name:ident _ ':=' _ lambda:('\\' _ lambda_param_decl_list _ '->' _)? '{' _ statements:(statement _)+ '}' _ ';' {
  const program = makeInput(take(statements, 0));
  return new Ast.Statement.Declaration(name, type, lambda !== null ? lambda[2] : {}, program);
}

with_param_declaration = 'let' __ 'stream' __ name:ident _ lambda:lambda_param_decl_list _ ':=' _ stream:stream _ ';' {
    return new Ast.Statement.Declaration(name, 'stream', lambda, stream);
} / 'let' __ 'query' __ name:ident _ lambda:lambda_param_decl_list _ ':=' _ table:table _ ';' {
    return new Ast.Statement.Declaration(name, 'query', lambda, table);
} / 'let' __ 'action' __ name:ident _ lambda:lambda_param_decl_list _ ':=' _ action:thingpedia_function_call _ ';' {
    return new Ast.Statement.Declaration(name, 'action', lambda,
        new Ast.Action.Invocation(action, null));
} / 'let' __ type:('program' / 'procedure') __ name:ident _ lambda:lambda_param_decl_list _ ':=' _ '{' _ statements:(statement _)+ '}' _ ';' {
  const program = makeInput(take(statements, 0));
  return new Ast.Statement.Declaration(name, type, lambda, program);
}
