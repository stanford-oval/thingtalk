...
scalar_function = name:ident _ args:scalar_function_args {
  return new Ast.ScalarExpression.Derived(location(), name, args);
}

macro_scalar_expression = fn:thingpedia_function_name _ in_params:input_param_list {
    const [selector, name] = fn;
    return new Ast.ScalarExpression.VarRef(location(), selector, name, in_params);
}

primary_scalar_expression = v:value {
  return new Ast.ScalarExpression.Primary(location(), v);
...
add_expr = first:mul_expr _ rest:(('+' / '-') _ mul_expr _)* {
  return rest.reduce(((x, y) => new Ast.ScalarExpression.Derived(location(), y[0], [x, y[2]])), first);
}
...
aggregation_expression = 'count' _ '(' _ list:list_expression _ ')' {
    return new Ast.ScalarExpression.Aggregation(location(), 'count', '*', list);
} / op:('sum' / 'avg' / 'max' / 'min') _ '(' _ field:(qualified_name _ 'of' __)? list:list_expression _ ')' {
    return new Ast.ScalarExpression.Aggregation(location(), op, field ? field[0] : null, list);
} / 'filter' _ '(' list:list_expression _ ')' {
    return new Ast.ScalarExpression.Filter(location(), list);
} / 'flat' _ '(' list:list_expression _ ')' {
    return new Ast.ScalarExpression.FlattenedList(location(), list);
}

list_expression = param:qualified_name _ filter:(',' _ or_expr)? {
    return new Ast.ListExpression(location(), param, filter ? filter[2] : null);
}
