...
short_remote_program = _ principal:program_principal _ statement:statement {
    return makeInput([statement], principal);
}

simple_program = _ statements:(statement _)* {
    return makeInput(take(statements, 0), null);
}
wrapped_program = _ principal:program_principal? _ '{' _ statements:(statement _)* '}' _ {
...
} / 'program'  _ ':=' _ program:wrapped_program _ annotations:(annotation _)* _ ';' {
    if (program instanceof Ast.Input.Library || program.rules.length === 0)
        error(`program example must include at least one rule`);
    if (program.principal !== null)
        error(`program example cannot be remote`);
    const [id, utterances, preprocessed, other] = checkExampleAnnotations(take(annotations, 0));
    return new Ast.Example(id, 'program', [], program, utterances, preprocessed, other);
...
sliced_table =
  table:primary_table _ '[' _ base:number _ ':' _ limit:number _ ']' { return new Ast.Table.Slice(table, base, limit, null); } /
  table:primary_table _ '[' _ indices:array_value_list _ ']' { return enw Ast.Table.Index(table, indices, null); } /
  table:primary_table

primary_table_expression =
  sliced_table / table_projection / window_expression / history_expression
  / aggregate_expression / computed_table / table_ref
...
statement = class_def / dataset / declaration / assignment / rule / on_input
...
keyword = literal_bool / ('let' / 'now' / 'new' / 'as' / 'of' / 'in' / 'out' / 'req' / 'opt' / 'notify' / 'return' / 'join' / 'edge' / 'monitor' / 'class' / 'extends' / 'mixin' / 'this' / 'import' / 'null' / 'enum' / 'aggregate' / 'dataset' / 'oninput') !identchar
