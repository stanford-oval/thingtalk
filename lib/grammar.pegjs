...
input_param = name:ident _ '=' _ value:scalar_expression {
...
principal_prefix = value:(entity_value / string_value / 'self') _ '::' {
    if (value === 'self')
        return null;
    else if (value.isString) // for compat and ease of use, a raw string is a contact name
        return Ast.Value.Entity(value.value, 'tt:contact_name', null);
...
primary_table =
  ...
  / table_projection / window_expression / timeseries_expression
...
aggregate_expression = 'aggregate' __ 'count' __ 'of' __ table:alias_table {
  return new Ast.Table.Aggregation(table, '*', 'count', null);
} / 'aggregate' __ op:('argmin' / 'argmax') _ num:literal_number? _ field:ident _ 'of' __ table:alias_table {
  return new Ast.Table.ArgMinMax(table, field, op, num === null ? 1 : num, null);
} / 'aggregate' __ op:ident __ field:ident _ 'of' __ table:alias_table {
...
primary_stream = timer / attimer / '(' _ stream:stream _ ')' { return stream; } / edge_trigger / stream_projection / stream_ref
...
stream_projection = '[' _ first:ident _ rest:(',' _ ident _)* _ ']' _ 'of' __ stream:alias_stream {
...
var_ref_value = first:ident _ rest:('.' _ ident _)*_ !'(' {
  if (rest.length === 0)
...
ident "identifier" = v:$(identstart identchar*) {
