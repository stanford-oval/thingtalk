...
attimer = 'attimer' _ '(' _ 'time' _ '=' _ time:(undefined_value / time_value) _ ')' {
  ...
  return new Ast.Stream.AtTimer([first].concat(take(rest, 2)), null, null);
} / 'attimer' _ '(' _ 'time' _ '=' _ time:(undefined_value / time_value) _ ',' _ 'expiration_date' _ '=' _ expiration_date:(undefined_value / short_date_value) _ ')' {
  return new Ast.Stream.AtTimer([time], expiration_date, null);
} / 'attimer' _ '(' _ 'time' _ '=' _ '[' _ first:time_value _ rest:(',' _ time_value _)* ']' _ ',' _ 'expiration_date' _ '=' _ expiration_date:(undefined_value / short_date_value) _ ')' {
  return new Ast.Stream.AtTimer([first].concat(take(rest, 2)), expiration_date, null);
}
