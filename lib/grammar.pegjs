...
dialogue_history_item = stmt:executable_statement _ results:history_result_list? _ {
    return new Ast.DialogueHistoryItem(location(), stmt, results);
}

history_result_list = '[' _ ']' {
    return [];
} / '[' _ first:history_result _ /*rest:(',' _ history_result _)*/ ']' _ ';' {
    const rest = [];
