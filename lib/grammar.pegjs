...
log_function_name = 'log.get' / 'log.new' / 'log.save'
...
log_trigger_or_query = log_aggregation / log_aggregation_sugar / log
log =
...
log_aggregation_sugar =
    fn:log_function_name agg:aggregation _ in_params:input_param_list _ filters:filter_list _ out_params:output_param_list {
    let field = in_params.find((e) => {
        return e.name === 'field'
    });
    if (!field)
        throw new Error('Missing field parameter!');
    let aggregation = Aggregation(agg, field.value.value, take(out_params, 'value'));
    return RulePart(Selector.Builtin, fn, in_params, filters, out_params, null, aggregation);
}
...
log_save_action = fn:log_function_name _ in_params:input_param_list {
    return RulePart(Selector.Builtin, fn, [], BooleanExpression.True, [], null, null);
}
...
action = notify_action / return_action / log_save_action / other_action
