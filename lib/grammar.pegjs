...

{
    ...
    function makeInput(statements, principal) {
        ...
        for (let stmt of statements) {
            ...
            else if (stmt.isCommand || stmt.isRule)
                commandsOrRules.push(stmt);
            else if (stmt.isOnInput && oninput !== null)
        ...
        if (datasets.length > 0) {
            if (declarations.length > 0 || commandsOrRules.length > 0 || oninput !== null)
        ...
        if (classes.length > 0 && declarations.length === 0 && commandsOrRules.length === 0 && oninput === null && principal === null)
            return new Ast.Input.Library(classes, datasets);

        return optimizeProgram(new Ast.Input.Program(classes, declarations, commandsOrRules, principal, oninput));
    }
}
...
statement = class_def / dataset / declaration / rule
