...

{
    ...
    function checkExampleAnnotations(annotations) {
        ...
        for (let annotation of annotations) {
            if (annotation.type === 'impl') {
                if (annotation.name !== 'id')
                    return error(`Invalid example annotation ${annotation.name}, only id is allowed`);
        ...
        return [id, utterances, preprocessed];
    }
}
...
example = 'stream'  _ params:(lambda_param_decl_list)? _ ':=' _ stream:stream _ annotations:(annotation _)* _ ';' {
    const [id, utterances, preprocessed] = checkExampleAnnotations(take(annotations, 0));
    return new Ast.Example(id, 'stream', params ? params : [], stream, utterances, preprocessed);
} / 'query'  _ params:(lambda_param_decl_list)? _ ':=' _ query:table _ annotations:(annotation _)* _ ';' {
    const [id, utterances, preprocessed] = checkExampleAnnotations(take(annotations, 0));
    return new Ast.Example(id, 'query', params ? params : [], query, utterances, preprocessed);
    ...
    const [id, utterances, preprocessed] = checkExampleAnnotations(take(annotations, 0));
    return new Ast.Example(id, 'action', params ? params : [], action, utterances, preprocessed);
} / 'program'  _ ':=' _ program:program _ annotations:(annotation _)* _ ';' {
    const [id, utterances, preprocessed] = checkExampleAnnotations(take(annotations, 0));
    return new Ast.Example(id, 'program', [], program, utterances, preprocessed);
}
