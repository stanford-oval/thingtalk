...
long_program_body = '{' _ classes:(class_def _)* _ declarations:(declaration _)* rules:(rule _)* '}' {
  return optimizeProgram(new Ast.Program(take(classes, 0), take(declarations, 0), take(rules, 0), null));
}
program = _ principal:program_principal? _ body:long_program_body _ {
    return body.set({ principal });
...
class_def =
    args:class_def_base { return new Ast.ClassDef(...args); } /
    'abstract' _ args:class_def_base { return new Ast.ClassDef(...args, true); }

class_def_base = 'class' _ name:class_name _ extends_:('extends' __ full_class_name_list)? annotations:(_ annotation)* _ '{' _ members:class_member* _ '}' _ {
    const queries = {};
    const actions = {};
    const imports = [];
    ...
    return [name, extends_ !== null ? extends_[2] : null, queries, actions, imports, nl_annotations, impl_annotations];
}
...
} / 'program'  _ ':=' _ program:long_program_body _ annotations:(annotation _)* _ ';' {
    const [id, utterances, preprocessed, other] = checkExampleAnnotations(take(annotations, 0));
...
} / stream:stream _ '=>' _ actions:action_list {
