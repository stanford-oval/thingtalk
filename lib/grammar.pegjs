...
program_principal = 'executor' _ '=' _ value:(entity_value / string_value / 'self') _ ':' {
    if (value === 'self')
        return null;
...
} / _ principal:program_principal? _ rule:rule {
...
device_attribute = name:('id'/'principal') _ '=' _ value:(entity_value / string_value / array_value) {
    return [name, value];
}
...
primary_table =
  ...
  / projection / window_expression / timeseries_expression
  / aggregate_expression / table_ref

projection = '[' _ first:ident _ rest:(',' _ ident _)* _ ']' _ 'of' __ table:table {
  return new Ast.Table.Projection(table, [first].concat(take(rest, 2)), null);
}

window_expression = 'window' __ 'from' __ from:primary_scalar_expression _ 'to' __ to:primary_scalar_expression _ 'of' __ stream:alias_stream {
  return new Ast.Table.Window(from, to, stream, null);
}
timeseries_expression = 'timeseries' __ 'from' __ from:primary_scalar_expression _ 'to' __ to:primary_scalar_expression _ 'of' __ stream:alias_stream {
  return new Ast.Table.TimeSeries(from, to, stream, null);
}
aggregate_expression = 'aggregate' __ op:ident __ field:ident _ 'of' __ table:alias_table {
...
table = first:filtered_table _ rest:('join' __ filtered_table _)* {
...
primary_stream = timer / attimer / '(' _ stream:stream _ ')' { return stream; } / edge_trigger / stream_ref
...
edge_trigger = 'edge' __ table:alias_table _ 'on' __ edge:('new' !identchar / or_expr) {
...
stream = first:filtered_stream _ rest:('join' __ filtered_table _)* {
...
rule = stream:stream _ '=>' _ action:thingpedia_function_call {
  return new Ast.Statement.Rule(stream, action);
} / 'now' __ '=>' _ table:table _ '=>' _ action:thingpedia_function_call _ ';' {
  return new Ast.Statement.Command(table, action);
} / 'now' __ '=>' _ action:thingpedia_function_call _ ';' {
...
function_style_predicate = fn:ident _ '(' _ lhs:add_expr _ ',' _ rhs:add_expr _ ')' {
    if (fn === 'substr')
        fn = '=~';
    return new Ast.BooleanExpression.Atom(lhs, fn, rhs);
}

infix_predicate = lhs:add_expr _ op:comparison_operator _ rhs:add_expr {
...
scalar_expression = expr:or_expr { return new Ast.ScalarExpression.Boolean(expr); } / add_expr
...
var_ref_value = name:ident _ !'(' { return Ast.Value.VarRef(name); }
undefined_value = '$undefined' remote:('.' _ 'remote')? {
    return Ast.Value.Undefined(remote === null);
}
compound_measure_value = first:measure_value _ rest:('+' _ measure_value _)* {
  if (rest.length === 0)
    return first;
...
relative_date_value = date:(long_date_value / short_date_value / unix_date_value / edge_date_value / now) _ '+' _ 'delta' _ '(' _ offset:compound_measure_value _ ')' {
