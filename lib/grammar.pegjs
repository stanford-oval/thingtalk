...
dialogue_state = _ '$dialogue' _ dialogueAct:short_function_name _ dialogueActParam:('(' _ qualified_name _ ')' _)? ';' _ history:dialogue_history_item* _ {
    return new Ast.DialogueState(location(), dialogueAct[0].kind, dialogueAct[1], dialogueActParam ? dialogueActParam[2] : null, history).optimize();
}

dialogue_history_item = stmt:rule_body _ results:history_results _ ';' _ {
    return new Ast.DialogueHistoryItem(location(), stmt, results, true);
} / stmt:rule_body _ '#[' _ 'confirm' _ '=' _ 'true' _ ']' _ ';' _ {
    return new Ast.DialogueHistoryItem(location(), stmt, null, true);
} / stmt:rule_body _ ';' _ {
    return new Ast.DialogueHistoryItem(location(), stmt, null, false);
}

history_results = '#[' _ 'results' _ '=' _ '[' _ ']' _ ']' {
    return new Ast.DialogueHistoryResultList(location(), [], new Ast.Value.Number(0), false);
} / annotations:(history_annotation _)+ {
    let results, more, count;
    ...
    return new Ast.DialogueHistoryResultList(location(), results, count, more);
}
...
history_result_list = '[' _ first:history_result _ rest:(',' _ history_result _)* ']' {
...
statement = no_param_declaration / with_param_declaration / assignment / executable_statement / on_input
