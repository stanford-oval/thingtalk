...
relative_date_value = date:(long_date_value / short_date_value / unix_date_value / now) _ op:('+' / '-') _ delta:literal_number _ unit:ident {
    var operator = op === '+' ? function(a, b) {return a + b;} : function(a, b) {return a - b};
    var d = date.value;
    switch (unit) {
        case 'year':
        case 'y': d.setFullYear(operator(d.getFullYear(), delta)); break;
        case 'month': d.setMonth(operator(d.getMonth(), delta)); break;
        case 'week': d.setDate(operator(d.getDate(), delta * 7)); break;
        case 'day':
        case 'd': d.setDate(operator(d.getDate(), delta)); break;
        case 'hour':
        case 'h': d.setHours(operator(d.getHours(), delta)); break;
        case 'minute':
        case 'min':
        case 'm': d.setMinutes(operator(d.getMinutes(), delta)); break;
        case 'second':
        case 's': d.setSeconds(operator(d.getSeconds(), delta)); break;
        default: throw new Error("Wrong unit for datetime: " + unit);
    }
    return Value.Date(d);
}
