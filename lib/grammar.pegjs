...
statement = var_decl / compute_module / command / rule
...
var_decl = 'var' __ name:ident _ ':' _ type:type_ref _ ';' {
    return Statement.VarDecl(name, type);
}

rule = once:'once'? _ trigger:input_list _ queries:('=>' _ query _)* _ '=>' _ action:action {
    return Statement.Rule(trigger, take(queries, 2), action, once !== null);
}
input_list = first:invocation _ rest:(',' _ rule_part _)* {
    return [first].concat(take(rest, 2));
}
rule_part = binding / builtin_predicate / condition

action = ('notify' / ('@$notify' _ '(' _ ')')) _ ';'{
    return [RulePart.Invocation(Selector.Builtin, 'notify', [])];
} / single:(invocation / binding) _ ';' { return [single]; } / '{' list:(_ (invocation / binding) _ ';')+ _ '}' {
    return take(list, 1);
}
query = input:input_list &(_ '=>') {
    return input;
}

command = 'now' _ queries:('=>' _ query _)* _ '=>' _ action:action  {
    return Statement.Command(take(queries, 2), action);
}

invocation = selector:device_selector _ '.' _ name:ident _ params:channel_param_list {
...
device_selector = '@' name:classident { return Selector.GlobalName(name); } /
...
literal "literal" = val:literal_bool { return Value.Boolean(val); } /
    val:literal_string type:('^^' ($(ident ':'))? ident)? {
        if (type != null)
