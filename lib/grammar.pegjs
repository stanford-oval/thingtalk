...
class_def = is_abstract:('abstract' __ )? 'class' _ name:class_name _ extends_:('extends' __ full_class_name_list)? annotations:(_ annotation)* _ '{' _ members:class_member* _ '}' _ ';'? {
    ...
    for (var ast of members) {
        if (Array.isArray(ast) && ast[0] instanceof Ast.FunctionDef) {
            const f = ast[0];
            const base = ast[1];
            switch (f.functionType) {
            case 'query':
                if (!(base in queries))
                    throw new TypeError(`Base query ${base} not found`);
                queries[f.name] = f.extends(queries[base]);
                break;
            case 'action':
                if (!(base in actions))
                    throw new TypeError(`Base action ${base} not found`);
                actions[f.name] = f.extends(actions[base]);
                break;
            }
        } else if (ast instanceof Ast.FunctionDef) {
...
function_def = modifiers:query_modifiers? _ type:('query' / 'action') __ name:ident _ extends_:('extends' _ ident _ )? args:function_param_decl_list _ annotations: ( _ annotation _)* ';' _ {
    let [is_monitorable, is_list] = modifiers || [false, false];
    const [nl_annotations, impl_annotations] = splitAnnotations(annotations);
    const f = new Ast.FunctionDef(type, name, args, is_list, is_monitorable, nl_annotations, impl_annotations);
    if (extends_)
        return [f, extends_[2]];
    return f;
}
