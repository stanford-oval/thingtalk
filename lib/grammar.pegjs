...
compute_predicate = lhs:aggregation_expression _ op:comparison_operator _ rhs:value {
...
} / aggregation_expression / scalar_function / '(' _ expr:scalar_expression _ ')' {
  return expr;
}
...
aggregation_expression = 'count' _ '(' _ param:qualified_name _ filter:(',' _ or_expr)? _ ')' {
    const list = new Ast.ListExpression(param, filter ? filter[2] : null);
    return new Ast.ScalarExpression.Aggregation('count', '*', list);
} / op:('sum' / 'avg' / 'max' / 'min') _ '(' _ field:(qualified_name _ 'of' __)? _ param:qualified_name _ filter:(',' _ or_expr)? _ ')' {
    const list = new Ast.ListExpression(param, filter ? filter[2] : null);
