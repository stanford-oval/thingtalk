...
program = _ principal:((entity_value / string_value / 'self') _ ':' _)? _ name:ident _ params:decl_param_list _ '{' _ classes:(class_def _)* _ rules:(rule _)+ '}' _ {
    if (principal !== null) {
...
} / _ principal:((entity_value / string_value / 'self') _ ':' _)? _ rules:(rule _)+ {
    if (principal !== null) {
        principal = principal[0];
        if (principal === 'self')
            principal = null;
...
query = q:trigger_or_query &(_ '=>') { return q; } / q:log_query &(_ '=>') { return q; }
...
log_query = log_aggregation / log_aggregation_sugar / log
log =
    'log.get' _ in_params:input_param_list _ filters:filter_list out_params:output_param_list {
    return RulePart(Selector.Builtin, 'log.get', in_params, filters, out_params, null, null);
}
log_aggregation =
    'log.get' _ in_params:input_param_list _ filters:filter_list _ ',' _ agg:(agg_param / argm_param) {
...
log_aggregation_sugar =
    'log.' agg:aggregation _ in_params:input_param_list _ filters:filter_list _ out_params:output_param_list {
...
argm_rhs = type:argm _ '(' _ field:ident _ ', ' _ cols:argm_lhs _ ')' {return Aggregation(type, field, cols);}
