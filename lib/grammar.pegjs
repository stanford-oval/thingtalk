...
// Types
type_ref = complex_type_ref / simple_type_ref

simple_type_ref = 'Measure' _ '(' _ unit:ident? _ ')' { return Type.Measure(unit); } /
...
complex_type_ref = 'Array' _ '(' _ type:simple_type_ref _ ')' { return Type.Array(type); } /
    '(' first:type_ref _ rest:(',' _ simple_type_ref _)* ')' { return Type.Tuple([first].concat(take(rest, 2))); } /
...
compound_type_field = name:ident _ ':' _ type:(simple_type_ref / compound_type_ref) annotations:( _ annotation _)* {
    const [nl_annotations, impl_annotations] = splitAnnotations(annotations);
