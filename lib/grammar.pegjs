...
program = _ name:ident _ params:decl_param_list _ '{' _ classes:(class_def _)* _ rules:(rule _)+ '}' _ {
    return Program(name, params, take(classes, 0), take(rules, 0));
} / _ rules:(rule _)+ {
    return Program('Main', [], [], take(rules, 0));
}

permission_rule = 'Allowed' channelType:('Trigger'/'Query'/'Action') _ '(' _ principal:(entity_value / '_') _ ',' _ kind:full_class_name _ '.' _ channel:ident _ precondition:(',' _ or_expr _)? postcondition:(',' _ postcondition:or_expr _)? _ ')' {
    ...
    return Ast.Allowed(kind, channel, channelType.toLowerCase(),
        optimizeFilter(precondition), optimizeFilter(postcondition), null);
}
...
class_def = 'class' _ name:full_class_name _ 'extends' _ extends_:full_class_name '{' _ members:class_member* _ '}' {
...
action = ('notify' / ('@$notify' _ '(' _ ')')) { return RulePart(Selector.Builtin, 'notify', [], BooleanExpression.True, [], null); }
...
device_selector = type:full_class_name _ values:('(' _ attribute_list _ ')')? {
    if (values !== null)
        values = values[2];
    else
        values = {};
...
bool_expr = '(' _ or:or_expr _ ')' { return or; } /
    ...
    f:filter { return BooleanExpression.Atom(f); }
...
function_filter = 'contains' / 'group_member' / 'substr' { return '=~' }
operator = '>=' / '<=' / '>' / '<' / '=~' / ('=' !'>') { return '='; } / '!='
