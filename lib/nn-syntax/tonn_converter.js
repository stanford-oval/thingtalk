        ...
        for (let and of filter.operands) {
            ...
            for (let or of and.operands) {
                ...
                if (or.isAtom) {
                    ...
                    let name = or.lhs.list.name;
                    let ptype = schema.inReq[name] || schema.inOpt[name] || schema.out[name];
                    let listExpression = List.concat(`param:${name}:${ptype}`);
                    if (or.lhs.list.filter) {
                        let args = [];
                        ...
                        let cnf = filterToCNF(or.lhs.list.filter);
                        listExpression = List.concat(listExpression, 'filter', this.cnfFilterToNN(cnf, localschema));
                    }
                    let lhs;
                    if (or.lhs.operator === 'count') {
                        lhs = List.concat(or.lhs.operator, '(', listExpression, ')');
                    } else if (or.lhs.field) {
                        let ftype = ptype.elem.fields[or.lhs.field].type;
                        lhs = List.concat(or.lhs.operator, '(', `param:${or.lhs.field}:${ftype}`, 'of', listExpression, ')');
                    } else {
                        lhs = List.concat(or.lhs.operator, '(', listExpression, ')');
                    }
