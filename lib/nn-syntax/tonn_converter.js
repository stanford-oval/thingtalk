        ...
        if (list.filter) {
            ...
            if (ptype.elem.isCompound) {
                for (let field in ptype.elem.fields)
                    args.push(new Ast.ArgumentDef(null, 'out', field, ptype.elem.fields[field].type, {}, {}));
            } else {
                args.push(new Ast.ArgumentDef(null, 'out', 'value', ptype.elem, {}, {}));
            }
            let localschema = new Ast.ExpressionSignature(null, 'query', null, [], args, {});
