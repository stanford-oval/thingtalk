...
function filterToCNF(filter) {
    filter = (function pushDownNegations(expr) {
        if (expr.isNot) {
            if (expr.expr.isAtom || expr.expr.isExternal)
                return expr;
            if (expr.expr.isAnd)
                return new Ast.BooleanExpression.Or(expr.expr.operands.map(pushDownNegations));
            if (expr.expr.isOr)
                return new Ast.BooleanExpression.And(expr.expr.operands.map(pushDownNegations));
            if (expr.expr.isTrue)
            ...
            return new Ast.BooleanExpression.And(expr.operands.map(pushDownNegations));
        } else if (expr.isOr) {
            return new Ast.BooleanExpression.Or(expr.operands.map(pushDownNegations));
        } else {
            return expr;
        }
    })(filter);
    ...
    for (let and of ands) {
        ...
        clauses.push(new Ast.BooleanExpression.Or(currentClause));
    }
    return new Ast.BooleanExpression.And(clauses);
}
...
module.exports = class ToNNConverter {
    ...
    permissionRuleToNN(rule) {
        ...
        if (filter.isTrue) {
            ...
            const localschema = new Ast.ExpressionSignature('query', null, [], [
