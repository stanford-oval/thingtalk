...
module.exports = class ToNNConverter {
    constructor(sentence, entities, allocateEntities = false, typeAnnotations = true) {
    ...
    dialogueStateToNN(state) {
        ...
        assert(state.dialogueActParam === null || typeof state.dialogueActParam === 'string');
        if (state.dialogueActParam)
            list = List.concat(list, `param:${state.dialogueActParam}`);
        list = List.concat(list, ';');
