...
function filterToCNF(filter) {
    filter = (function pushDownNegations(expr) {
        if (expr.isNot) {
            if (expr.expr.isAtom || expr.expr.isExternal || expr.expr.isDontCare)
                return expr;
            if (expr.expr.isAnd)
                return new Ast.BooleanExpression.Or(null, expr.expr.operands.map(pushDownNegations));
            if (expr.expr.isOr)
                return new Ast.BooleanExpression.And(null, expr.expr.operands.map(pushDownNegations));
            if (expr.expr.isTrue)
...
function entityToString(entityType, entity) {
    ...
    // "tokenize"
    entityString = entityString.replace(/([^ ])([,.?!])/g, '$1 $2');
...
class EntityRetriever {
    ...
    _findEntityFromSentence(entityType, entity) {
        let entityString = entityToString(entityType, entity);
...
class SequentialEntityAllocator {
    constructor(entities, explicitStrings) {
        this.offsets = {};
...
module.exports = class ToNNConverter {
    constructor(sentence, entities, allocateEntities = false, typeAnnotations = true, explicitStrings = false) {
        this.typeAnnotations = typeAnnotations;
        if (allocateEntities)
            this.entityFinder = new SequentialEntityAllocator(entities, explicitStrings);
        else
    ...
    valueToNN(value, scope) {
        if (value.isArray) {
            ...
            if (value.value === null) {
                ...
                let date = this.findEntity('DATE', value, { ignoreNotFound: true });
                if (date === null) {
                    date = List.concat('new', 'Date', '(',
                        this.findEntity('QUOTED_STRING', new Ast.Value.String(value.value.toISOString())), ')');
                }
                return date;
            }
    ...
    permissionRuleToNN(rule) {
        ...
        if (filter.isTrue) {
            ...
            const localschema = new Ast.ExpressionSignature(null, 'query', null, [], [
                new Ast.ArgumentDef(null, Ast.ArgDirection.OUT, 'source', Type.Entity('tt:contact'), {}, {}),
    ...
    _historyItemToNN(item) {
        let sequence;
        if (item.stmt.isRule)
            sequence = List.concat(this.ruleToNN(item.stmt));
        else
            sequence = List.concat(this.commandToNN(item.stmt));
        if (item.results !== null)
            sequence = List.concat(sequence, this._historyResultToNN(item.results));
        else if (item.confirm)
            sequence = List.concat(sequence, '#[', 'confirm', '=', 'true', ']');
