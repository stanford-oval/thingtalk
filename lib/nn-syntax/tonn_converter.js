...
module.exports = class ToNNConverter {
    ...
    _scalarExpressionToNN(expr, schema) {
        if (expr.isPrimary)
            return this.valueToNN(expr.value, schema);
        if (expr.isDerived) {
            ...
            for (let operand of expr.operands) {
                ...
                list = List.concat(list, this._scalarExpressionToNN(operand, schema));
            }
            list = List.concat(list, ')');
            return list;
        }
        if (expr.isAggregation) {
            ...
            let ptype = schema.inReq[name] || schema.inOpt[name] || schema.out[name];
        ...
        if (expr.isFilter) {
            let name = expr.list.name;
            let ptype = schema.inReq[name] || schema.inOpt[name] || schema.out[name];
            return List.concat('filter', '(', this._listExpressionToNN(expr.list, ptype), ')');
        }
        throw new TypeError();
    }

    _listExpressionToNN(list, ptype) {
        ...
        if (list.filter) {
            ...
            listExpression = List.concat(listExpression, 'filter', this.cnfFilterToNN(cnf, localschema));
        }
        ...
        for (let and of filter.operands) {
            ...
            for (let or of and.operands) {
                ...
                if (or.isAtom) {
                    ...
                    let lhs = this._scalarExpressionToNN(or.lhs, schema);
                    orclause = List.concat(lhs, or.operator, this.valueToNN(or.rhs, schema));
                } else {
    ...
    streamToNN(stream) {
        if (stream.isVarRef) {
            ...
            return List.concat('compute',
                this._scalarExpressionToNN(stream.expression, stream.stream.schema), 'of', '(',
    ...
    tableToNN(table) {
        if (table.isVarRef) {
            ...
            return List.concat('compute',
                this._scalarExpressionToNN(table.expression, table.table.schema), 'of', '(',
