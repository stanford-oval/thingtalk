...
function currencySyntax(locale, result, entities) {
 ...
 *
 * Each program in `programs` should be an array of tokens. The array
 * will be modified in place with the new sequence of tokens.
 *
 * @param {string} locale - the locale to use for the conversion
 * @param {Array<Array<string>>} programs - array of programs to convert
 * @param {Object<string,any>} entities - entities present in the program
 * @param {string} thingtalk_version - the version of the ThingTalk library to target
 */
export default function applyCompatibility(locale, programs, entities, thingtalk_version) {
    for (let [range, fix] of COMPATIBILITY_FIXES) {
        if (semver.satisfies(thingtalk_version, range)) {
            for (let program of programs)
                fix(locale, program, entities);
        }
    }
}
