...
function compileStreamToOps(stream) {
    ...
    if (stream.isVarRef) {
        ...
        return new StreamOp.AtTimer(stream.time, stream.expiration_date);
...
function compileTableToOps(table, extra_in_params) {
    ...
    if (table.isVarRef) {
        ...
        return new TableOp.ReadResult(table.kind + ':' + table.channel, table.index, table.schema);
    } else if (table.isInvocation) {
        const device = table.invocation.selector;
        const query_language = table.schema.annotations['query_language'] ? table.schema.annotations['query_language'].value : null;
        ...
        if (table.in_params.length === 0) {
            ...
            const query_language = sameDevice(lhs.device, rhs.device) ? lhs.query_language : null;

            return new TableOp.CrossJoin(lhs, rhs, invocation, query_language);
        } else {
            let lhs_in_params = [];
            let rhs_in_params = [];
            ...
            return new TableOp.NestedLoopJoin(lhs, rhs, device, query_language);
        }
    } else {
        throw new TypeError();
    }
