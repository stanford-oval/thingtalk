...
function compileTableToOps(table, extra_in_params) {
    ...
    if (table.isVarRef) {
        ...
        if (table.in_params.length === 0) {
            ...
            const query_language = sameDevice(lhs.device, rhs.device) && lhs.query_language === rhs.query_language ? lhs.query_language : null;

            return new TableOp.CrossJoin(lhs, rhs, invocation, query_language, table);
        } else {
            let lhs_in_params = [];
            let rhs_in_params = [];
            ...
            return new TableOp.NestedLoopJoin(lhs, rhs, device, query_language, table);
        }
    } else {
        throw new TypeError();
    }
