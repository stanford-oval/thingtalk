...
function compileTableToOps(table, extra_in_params) {
    ...
    if (schema.is_db && !schema.isFederated()) {
        return compileDBTableToOps(table);
...
function compileDBTableToOps(table) {
    let db_server = table.schema.db_server;
    let sqlcompiler = new SqlCompiler(table, {});
    let sql = sqlcompiler.compile();
    return new TableOp.InvokeDBGet(table.invocation, db_server, sql);
}
