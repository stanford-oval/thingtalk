...
function compileStreamToOps(stream) {
    ...
    if (stream.isVarRef) {
        ...
        let op = compileStreamToOps(stream.stream);
        return new StreamOp.Map(op, new PointWiseOp.Projection(addMinimalProjection(stream.args, stream.schema)), op.ast);
        ...
        let streamOp = compileStreamToOps(stream.stream);
        let tableOp = compileTableToOps(stream.table, stream.in_params);
...
function compileTableToOps(table, extra_in_params) {
    ...
    if (table.isVarRef) {
        const compiled = new TableOp.InvokeVarRef(table.name, table.in_params.concat(extra_in_params), table);
        compiled.device = null;
        compiled.handle_thingtalk = false;
        return compiled;
    } else if (table.isResultRef) {
        const compiled = new TableOp.ReadResult(table.kind + ':' + table.channel, table.index, table.schema, table);
        compiled.device = null;
        compiled.handle_thingtalk = false;
        return compiled;
        ...
        return new TableOp.Map(
            compiled,
            new PointWiseOp.Projection(addMinimalProjection(table.args, table.schema)),
            compiled.device,
            compiled.handle_thingtalk,
            table
        );
    ...
    if (statement.isRule) {
        ...
        if (args.length > 0) {
            ...
            streamop = new StreamOp.Map(
                ...
                new Ast.Stream.Projection(null, statement.stream, args, schema)
            );
        }
    } else if (statement.table) {
        ...
        if (args.length > 0) {
            ...
            schema = schema.filterArguments((a) => a.is_input || args.includes(a.name));
            let newtable = new Ast.Table.Projection(null, statement.table, args, schema);
