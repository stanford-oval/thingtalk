...
class InvokeMonitor {
    ...
    codegen(prefix) {
        return `${prefix}_t_${this._into} = await env.invokeMonitor(${stringEscape(this._kind)}, ${objectToJS(this._attrs)}, ${stringEscape(this._fname)}, _t_${this._args}, ${this._once});`;
    }
}
...
class InvokeQuery {
    ...
    codegen(prefix) {
        return `${prefix}_t_${this._into} = await env.invokeQuery(${stringEscape(this._kind)}, ${objectToJS(this._attrs)}, ${stringEscape(this._fname)}, _t_${this._args});`;
    }
}
...
class InvokeStreamVarRef {
    ...
    codegen(prefix) {
        return `${prefix}_t_${this._into} = await __builtin.invokeStreamVarRef(env, __scope.${this._name}${this._args.map((a) => ', _t_' + a).join('')});`;
    }
}
...
class InvokeAction {
    ...
    codegen(prefix) {
        return `${prefix}await env.invokeAction(${stringEscape(this._kind)}, ${objectToJS(this._attrs)}, ${stringEscape(this._fname)}, _t_${this._args});`;
    }
}
...
class InvokeActionVarRef {
    ...
    codegen(prefix) {
        return `${prefix}await __scope.${this._name}(env${this._args.map((a) => ', _t_' + a).join('')});`;
    }
}
...
class IRBuilder {
    ...
    codegen() {
        for (let reg = this._nArgs; reg < this._nextRegister; reg++)
            this._root.declare(reg);
        return this._root.codegen('');
    }
    compile(scope) {
        let code = this.codegen();
        const args = ['__builtin', '__scope', 'env', ...this._extraArgs];
        for (let i = 0; i < this._nArgs; i++)
            args.push('_t_' + i);

        let f = new AsyncFunction(...args, code);
        return f.bind(null, Builtin, scope);
    }
    ...
    allocArgument() {
        assert(this._nArgs === this._nextRegister);
        var reg = this._nextRegister++;
        this._nArgs++;
        return reg;
    }
