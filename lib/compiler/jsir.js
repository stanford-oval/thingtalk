...
class InvokeTimer {
    ...
    codegen(prefix) {
        return `${prefix}_t_${this._into} = await __env.invokeTimer(_t_${this._base}, _t_${this._interval});`;
    }
}
...
class InvokeAtTimer {
    ...
    codegen(prefix) {
        return `${prefix}_t_${this._into} = await __env.invokeAtTimer(_t_${this._time});`;
    }
}
...
class InvokeDBQuery {
    ...
    codegen(prefix) {
        return prefix + '_t_' + this._into + ' = await __env.invokeDBQuery(' + '_t_' + this._db + ', _t_' + this._sql + ');';
    }
}
...
class IRBuilder {
    ...
    compile(scope) {
        ...
        const args = ['__builtin', '__scope', '__env', ...this._extraArgs];
        for (let i = 0; i < this._nArgs; i++)
            args.push('_t_' + i);
