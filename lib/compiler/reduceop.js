...
const { getRegister, typeForValue, compileValue } = require('./utils');
...
ReduceOp.SimpleArgMinMax = class SimpleArgMinMax extends ReduceOp {
    ...
    finish({ anyResult, value, tuple, outputType }, irBuilder, currentScope, varScopeNames) {
        ...
        for (let name of varScopeNames) {
            ...
            newScope.set(name, {
                type: 'scalar',
                tt_type: currentScope.get(name).tt_type,
                register: value
            });
        }
...
ReduceOp.ComplexArgMinMax = class ComplexArgMinMax extends ReduceOp {
    ...
    init(irBuilder, currentScope) {
        const base = compileValue(irBuilder, this.base, currentScope);
        const limit = compileValue(irBuilder, this.limit, currentScope);
...
ReduceOp.SimpleIndex = class SimpleIndex extends ReduceOp {
    ...
    init(irBuilder, currentScope) {
        const index = compileValue(irBuilder, this.index, currentScope);
...
ReduceOp.ComplexIndex = class ComplexIndex extends ArrayReduceOp {
    ...
    init(irBuilder, currentScope) {
        ...
        if (indicesType.isNumber)
            indices = compileValue(irBuilder, Ast.Value.Array(this.indices), currentScope);
        else
            indices = compileValue(irBuilder, this.indices[0], currentScope);
