...
const { typeForValue, compileValue } = require('./utils');
...
ReduceOp.SimpleIndex = class SimpleIndex extends ReduceOp {
    ...
    init(irBuilder, varScope, typeScope) {
        const index = compileValue(irBuilder, this.index, varScope);
    ...
    advance({ anyResult, index, counter }, irBuilder, varScope, typeScope, varScopeNames) {
    ...
    finish({ anyResult, value, tuple, outputType }, irBuilder, varScope, typeScope, varScopeNames) {
        ...
        return [varScope, typeScope, varScopeNames];
    }
};
...
class ArrayReduceOp extends ReduceOp {
    ...
    advance({ array }, irBuilder, varScope, typeScope, varScopeNames) {
        let resultAndTypeTuple = irBuilder.allocRegister();
        irBuilder.add(new JSIr.CreateTuple(2, resultAndTypeTuple));
        irBuilder.add(new JSIr.SetIndex(resultAndTypeTuple, 0, varScope.$output));
        irBuilder.add(new JSIr.SetIndex(resultAndTypeTuple, 1, varScope.$outputType));
...
ReduceOp.ComplexIndex = class ComplexIndex extends ArrayReduceOp {
    ...
    init(irBuilder, varScope, typeScope) {
        const indicesType = this.indices.length === 1 ?
            typeForValue(this.indices[0], typeScope) : Type.Number;

        let indices;
        if (indicesType.isNumber)
            indices = compileValue(irBuilder, Ast.Value.Array(this.indices), varScope);
        else
            indices = compileValue(irBuilder, this.indices[0], varScope);

        const state = super.init(irBuilder, varScope, typeScope);
