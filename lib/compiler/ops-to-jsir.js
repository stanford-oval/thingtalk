...
const { getRegister, typeForValue, compileBinaryOp, compileUnaryOp, compileValue, compileCast, isRemoteSend } = require('./utils');
...
module.exports = class OpCompiler {
    ...
    _readResultKey(result, key, fullName, type, isInVarScopeNames) {
        let reg = this._irBuilder.allocRegister();
        this._irBuilder.add(new JSIr.GetKey(result, key, reg));

        this._currentScope.set(fullName, {
            type: 'scalar',
            tt_type: type,
            register: reg,
            isInVarScopeNames
        ...
        if (type.isCompound) {
            ...
            for (let field in type.fields) {
                ...
                this._readResultKey(reg, field, fullName + '.' + field, fieldtype, false);
            }
            this._irBuilder.popBlock();
        }
    }
    ...
    _setInvocationOutputs(schema, argmap, typeAndResult) {
        ...
        for (let outArg of schema.iterateArguments()) {
            ...
            if (outArg.name.indexOf('.') >= 0)
                continue;
            this._readResultKey(result, outArg.name, outArg.name, outArg.type, false);
            // note: input parameters && __result do not participate in varScopeNames (which is used to
            // compare tuples for equality in monitor())
        }
        ...
        if (!schema.hasArgument('__response'))
            this._readResultKey(result, '__response', '__response', Type.String);
        ...
        for (let outArg of schema.iterateArguments()) {
            ...
            if (outArg.name.indexOf('.') >= 0)
                continue;
            this._readResultKey(result, outArg.name, outArg.name, outArg.type, true);
    ...
    _compileDatabaseQuery(tableop) {
        ...
        let query = new Ast.Input.Program(
            [],
            [],
            [new Ast.Statement.Command(tableop.ast, [notifyAction()])],
            null,
            []
        );
