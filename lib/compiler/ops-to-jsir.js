...
module.exports = class OpCompiler {
    ...
    _compileDatabaseQuery(tableop) {
        ...
        let list = this._irBuilder.allocRegister();
        let query = this._irBuilder.allocRegister();
        let q;
        if (tableop.query_language === 'thingtalk') {
            let program = new Ast.Input.Program(
                [],
                [],
            ...
            q = program.prettyprint();
        } else if (tableop.query_language === 'sql') {
            throw new NotImplementedError(tableop);
        } else {
            throw new NotImplementedError(tableop);
        }

        this._irBuilder.add(new JSIr.LoadConstant(Ast.Value.String(q), query));
        this._irBuilder.add(new JSIr.InvokeDBQuery(kind, attrs, list, query));
