...
module.exports = class OpCompiler {
    ...
    _compileAtTimer(streamop) {
        ...
        for (let i = 0; i < streamop.time.length; i++) {
          let time = compileValue(this._irBuilder, streamop.time[i], this._varScope);
          this._irBuilder.add(new JSIr.SetIndex(timeArray, i, streamop.time));
        }

        let expiration_date = compileValue(this._irBuilder, streamop.expiration_date, this._currentScope);
