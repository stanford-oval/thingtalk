...
const { prettyprint, prettyprintAllowed } = require('./prettyprint');
...
const Value = adt.data({
    ...
    Null: null, // a special placeholder for optional input arguments that are not specified
...
Value.prototype.getType = function getType() {
    const v = this;
    if (v.isVarRef || v.isUndefined || v.isNull)
        return Type.Any;
    if (v.isBoolean)
        return Type.Boolean;
    if (v.isString)
...
var Allowed = adt.newtype('Allowed', {
    kind: adt.only(String),
    channel: adt.only(String),
    channelType: adt.only('trigger', 'query', 'action'),
    precondition: adt.only(BooleanExpression),
    postcondition: adt.only(BooleanExpression),
    schema: adt.only(FunctionDef, null)
});
module.exports.Allowed = Allowed.seal();

module.exports.prettyprint = prettyprint;
module.exports.prettyprintAllowed = prettyprintAllowed;
