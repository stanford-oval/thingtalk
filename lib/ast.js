...
Value.fromJS = function fromJS(type, v) {
    ...
    if (type.isEntity)
        return new Value.Entity(v.value ? v.value : String(v), v.display||null);
    if (type.isMeasure)
...
Value.prototype.toJS = function toJS() {
    ...
    if (v.isDate)
        return Internal.normalizeDate(v.value, v.offset ? v.offset.toJS() : 0);
    return v.value;
};
...
ScalarExpression.type('Boolean', {
    value: adt.only(BooleanExpression)
});
...
const InputParam = adt.newtype('InputParam', {
    name: adt.only(String),
    value: adt.only(ScalarExpression)
});
...
Table.type('ArgMinMax', {
    ...
    field: adt.only(String),
    operator: adt.only(String),
    base: adt.only(ScalarExpression),
    limit: adt.only(ScalarExpression),
    schema: adt.only(FunctionDef, null)
});
Table.type('Join', {
    lhs: adt.only(Table),
...
var Stream = adt.data({
    ...
    Timer: {
        base: adt.only(Value.Date, Value.Undefined),
        interval: adt.only(Value.Measure, Value.Undefined),
        schema: adt.only(FunctionDef, null)
    },
    AtTimer: {
        time: adt.only(Value.Time, Value.Undefined),
        schema: adt.only(FunctionDef, null)
    },
    Monitor: {
        table: adt.only(Table),
...
Table.type('Window', {
    base: adt.only(ScalarExpression), // : Number
    delta: adt.only(ScalarExpression), // : Number
    stream: adt.only(Stream),
    schema: adt.only(FunctionDef, null)
});
Table.type('TimeSeries', {
    base: adt.only(ScalarExpression), // : Date
    delta: adt.only(ScalarExpression), // : Measure(ms)
    stream: adt.only(Stream),
    schema: adt.only(FunctionDef, null)
});
Table.type('Sequence', {
...
const Statement = adt.data({
    Declaration: {
        name: adt.only(String),
        type: adt.only('stream', 'table'),
        args: adt.only(Object), // maps name to Type
        value: adt.only(Stream, Table)
    },
    Rule: {
        stream: adt.only(Stream),
