...
const Ast = require('./class_def_ast');
...
module.exports = class SchemaRetriever {
    ...
    injectManifest(kind, manifest) {
        const classDef = Ast.ClassDef.fromManifest(kind, manifest);
        this.injectClassDef(classDef);
    }
    injectClass(classDef) {
        this._classCache[false].set(classDef.kind, classDef);
        // FIXME: we do not inject the class as meta cache (classCache[true])
        // because the manifest API from Thingpedia does not return translated
        // classes, whereas the /schema-metadata API endpoint does
    ...
    async _makeRequest(isMeta) {
        ...
        let resolved;
        // FIXME clean up the TpClient API...
        if (isMeta)
            resolved = await this._thingpediaClient.getMetas(pending);
        else
            resolved = await this._thingpediaClient.getSchemas(pending);

        const result = {};
        for (var kind in resolved) {
            try {
                result[kind] = makeSchemaClassDef(kind, resolved[kind], isMeta);
                this._classCache[isMeta].set(kind, result[kind]);
            } catch(e) {
                result[kind] = e;
            }
        }
