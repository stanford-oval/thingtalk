...
module.exports = class InputCompilerVisitor extends Visitor.RulePart {
    constructor(schemas, globalScope, modules, keywordDecls, feedAccess, scope, currentKeywords, forTrigger) {
        super();
    ...
    visitInvocation(ast) {
        ...
        return schema.then((schema) => {
            ...
            params.forEach((param, i) => {
                ...
                if (param.isNull) {
                ...
                } else if (param.isVarRef && !(param.name in this._scope)) {
                ...
                } else if (param.isVarRef || param.isConstant || !this._forTrigger) {
                    ...
                    // query is done
                    try {
                        var compiled = this.compileExpression(param, initialScope);
                        triggerParams.push(param);
                        queryInputs.push(compiled[1]);
                    } catch(e) {
                        console.log('Compiling in initial scope failed: ' + e.message);
                        var compiled = this.compileExpression(param, this._scope);
                        triggerParams.push(undefined);
                        queryInputs.push(() => undefined);
                    }
                    schema[i] = typeUnify(schema[i], compiled[0]);
                    var op = compiled[1];
                    paramfns.push(paramfns.push(function(env) {
    ...
    visitKeyword(ast) {
        ...
        params.forEach((param, i) => {
            ...
            if (param.isVarRef && !(param.name in this._scope)) {
                ...
                var expr = this.compileExpression(param, this._scope);
                decl.schema[i] = typeUnify(expr[0], decl.schema[i]);
                var op = expr[1];
                paramfns.push(function(env, value) {
    ...
    visitRegex(ast) {
        ...
        bindersast.forEach((binder, i) => {
            if (binder.isVarRef && !(binder.name in this._scope)) {
                ...
                var binderexp = this.compileExpression(binder, this._scope);
    ...
    visitContains(ast) {
        ...
        var arrayexp = this.compileExpression(argsast[0], this._scope);
        var type = null;
        try {
