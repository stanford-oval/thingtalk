...
function simpleCombine(semanticAction, flag) {
    return function(children) {
        ...
        if (!result.hasPlaceholders())
            return checkConstants(result, false); // FIXME: false
        else
            return result;
    };
}
...
class Grammar {
    ...
    addRule(symbol, expansion, combiner) {
        console.log(`addrule ${symbol} -> ${expansion}`);
        if (this._finalized)
            throw new Error(`Grammar was finalized, cannot add more rules`);
    ...
    _computeDistanceFromRoot() {
        ...
        while (queue.length > 0) {
            let [category, distance, parent] = queue.shift();
            if (distance > this._minDistanceFromRoot[category])
                continue;

            if (!this._rules[category])
                throw new Error(`Non terminal ${category} undefined, referenced by ${parent}`);
                ...
                for (let expansion of rule[0]) {
                    if (expansion instanceof NonTerminal) {
                        ...
                        if (!(distance+1 >= existingDistance)) { // undefined/NaN-safe comparison
                            ...
                            queue.push([expansion.symbol, distance+1, category]);
                        }
                    }
                }
            }
