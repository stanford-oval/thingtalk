...
function describeFilter(_, expr, schema, scope = {}) {
    return (function recursiveHelper(expr) {
        ...
        if (expr.isExternal) {
            ...
            const primdesc = describePrimitive(_, expr, 'query', scope);
            ...
            } else if (expr.filter.isNot && expr.filter.isAtom) {
                // common case 2
                return describeOperator(_, _("the %s of %s").format(expr.filter.name, primdesc),
                                        expr.filter.operator, describeArg(_, expr.filter.value, scope), true,
                                        expr.schema.out[expr.filter.name]);
            } else {
                // general case
