...
const { Currency } = require('./builtin_values');
...
class Describer {
    constructor(gettext, locale, timezone) {
        ...
        this._format = new FormatUtils(locale, timezone);
    }
    ...
    describeArg(arg, scope = {}) {
        ...
        if (arg.isCurrency)
            return new Currency(arg.value, arg.code).toLocaleString(this._locale);
        if (arg.isEnum)
    ...
    _describeAtomFilter(expr, schema, scope, negate) {
        let filter = expr;
    ...
    describeFilter(expr, schema, scope = {}) {
        const recursiveHelper = (expr) => {
            ...
            if (expr.isNot && expr.expr.isAtom)
                return this._describeAtomFilter(expr.expr, schema, scope, true);
            if (expr.isNot)
            ...
            if (expr.isExternal) {
                if (expr.selector.kind === 'org.thingpedia.builtin.thingengine.builtin' &&
                    ...
                    schema.argcanonicals[0] = this._("current time");
                    return this.describeFilter(expr.filter, schema, scope);
                }
                if (expr.selector.kind === 'org.thingpedia.builtin.thingengine.phone' &&
                    expr.channel === 'get_gps') {
                    const schema = expr.schema.clone();
                    schema.argcanonicals[0] = this._("my location");
                    return this.describeFilter(expr.filter, schema, scope);
                }
                ...
                if (expr.filter.isAtom) {
                    ...
                    return this._("for %s, %s").format(primdesc,
                        this.describeFilter(expr.filter, expr.schema, scope));
                }
            }
            return this._describeAtomFilter(expr, schema, scope, false);
        };
    ...
    describeTable(table, extraInParams) {
        if (table.isVarRef) {
            ...
            return desc.format(
                this._describeArgName(table.field, table.schema),
                this.describeTable(table.table, extraInParams)
            );
    ...
    _describeArgList(args, schema) {
        return args.map((argname) => this._describeArgName(argname, schema)).join(", ");
    }
    ...
    describeStream(stream) {
        if (stream.isVarRef) {
            ...
            return this._("%s and then get %s").format(
                this.describeStream(stream.stream),
    ...
    _describeDeclaration(d) {
        if (d.type === 'stream')
            ...
            return this._("let %s be %s").format(clean(d.name), this.describePrimitive(d.value));
    }
