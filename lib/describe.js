...
class Describer {
    ...
    describeArg(arg, scope = {}) {
        ...
        if (arg.isComputation) {
            if ((arg.op === '+' || arg.op === '-') &&
                ...
                let base = this.describeArg(arg.operands[0], scope);
                let offset = this.describeArg(arg.operands[1], scope);

                if (arg.op === '+')
                    return this._("%s past %s").format(offset, base);
                else
                    return this._("%s before %s").format(offset, base);
            }

            if (arg.op === '+' && arg.operands.every((op) => op.isMeasure))
                return arg.operands.map((v) => this.describeArg(v, scope)).join(' ');

            let operands = arg.operands.map((v) => this.describeArg(v, scope));
            switch (arg.op) {
            case '+':
                return this._("%s plus %s").format(operands[0], operands[1]);
            case '-':
                return this._("%s minus %s").format(operands[0], operands[1]);
            case '*':
                return this._("%s times %s").format(operands[0], operands[1]);
            case '/':
                return this._("%s divided by %s").format(operands[0], operands[1]);
            case '%':
                return this._("%s modulo %s").format(operands[0], operands[1]);
            case '**':
                return this._("%s to the power of %s").format(operands[0], operands[1]);
            case 'distance':
                return this._("the distance between %s and %s").format(operands[0], operands[1]);
            case 'max':
                return this._("the maximum %s").format(operands[0]);
            case 'min':
                return this._("the minimum %s").format(operands[0]);
            case 'avg':
                return this._("the average %s").format(operands[0]);
            case 'sum':
                return this._("the total %s").format(operands[0]);
            case 'count':
                return this._("the number of %s").format(operands[0]);
            default:
        ...
        if (arg.isFilter) {
            return this._("%s for which %s").format(this.describeArg(arg.value, scope),
                this.describeFilter(arg.filter, this._compoundTypeToSchema(arg.type.elem), scope));
        }
        if (arg.isArrayField)
            return this._("the %s of %s").format(arg.arg.canonical, this.describeArg(arg.value, scope));
    ...
    describeTable(table, extraInParams) {
        if (table.isVarRef) {
            ...
            return this._("%s and %s").format(
                this.describeTable(table.table, extraInParams),
                this.describeArg(table.expression, table.table.schema));
    ...
    describeStream(stream) {
        if (stream.isVarRef) {
            ...
            return this._("%s and %s").format(
                this.describeStream(stream.stream),
                this.describeArg(stream.expression, stream.stream.schema));
