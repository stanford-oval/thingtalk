...
class Describer {
    ...
    _describeDate(date, operator, offset) {
        ...
        if (offset && operator === '+') {
            return this._interp(this._("${offset} past ${base}"), {
                base,
                offset: this.describeArg(offset, {})
            });
        } else if (offset && operator === '-') {
            return this._interp(this._("${offset} before ${base}"), {
                base,
                offset: this.describeArg(offset, {})
            });
        } else {
    ...
    describeArg(arg, scope = {}) {
        if (arg.isArray)
            return arg.value.map((v) => this.describeArg(v, scope));
    ...
    describePrimitive(obj, scope, extraInParams = []) {
        ...
        for (const inParam of obj.in_params.concat(extraInParams)) {
            ...
            if (inParam.value.isUndefined && obj.schema.isArgRequired(argname))
                continue;
            argMap[argname] = this.describeArg(inParam.value, scope);
        }
        ...
        for (const inParam of obj.in_params.concat(extraInParams)) {
            ...
            const value = this.describeArg(inParam.value, scope);
    ...
    describeTable(table, extraInParams) {
        if (table.isVarRef) {
            ...
            return this._interp(this._("the ${param} of ${table}"), {
                table: this.describeTable(table.table, extraInParams),
                param: this._describeArgList(table.args, table.schema)
            });
        } else if (table.isCompute) {
            return this._interp(this._("a value computed from ${table}"), {
                table: this.describeTable(table.table, extraInParams)
            }); // FIXME
        ...
        // recognize argmin/argmax top K
            ...
            return this._interp(this._("${indices.length:plural:\
                ...
                indices: this.describeArg(Ast.Value.Array(table.indices)),
                table: this.describeTable(table.table, extraInParams),
            });
            ...
            if (table.base.isNumber && table.base.value === 1) {
                return this._interp(this._("the latest ${delta} results of ${table}"), {
                    delta: this.describeArg(table.delta),
    ...
    describeStream(stream) {
        if (stream.isVarRef) {
            ...
            return this._interp(this._("${frequency:plural:\
                ...
                frequency: stream.frequency !== null ? this.describeArg(stream.frequency) : 1,
                interval: this.describeArg(stream.interval),
                base: stream.base.value !== null ? this.describeArg(stream.base) : null
            });
        } else if (stream.isAtTimer) {
            return this._interp(this._("every day at ${times}${? until ${expiration}}"), {
                times: stream.time.map((t) => this.describeArg(t)),
                expiration: stream.expiration_date !== null ? this.describeArg(stream.expiration_date) : null
            });
            ...
            return this._interp(this._("the ${param} of ${stream}"), {
                stream: this.describeStream(stream.stream),
                param: this._describeArgList(stream.args, stream.schema),
            });
        }  else if (stream.isCompute) {
            return this._interp(this._("a value computed from ${stream}"), {
                stream: this.describeStream(stream.stream),
            }); // FIXME
    ...
    describePermissionFunction(permissionFunction, functionType, scope) {
        if (permissionFunction.isSpecified) {
            ...
            filterClone.operands.forEach((operand, i) => {
                ...
                argMap.set(argname, [this.describeArg(operand.value, scope), i]);
            });
    ...
    describePermissionRule(permissionRule) {
        ...
        if (permissionRule.principal.isTrue) {
            ...
            principal = this._interp(this._("anyone in the ${group} group"), {
                group: this.describeArg(permissionRule.principal.value)
        });
        } else {
    ...
    _describeBookkeeping(input) {
        ...
        if (intent.isSpecial) {
            ...
            return this._interp(this._("list the commands of ${device}, in category ${category}"), {
                device: this.describeArg(intent.device),
            ...
            return this._interp('${v}', { v: this.describeArg(intent.value) });
