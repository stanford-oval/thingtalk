...
module.exports = class ExpressionCompilerVisitor extends ExpressionVisitor {
    ...
    visitVarRef(name) {
        if (name === 'F' && this.compiler._feedAccess) {
            return [Type.Feed, function(env) {
                return env.readFeed();
            }];
        }
        if (name in this.compiler._keywords) {
            var decl = this.compiler._keywords[name];
            this.compiler._currentKeywords.push(Ast.Keyword(name, decl.feedAccess));
