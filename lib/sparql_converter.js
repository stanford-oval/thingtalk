...
//triplet required if there is a join
const get_join_triplet = `?v2 wdt:%s ?v1.\n`;

//triplet required if there is a projection
const get_projection_triplet = `?v1 wdt:%s ?v3.\n`;

//triplet required if there is a projection
const sort_results_triplet = `?v%s wdt:%s ?counter.\n`;

//for ordering results either ascending or descending
const order_template = `ORDER BY %s(?counter)`;
...
module.exports = {
    ...
    program_to_sparql(program) {
        ...
        if (
            ...
            direction = table["table"]["direction"];
        }
        ...
        if (table["lhs"] !== undefined || table["table"]["lhs"] !== undefined) {
            ...
            let rhs_param =
                table["rhs"]["args"][table["rhs"]["args"].length - 1];
            //triplet for joins
            let join_triplet = get_join_triplet.format(rhs_param);

            //if there is a sort
            if (field !== "") {
                //create a sparql query without sorting to identify output variable
                //the output variable is either v1, v2, or v3 depending on joins and projections
                ...
                sort_triplet = sort_results_triplet.format(output_var, field);
                order_results = order_template.format(direction);
            }
    ...
    filter_to_triplet(filter_input, schema_input, index) {
        ...
        } else if (`${type}` === "Number" || `${type}`.includes("Measure")) {
            let formatted_value = `"%s"^^xsd:decimal`.format(value);
            return [no_label_triplet.format(parameter, formatted_value), false];
