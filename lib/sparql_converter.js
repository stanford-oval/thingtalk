...
module.exports = {
  toSparql(input) {
    try {
      let table = input["rules"][0]["table"]["table"];
      return this.program_to_sparql(table);
    } catch (error) {
      //user inputted table directly
      let sparql_query = this.table_to_sparql(input).replace("?v2Label", "");
      return [sparql_query.replace("*Join*", ""), [0, 0]];
    }
  },

  program_to_sparql(table) {
    let slice = [0, 0];

    if (table["lhs"] !== undefined) {
      let lhs_sparql = this.table_to_sparql(table["lhs"]["table"]);
      let rhs_param = table["rhs"]["args"][1];
      lhs_sparql = lhs_sparql.replace("?vLabel", "");
      return [
        lhs_sparql.replace("*Join*", "?v2 wdt:%s ?v").format(rhs_param),
        slice
      ];
    } else if (table["table"]["base"] !== undefined) {
    ...
    let sparql_query = this.table_to_sparql(table).replace("?v2Label", "");
    return [sparql_query.replace("*Join*", ""), slice];
  },
