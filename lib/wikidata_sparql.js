...
module.exports = {
    ...
    table_to_subquery(
        ...
        if (table.isFilter) {
            ...
            outputVarIndex = queryCode[2];
            projectionObject = queryCode[3];
    ...
    filter_to_triplet(
        ...
        if (projectionObject[outputVarIndex] === undefined)
            projectionObject[outputVarIndex] = [[""]];
        else projectionObject[outputVarIndex].push([""]);
    ...
    build_query(body, additionalSparql, outputVarIndex, projectionObject) {
        body = body.reverse();
        ...
        for (let objectKey in projectionObject) {
            let projectionArray = projectionObject[objectKey];

            let filteredArray = [];
            if (projectionArray.length > 1) {
                for (let value of projectionArray)
                    if (value[0] !== "") filteredArray.push(value);

                projectionObject[objectKey] = filteredArray;
            } else {
                if (projectionArray[0][0] === "")
                    delete projectionObject[objectKey];
            }
        }
        ...
        for (let x = 0; x < keys.length; x++) {
            let reversedIndex = keys[keys.length - x - 1];
