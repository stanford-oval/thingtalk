...
module.exports = {
    ...
    table_to_subquery(
        ...
        if (table.isFilter) {
            ...
            let sparql_subject = table.schema.getAnnotation("wikidata_subject");
            ...
            if (Object.prototype.hasOwnProperty.call(varRefObj, table.name)) {
                ...
                let projection = varRefObj[table.name][0];
                let projection_property = [`${table.name}_${projection}`];

                let isId = varRefObj[table.name][1];

                if (projection_property[0].includes("id"))
                    projection_property[0] = `${table.name}_id:${sparql_subject}`;
    ...
    projection_to_triplet(projectionArray, index) {
        ...
        if (projectionId !== undefined) {
            let split = projectionId.split(":");
            let sparqlId = split[1];
            let parameterValue = split[0];
            if (sparqlId === undefined) sparqlId = projectionId.split("_")[1];

            id_triplet = `?v${index} wdt:P31/wdt:P279 wd:${sparqlId}.\n`;
            let id_index = projectionArray.indexOf(projectionId);
            if (id_index > -1) projectionArray.splice(id_index, 1);
            queryHeader += `(?v${index} as ?${parameterValue}) (?v${index}Label as ?${parameterValue}Label ) `;
            aliasLabels += `?v${index} rdfs:label ?v${index}Label. `;
        }

        let projection_triplets = "";

        for (let projection of projectionArray) {
            let raw_projection = projection;

            if (projection.includes("_")) {
                let split = projection.split("_");
                raw_projection = split[1];
            }
            projection_triplets += `?v${index} wdt:${raw_projection} ?${projection}.\n`;
            queryHeader += `?${projection} ?${projection}Label `;
            aliasLabels += `?${projection} rdfs:label ?${projection}Label. `;
        }

        return [projection_triplets + id_triplet, queryHeader, aliasLabels];
    },
    ...
    filter_to_triplet(
        ...
        if (!value.isVarRef) {
            if (parameter === "id") {
                return [
                    "",
                    `?v${tempVarIndex} wdt:P31/wdt:P279 wd:${idValue}.\n`,
                    index,
                    outputVarIndex,
                    varRefObj,
                    projectionObject,
        ...
        if (type.isString) {
            if (!isUnique) {
                get_label_triplet = `?item${temp_label_index} ?label${temp_label_index} '${value}'@en.\n`;
                value = `?item${temp_label_index}`;
                index += 1;
            } else {
                value = `wd:${value}`;
            }
        ...
        if (operator === "==" || operator === "contains") {
            ...
            return [
                ...
                ?compValue rdfs:label ?label${temp_label_index} .
                FILTER CONTAINS(?label${temp_label_index}, '${raw_value}').
                `,
                index,
                outputVarIndex,
                varRefObj,
    ...
    array_to_triplets(input_array, parameter, index, type) {
        ...
        for (var i = 0; i < input_array.length; i++) {
            ...
            if (type.elem.isEntity) {
                ...
                get_label_triplets += `?item${index + 1} ?label${index +
                    1} '${value}'@en.\n`;
    ...
    build_query(
        ...
        //creates array of triplets used in the body of the query
        let queryBodyArray = [];
        ...
        //gets the variable index of each triple
        let triplet_indexes = [];
        ...
        //reverse the projection the object
        let reversed = {};
        ...
        //get the right most instance of each projection
        let uniqueProjections = {};
        let counter = keys.length - 1;
        while (counter >= 0) {
            let index = keys[counter];
            let projectionArrays = projectionObject[index];
            if (projectionArrays !== undefined) {
                for (let projectionArray of projectionArrays) {
                    for (let projection of projectionArray) {
                        let split = projection.split(":");
                        projection = split[0];
                        split = projection.split("_");

                        if (split.length > 1) projection = split[1];

                        uniqueProjections[projection] = index;
                    }
                }
            }
        ...
        //the rightmost side always wins
        counter = keys.length - 1;
        while (counter >= 0) {
            let index = keys[counter];
            let projectionArrays = projectionObject[index];
            if (projectionArrays !== undefined) {
                for (let i = 0; i < projectionArrays.length; i++) {
                    let projectionArray = projectionArrays[i];
                    for (let j = 0; j < projectionArray.length; j++) {
                        let projection = projectionArray[j];
                        let split = projection.split(":");
                        projection = split[0];

                        if (uniqueProjections[projection] < index)
                            projectionObject[index][i].splice(j, 1);
                    }
                }
            }
        ...
        //Combine projection arrays if necessary, so they are all in one array
        ...
        //make sure there is only one projection of ID per table
        //e.g. lhs_id overides id
        for (let key of keys) {
            let projectionArrays = projectionObject[key];
            for (let projectionArray of projectionArrays) {
                let idIndexes = [];
                for (let i = 0; i < projectionArray.length; i++) {
                    let projection = projectionArray[i];
                    if (projection.includes("id")) idIndexes.push(i);
                }
                if (idIndexes.length > 1) {
                    for (let j = 0; j < projectionArray.length; j++) {
                        if (!projectionArray[j].includes("_"))
                            projectionArray.splice(j, 1);
                    }
                }
            }
        }
        ...
        //puts together the components of the sparql query
        for (let i = 0; i < queryBodyArray.length; i++) {
            let next_triplet = queryBodyArray[i];
            let triplet_index = triplet_indexes[i];

            if (projectionObject[triplet_index] !== undefined) {
                let projectionResult = this.projection_to_triplet(
                    projectionObject[triplet_index][0],
                    triplet_index
                );

                queryBody += next_triplet;
                queryBody += projectionResult[0];
                queryHeaders += projectionResult[1];
                aliasLabels += projectionResult[2];
        ...
        //default case if there are no projections
        if (queryHeaders === "") {
            queryHeaders = ["(?v1 as ?id) (?v1Label as ?idLabel) "];
            aliasLabels += "?v1 rdfs:label ?v1Label. ";
        }
        ...
        return `
        SELECT distinct ${queryHeaders} WHERE{
          ${queryBody}
