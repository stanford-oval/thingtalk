...
module.exports = {
    ...
    projection_to_triplet(projectionArray, index, schema) {
        ...
        if (projectionId !== undefined) {
            ...
            if (index > 1) {
                ...
                let canonical = schema.getArgument("id").metadata.canonical;
                canonical = canonical.replace(/ /g, "_");
                id_triplet = `?${canonical} wdt:P31 wd:${sparqlId}.\n`;
                queryHeader.push(`${canonical}`);
                let id_index = projectionArray.indexOf(projectionId);
                if (id_index > -1) projectionArray.splice(id_index, 1);
            }
        }
        ...
        for (let projection of projectionArray) {
            if (index > 1) {
                ...
                let canonical = schema.getArgument(projection).metadata
                    .canonical;
                //if there are special annotations
                if (canonical.default !== undefined) {
                    let defaultGrammarType = canonical.default;
                    canonical = canonical[defaultGrammarType][0];
                }

                canonical = canonical.replace(/ /g, "_");
                projection_triplets += `?v${index +
                    1} wdt:${projection} ?${canonical}.\n`;
                queryHeader.push(`${canonical}`);
            }
        }
    ...
    build_query(
        ...
        let queryHeader = "";
        for (let header of queryHeaders) queryHeader += `?${header}Label `;
