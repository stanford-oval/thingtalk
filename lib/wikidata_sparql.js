...
module.exports = {
    ...
    projection_to_triplet(projectionArray, index, adjustedShift) {
        ...
        if (projectionId !== undefined) {
            ...
            if (index > 1) {
                ...
                id_triplet = `?id wdt:P31 wd:${sparqlId}.\n`;
                queryHeader.push("id");
                let id_index = projectionArray.indexOf(projectionId);
                if (id_index > -1) projectionArray.splice(id_index, 1);
            }
        ...
        for (let projection of projectionArray) {
            if (index > 1) {
                projection_triplets += `?v${index +
                    1} wdt:${projection} ?v${index}.\n`;
            } else {
                if (adjustedShift) {
                    projection_triplets += `?id wdt:${projection} ?${projection}.\n`;
                    queryHeader.push(projection);
                } else {
                    projection_triplets += `?v${index +
                        1} wdt:${projection} ?${projection}.\n`;
                    queryHeader.push(projection);
                }
            }
        }

        return [projection_triplets + id_triplet, queryHeader];
    },
    ...
    build_query(
        ...
        if (projectionObject[1] !== undefined) {
            if (
                projectionObject[1][0].findIndex((element) =>
                    element.includes("id")
                ) >= 0 &&
                projectionObject[1][0].length > 1
            ) {
                requiredShift -= 1;
                adjustedShift = true;
            }
        ...
        for (let i = 0; i < queryBodyArray.length; i++) {
            ...
            if (
                ...
                let replace = `v${triplet_index + requiredShift}`;

                //this code ensures all instances of ?v1 are replaced with
                //the correct projection parameter
                if (triplet_index + requiredShift === 1) {
                    let split = projectionObject[1][0][0].split(":");
                    replace = split[0];
                }

                regex = new RegExp(find, "g");
                next_triplet = next_triplet.replace(regex, replace);
                ...
                if (triplet_index === 1) {
                    projectionResult = this.projection_to_triplet(
                ...
                queryBody += next_triplet + projectionResult[0];

                if (projectionResult[1].length > 0)
                ...
                find = `v${triplet_index}`;
                let replace = `v${triplet_index + requiredShift}`;
                if (triplet_index + requiredShift === 1) replace = "id";
        ...
        let queryHeader = "";
        for (let header of queryHeaders)
            queryHeader += `?${header} ?${header}Label `;
