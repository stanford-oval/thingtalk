...
export default class WikidataSparqlConverter {
    constructor(options) {
        this._priority = options.priority; // options.priority = speed / coverage
    } 
    ...
    convert(program) {
        ...
        let table = expression.toLegacy();
        assert(table instanceof Ast.Table);

        // HACK: remove extra projection in sort by count
        if (table.isProjection && table.args.length === 1 && table.args[0] === 'count' &&
            table.table.isCompute && 
            table.table.table.isSort && table.table.table.field === 'count' &&
            table.table.table.table.isCompute) 
            table = table.table.table;

        const [scope, proj] = this._tableToSubquery(table);

        // always have id in the projection
        for (const arg in scope) {
            if (arg === 'id' || arg.endsWith('.id')) {
                if (!proj.includes(arg)) 
                    proj.unshift(arg);
            }
        }
    ...
    _buildQuery(scope, proj) {
        ...
        const convertedProj = proj.map((pname) => {
            ...
            if (sparqlVar.startsWith('?table')) {
                // HACK: request for domain label as the minimum restriction for the query
                // in case there is no other restriction on each table
                // otherwise, wikidata will return nothing
                const tableIndex = sparqlVar.slice('?table'.length);
                getLabels += `?domain${tableIndex} rdfs:label ?domain${tableIndex}Label. `;
            } 
            
            if (scopeEntry.type.isEntity || (scopeEntry.type.isArray && scopeEntry.type.elem.isEntity))
                return `(${sparqlVar} as ?${cleanName}) (${sparqlVar}Label as ?${cleanName}Label)`;
            else return `(${sparqlVar} as ?${cleanName})`;
        });

        this._additionalProjection.forEach((proj) => convertedProj.push(proj));
        ...
        }${this._additionalSparql.join("\n")} LIMIT ${this._limitCount} OFFSET ${this._limitOffset}`;
    ...
    _tableToSubquery(table) {
        if (table.isInvocation) {
            ...
            if (!wikidataSubject)
                throw new Error(`Wikidata subject missing for function ${table.channel}`);

            const projection = table.schema.default_projection.length > 0 ? table.schema.default_projection : ['id'];
            const instanceOf = this._priority === 'speed' ? 'wdt:P31' : 'p:P31/ps:P31/wdt:P279*';
            if (Array.isArray(wikidataSubject)) {
                this._restrictDomains.push(wikidataSubject.map((subject) => 
                    `{?table${tableIndex} ${instanceOf} wd:${subject}}` 
                ).join(' UNION '));
            } else {
                this._restrictDomains.push(
                    `?table${tableIndex} ${instanceOf} wd:${wikidataSubject}`
                );
            }
            // HACK: always inject a "restriction" that every item satisfies
            // Wikidata SPARQL backend refuses to return anything if there is only
            // id filter and domain filter
            // this prevents such behavior without extra computation needed 
            this._whereClause.push(`?table${tableIndex} wdt:P31 ?domain${tableIndex}`);
            ...
            let sort_code;
            if (field === 'count' && table.table.isCompute) {
                const arg = table.table.expression.operands[0];
                sort_code = ` ORDER BY ${direction}(?p${scope[arg.name].paramId})`;
            } else {
                const filterLHS = this._getSparqlVar(scope, field);
                sort_code = ` ORDER BY ${direction}(${filterLHS})`;
            }
            ...
            const compute = table.expression;
            assert(compute.isComputation && compute.op === 'count');
            assert(compute.operands.length === 1 && compute.operands[0].isVarRef);
            const [scope, projection] = this._tableToSubquery(table.table);
            scope['count'] = {
                isId: false,
                isUnique: false,
                tableIndex: this._nextTableIndex - 1,
                paramId: this._nextParamId++,
                type: Type.Number,
            };
            const v = this._getSparqlVar(scope, compute.operands[0].name);
            this._additionalSparql.push(` GROUP BY ?table0 ?table0Label`);
            this._additionalProjection.push(`(COUNT(${v}) as ${v}_count)`);
            return [scope, projection];
    ...
    _convertOperator(operator, negate = false) {
    ...
    _mapFilter(table, filter, scope, negate = false) {
        if (filter.isTrue || filter.isDontCare) return [];
        ...
        if (filter.isCompute) {
            if (filter.lhs.isComputation && filter.lhs.op === 'count') {
                assert(filter.lhs.operands.length === 1 && filter.lhs.operands[0].isVarRef);
                const filterLHS = this._getSparqlVar(scope, filter.lhs.operands[0].name);
                const operator = this._convertOperator(filter.operator, negate);
                assert(filter.rhs.isNumber);
                const filterRHS = filter.rhs.toJS();
                this._additionalSparql.push(` GROUP BY ?table0 ?table0Label`, ` HAVING (${filterLHS}_count ${operator} ${filterRHS})`);
                this._additionalProjection.push(`(COUNT(${filterLHS}) as ${filterLHS}_count)`);
                return [];
            }
            throw new Error('Not implemented');
        }

        if (filter.isExternal || filter.isExistentialSubquery || filter.isComparisonSubquery)
            throw new Error('Not implemented');
        ...
        operator = this._convertOperator(operator, negate);
