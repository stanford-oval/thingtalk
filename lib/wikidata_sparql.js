...
module.exports = {
    ...
    projection_to_triplet(projectionArray, index) {
        ...
        if (projectionId !== undefined) {
            ...
            queryHeader.push(
                `(?v${index} as ?${parameterValue}) (?v${index}Label as ?${parameterValue}Label ) `
            );
        ...
        for (let projection of projectionArray) {
            ...
            queryHeader.push(`?${projection} ?${projection}Label `);
    ...
    build_query(
        ...
        for (let i = 0; i < queryBodyArray.length; i++) {
            ...
            if (projectionObject[triplet_index] !== undefined) {
                ...
                queryHeaders = queryHeaders.concat(projectionResult[1]);
        ...
        let queryHeader = "";
        for (let header of queryHeaders) queryHeader += header;

        for (let j = 0; j < additionalSparql.length; j++)
            additionalCode += additionalSparql[j];

        return `
      SELECT distinct ${queryHeader} WHERE{
        ${queryBody}
