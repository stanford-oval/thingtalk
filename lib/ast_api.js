...
ProgramProto.getIcon = function() {
    let icon = null;
    for (let [, prim] of this.iteratePrimitives()) {
        if (prim.selector.isBuiltin)
            continue;
        let newIcon = getIcon(prim);
        if (newIcon)
            icon = newIcon;
    }
    return icon;
...
Ast.Stream.prototype.iterateSlots = function* iterateSlotsStream(scope) {
    ...
    if (this.isVarRef) {
        ...
        //console.error(String(this))
        //console.error(scope)
        //console.error(this.schema.args)
        
        // let newScope = {};
        // for (let name of this.schema.args)
        //     newScope[name] = nestedScope[name];

        //console.error(this)
        // no primitive here
        //yield* this.filter.iterateSlots(this, prim, newScope);
        ...
        //console.error(nestedScope)
        let newScope = {};
        for (let name of this.args)
            //console.error(nestedScope[name]);
            newScope[name] = nestedScope[name];
        //console.error(newScope)
        return [prim, newScope];
...
Ast.Statement.prototype.iterateSlots = function* iterateSlots(scope) {
    if (this.isDeclaration) {
        yield* this.value.iterateSlots(scope);
    } else {
        if (this.isRule)
            [, scope] = yield* this.stream.iterateSlots(scope);
        if (this.isCommand && this.table)
            [, scope] = yield* this.table.iterateSlots(scope);
...
ProgramProto.iterateSlots = function* iterateSlots() {
    ...
    for (let decl of this.declarations)
        yield* decl.iterateSlots(scope);
    for (let rule of this.rules) 
        yield* rule.iterateSlots(scope);
};
...
Ast.Action.prototype.iteratePrimitives = function* iteratePrimitivesAction(includeVarRef) {
    if (this.isVarRef) {
        ...
        yield *this.rule.iteratePrimitives(includeVarRef);
    } else {
        throw new TypeError("Can't handle " + this);
    }
};
