...
const RuleProto = Object.getPrototypeOf(new Ast.Statement.Rule(new Ast.Stream.VarRef('', [], null), []));
const CommandProto = Object.getPrototypeOf(new Ast.Statement.Command(null, []));
...
RuleProto.iterateSlots = function* iterateSlots(scope) {
    [,scope] = yield* this.stream.iterateSlots(scope);
    for (let action of this.actions)
        yield* action.iterateSlots(scope);
};

CommandProto.iterateSlots = function* iterateSlots(scope) {
    if (this.table)
