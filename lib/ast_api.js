...
let { typeCheckFilter, typeCheckProgram, typeCheckPermissionRule, typeCheckDataset } = require('./typecheck');
...
const DatasetProto = Object.getPrototypeOf(new Ast.Input.Dataset('foo', 'en', []));
const ExampleProto = Object.getPrototypeOf(new Ast.Example('action', {}, new Ast.Invocation(Ast.Selector.Builtin, 'notify', [], null), []));
...
Ast.Input.prototype.typecheck = function(schemas, getMeta = false) {
    if (this.isProgram)
        ...
        return typeCheckDataset(this, schemas, getMeta).then(() => this);
};
