...
module.exports = class AppCompiler {
    ...
    compileOutputs(outputs, scope) {
        var visitor = new OutputCompilerVisitor(this._schemaRetriever,
                                                this._scope,
                                                this._modules,
                                                this._keywords,
                                                this._feedAccess,
                                                this._currentKeywords,
                                                scope);
        return visitor.visitOrderedSequence(outputs).then(() => {
            return visitor.outputs;
        });
    }

    compileRuleOrCommand(ast, isRule) {
        this._currentKeywords = [];
        ...
        return Q.try(() => {
            if (inputs !== null)
                return this.compileInputs(inputs, true, scope);
            else
                return null;
        }).then((compiledInputs) => {
            var compiledQueries = [];
    ...
    compileRule(ast) {
        return this.compileRuleOrCommand(ast, true);
    }

    compileCommand(ast) {
        return this.compileRuleOrCommand(ast, false);
    }
