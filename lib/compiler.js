...
const { prettyprintRule } = require('./prettyprint');
...
function compileStreamToOps(stream) {
    ...
    if (stream.isTimer) {
        return new StreamOp.Timer([stream.in_params[0], stream.in_params[1]]);
...
class RuleCompiler {
    ...
    _compileTimer(streamop) {
        ...
        let iterator = this._irBuilder.allocRegister();
        let base = compileValue(this._irBuilder, streamop.in_params[0], this._varScope);
        let interval = compileValue(this._irBuilder, streamop.in_params[1], this._varScope);
    ...
    _compileAtTimer(ast) {
        ...
        this._irBuilder.add(new JSIr.CreateTuple(ast.time.length, timeArray));
        for (let i = 0; i < ast.time.length; i++) {
            let time = compileValue(this._irBuilder, ast.time[i], this._varScope);
            this._irBuilder.add(new JSIr.SetIndex(timeArray, i, time));
        }

        this._irBuilder.add(new JSIr.InvokeAtTimer(iterator, timeArray));
    ...
    _compileAction(ast) {
        ...
        if (ast.isRule) {
            this._irBuilder.add(new JSIr.InvokeRule(prettyprintRule(ast.rule, '')));
