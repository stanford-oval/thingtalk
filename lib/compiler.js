    ...
    _typeCheckAll(trigger, queries, actions) {
        ...
        return Q.try(() => {
            ...
            function typeCheckQueryLoop(i) {
                if (i === queries.length)
                    return Q();
                return typeCheckInput(queries[i], this._schemaRetriever, scope, false, this._classes).then(function() {
            ...
            return Q.all(actions.map((action) => {
                return typeCheckOutput(action, this._schemaRetriever, scope, this._classes);
            }));
        });
    }
