...
module.exports = new lang.Class({
    ...
    compileInputInvocation: function(ast, scope) {
    ...
    compileInputs: function(inputs) {
        ...
        if (countInvocations > 2)
            throw new Error("Multiple invocations in a rule trigger are not allowed (use two rules)");
        ...
        return Q.try(function() {
            if (inputs[0].isInvocation) {
                return this.compileInputInvocation(inputs[0], scope).then(function(compiled) {
    ...
    compileOneOutput: function(output, scope) {
    ...
    compileOutputs: function(outputs, scope) {
    ...
    compileRule: function(ast) {
        ...
        if (queries.length > 0)
            throw new Error('Queries are not yet supported');
        return this.compileInputs(inputs).then(function(compiledInputs) {
            var scope = compiledInputs.scope;
            delete compiledInputs.scope;
            return this.compileOutputs(outputs, scope).then(function(compiledOutputs) {
                var retval = { inputs: compiledInputs, outputs: compiledOutputs };
