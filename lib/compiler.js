...
module.exports = class AppCompiler {
    ...
    _getSchemaForSelector(selector, name, builtins, inModule, schema) {
        if (selector.isBuiltin) {
            var builtinName = selector.name;
            if (!(builtinName in builtins))
                throw new TypeError('Unknown built-in ' + builtinName);
            return Q(builtins[builtinName]);
    ...
    compileInputInvocation(ast, forTrigger, scope) {
        ...
        var schema;
        if (forTrigger)
            schema = this._getSchemaForSelector(selector, name, Builtin.Triggers, 'events', 'triggers');
        else
            schema = this._getSchemaForSelector(selector, name, Builtin.Queries, '', 'queries');
    ...
    compileOneOutput(output, scope) {
        ...
        return Q.try(function() {
            if (output.isInvocation) {
                ...
                return this._getSchemaForSelector(output.selector, output.name, Builtin.Actions, 'functionSchemas', 'actions')
