...
function compileInputParams(ast, irBuilder, typeScope, varScope) {
    ...
    for (let inParam of ast.in_params) {
        let reg = compileValue(irBuilder, inParam.value, varScope);
        let ptype = ast.schema.inReq[inParam.name] || ast.schema.inOpt[inParam.name];
        reg = compileCast(irBuilder, reg, typeForValue(inParam.value, typeScope), ptype);
        irBuilder.add(new JSIr.SetIndex(tuple, ast.schema.index[inParam.name], reg));
    }
    return tuple;
}

function compileFilter(ast, irBuilder, typeScope, varScope) {
    return (function recursiveHelper(expr) {
        let cond = irBuilder.allocRegister();
        if (expr.isTrue) {
            ...
            const op = recursiveHelper(expr.expr);
            irBuilder.add(new JSIr.UnaryOp(op, '!', cond));
        } else if (expr.isExternal) {
            console.error('TODO: External predicate is not yet implemented');
            irBuilder.add(new JSIr.LoadConstant(Ast.Value.Boolean(false), cond));
        } else {
            ...
            if (ast.schema.inReq[filter.name] || ast.schema.inOpt[filter.name]) {
                ...
                lhs = irBuilder.allocRegister();
                irBuilder.add(new JSIr.GetIndex(varScope.$input, ast.schema.index[filter.name], lhs));
            } else if (ast.schema.out[filter.name]) {
                lhsType = ast.schema.out[filter.name];
                lhs = irBuilder.allocRegister();
                irBuilder.add(new JSIr.GetIndex(varScope.$output, ast.schema.index[filter.name], lhs));
            } else {
                lhsType = typeScope[filter.name];
                lhs = compileValue(irBuilder, Ast.Value.VarRef(filter.name), varScope);
            }
            lhs = compileCast(irBuilder, lhs, lhsType, filter.overload[0]);
            let rhs = compileValue(irBuilder, filter.value, varScope);
            rhs = compileCast(irBuilder, rhs, typeForValue(filter.value, typeScope), filter.overload[1]);
            let negate = false;
            if (op === '!=') {
                // lower into '!' + '='
                negate = true;
...
class RuleCompiler {
    ...
    _allocFunction(ast, type) {
        ...
        this._functions[id] = new Invocation(ast.selector, ast.channel, type);
        return id;
    }
    ...
    _filterAndSetOutputs(ast, args, channelAndResult) {
        ...
        if (!ast.filter.isTrue) {
            let filterResult = compileFilter(ast, this._irBuilder, this._typeScope, this._varScope);
    ...
    _compileTrigger(ast) {
        ...
        let args = compileInputParams(ast, this._irBuilder, this._typeScope, this._varScope);
        ...
        for (let action of this._rule.actions) {
            if (action.selector.isDevice && isRemoteSend(action)) {
                ...
                for (let inParam of action.in_params) {
                    if (inParam.name !== '__principal' && inParam.name !== '__token')
                        continue;
                    let reg = compileValue(this._irBuilder, inParam.value, this._varScope);
                    if (inParam.name === '__token')
                        uuid = reg;
                    else
                        principal = reg;
                }
                this._irBuilder.add(new JSIr.SendEndOfFlow(principal, uuid));
