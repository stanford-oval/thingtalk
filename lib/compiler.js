...
function compileTriggerOrQuery(ast, scope, inParamAccess, outParamAccess) {
    ...
    function compileFilter(expr) {
        ...
        if (expr.isTrue) {
            ...
            let filterOp = Builtin.BinaryOps[filter.operator];
            if (filterOp.op)
                irBuilder.add(new JSIr.BinaryOp(lhs, rhs, filterOp.op, cond));
            else if (filterOp.flip)
                irBuilder.add(new JSIr.FunctionOp(rhs, lhs, filterOp.fn, cond));
            else
                irBuilder.add(new JSIr.FunctionOp(lhs, rhs, filterOp.fn, cond));
