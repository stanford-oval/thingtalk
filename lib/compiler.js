...
const normalizeConstant = Ast.normalizeConstant;
...
module.exports = class AppCompiler {
    ...
    compileInputKeyword(ast, forTrigger, scope) {
        ...
        for (var i = 0; i < params.length; i++) {
            ...
            if (param.isVarRef) {
                ...
                var constexpr = this.compileExpression(param, {});
                if (decl.type.isTuple) {
    ...
    compileExpression(expression, scope) {
        var visitor = new ExpressionCompilerVisitor(scope, this);
        ...
        return schema.then(function(schema) {
            ...
            for (var i = 0; i < params.length; i++) {
                ...
                if (param.isNull) {
                    ...
                    channelParams.push(Ast.Expression.Constant(normalizeConstant(param.value)));
                } else {
    ...
    compileContains(argsast, scope) {
        ...
        if (argsast[1].isVarRef && !(argsast[1].name in scope)) {
            ...
            var filter = this.compileExpression(Ast.Expression.FunctionCall('contains', argsast), scope);
    ...
    compileInputs(inputs, forTrigger, scope) {
        ...
        return Q.try(function() {
            ...
            if (invocations.length < 1 && !forTrigger)
                throw new Error("Query must include exactly one invocation");

            if (invocations.length > 0) {
                return this.compileInputInvocation(invocations[0], forTrigger, scope).then(function(compiled) {
                    var invocation = { selector: compiled[0],
                                       name: compiled[1],
                                       params: compiled[2],
                                       fn: compiled[3] };
                    return invocation;
        ...
        }.bind(this)).then(function(invocation) {
            var inputFunctions = [];
            var filterFunctions = [];
            var memberBindings = [];
            ...
            if (invocation !== null) {
                inputFunctions.push(invocation.fn);
                delete invocation.fn;
            }

            for (var input of memberBindingParts) {
            ...
            for (var input of builtinPredicates)
                inputFunctions.push(this.compileBuiltinPredicate(input, scope));
            for (var input of bindings)
                filterFunctions.push(this.compileInputBinding(input, scope));
            for (var input of conditions)
                filterFunctions.push(this.compileCondition(input, scope));
