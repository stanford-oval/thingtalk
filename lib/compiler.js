...
class RuleCompiler {
    ...
    _compileQuery(ast) {
        ...
        if (ast.selector.isBuiltin) {
            ...
            this._irBuilder.add(new JSIr.GetTableVersion(version, ast.__table));
            this._versions[ast.__table] = version;

            let args = this._compileInputParams(ast);
            let aggregation = this._compileAggregation(ast);
            let list = this._irBuilder.allocRegister();
            this._irBuilder.add(new JSIr.InvokeMemoryQuery(list, args, aggregation));
