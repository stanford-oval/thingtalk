...
class ArgumentDef {
    ...
    toManifest() {
        return {
            ...
            json_key: this.annotations.json_key ? this.annotations.json_key.toJS(): undefined
        };
    }
    ...
    static fromManifest(manifest) {
        ...
        let metadata = {};
        if (manifest.question && manifest.question.length > 0)
            metadata.prompt = manifest.question;
        let annotations = {};
        if (manifest.json_key)
            annotations.json_key = new Ast.Value.String(manifest.json_key);
