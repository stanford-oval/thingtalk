...
const { prettyprintType, prettyprintValue } = require('./prettyprint');
...
class ArgumentDef {
    ...
    toString() {
        ...
        Object.entries(this.metadata).forEach(([name, value]) => {
            if (!(!value || 'value' in value && value.value.length === 0))
                annotations += ` #_[${name}=${prettyprintValue(value)}]`;
        });
    ...
    toManifest() {
        return {
            ...
            question: getString(this.metadata['prompt']) || '',
    ...
    fromManifest(manifest) {
        this.is_input = manifest.is_input;
        this.required = manifest.required;
        this.direction = this.is_input ? (this.required ? ArgDirection.IN_REQ : ArgDirection.IN_OPT) : ArgDirection.OUT;
        this.name = manifest.name;
        this.type = Type.fromString(manifest.type);
        this.metadata = {
            prompt: Ast.Value.String(manifest.question)
        };
        this.annotations = {};
    }
}
...
class ExpressionSignature {
    constructor(functionType, args, is_list, is_monitorable) {
        ...
        if (arguments.length === 2) {
            //load from manifest
            let manifest;
            [functionType, manifest] = arguments;
            args = manifest.args.map((a) => new ArgumentDef(a));
            is_list = manifest.is_list;
            is_monitorable = manifest.poll_interval !== -1;
        }
...
class FunctionDef extends ExpressionSignature {
    constructor(functionType, name, args, is_list, is_monitorable, metadata, annotations) {
        if (arguments.length === 3) {
            let manifest;
            [functionType, name, manifest] = arguments;
            super(functionType, manifest);
            annotations = this.is_monitorable ? { 'poll_interval': new Ast.Value.Measure(manifest.poll_interval, 'ms') } : {};
            metadata = {
                canonical: new Ast.Value.String(manifest.canonical || ''),
                confirmation: new Ast.Value.String(manifest.confirmation || ''),
                confirmation_remote: new Ast.Value.String(manifest.confirmation_remote || ''),
                formatted: new Ast.Value.fromJS(Type.Array(Type.Any), manifest.formatted)
            };
    ...
    toString(prefix = '') {
        ...
        Object.entries(this.metadata).forEach(([name, value]) => {
            if (!(!value || 'value' in value && value.value.length === 0))
                annotations += `\n${prefix}#_[${name}=${prettyprintValue(value)}]`;
        });
        Object.entries(this.annotations).forEach(([name, value]) => {
            if (!(!value || 'value' in value && value.value.length === 0))
    ...
    // for compatibility
    get canonical() {
        return getString(this.metadata.canonical);
    }
    get confirmation() {
        return getString(this.metadata.confirmation);
    }
    get confirmation_remote() {
        return getString(this.metadata.confirmation_remote);
    }

    get formatted() {
        return this.metadata.formatted ? this.metadata.formatted.toJS() : [];
    }
...
class ClassDef {
    constructor(kind, _extends, queries, actions, imports, metadata, annotations) {
        if (arguments.length === 1) {
            ...
            assert(_extends === null || typeof _extends === 'string');
    ...
    get loader() {
        return this.imports.find((i) => 'loader' in i.facets);
    }
        ...
        if (this.config) {
            switch (this.config.module) {
                ...
                case 'org.thingpedia.config.discovery':
                    return { type: 'discovery', protocol: this.config.in_params[0].value };
    ...
    toManifest() {
        ...
        return {
            ...
            kind: this.kind,
            name: getString(this.metadata.name),
            description: getString(this.metadata.description),
            category: getString(this.annotations.category),
            subcategory: getString(this.annotations.subcategory),
            params: this.params,
            auth: this.auth,
            queries,
            actions
    ...
    fromManifest(manifest) {
        ...
        this.actions = {};
        this.metadata = {
            name: Ast.Value.String(manifest.name),
            description: Ast.Value.String(manifest.description)
        };
        this.annotations = {
            category: Ast.Value.String(manifest.category),
            subcategory: Ast.Value.String(manifest.subcategory)
        };
        Object.entries(manifest.queries).forEach(([name, query]) => this.queries[name] = new FunctionDef('query', name, query));
        Object.entries(manifest.actions).forEach(([name, action]) => this.actions[name] = new FunctionDef('action', name, action));

        this.imports.push(new ImportStmt.Mixin(['loader'], manifest.module_type, []));
        let argmap = {};
        ...
        if (manifest.auth) {
            switch (manifest.auth.type) {
                ...
                case 'custom_oauth': {
                    ...
                    this.imports.push(new ImportStmt.Mixin(['config'], 'org.thingpedia.config.' + manifest.auth.type, params));
                    break;
                }
                case 'interactive':
                    this.imports.push(new ImportStmt.Mixin(['config'], 'org.thingpedia.config.interactive', []));
                    break;
                case 'discovery': {
                    let type = new Ast.InputParam('protocol', manifest.auth.protocol || manifest.auth.discoveryType);
                    this.imports.push(new ImportStmt.Mixin(['config'], 'org.thingpedia.config.discovery', [type]));
                    break;
                }
                case 'basic':
                    this.imports.push(new ImportStmt.Mixin(['config'], 'org.thingpedia.config.basic_auth', [new Ast.InputParam('extra_params', argmap)]));
                    break;
                case 'builtin':
                    this.imports.push(new ImportStmt.Mixin(['config'], 'org.thingpedia.config.builtin', []));
                    break;
                case 'none':
                    if (Object.keys(manifest.params).length > 0) {
                        this.imports.push(new ImportStmt.Mixin(['config'], 'org.thingpedia.config.form', [new Ast.InputParam('params', argmap)]));
                        break;
                    }
                default:
                    this.imports.push(new ImportStmt.Mixin(['config'], 'org.thingpedia.config.none', []));
            }
        }
    }
}
