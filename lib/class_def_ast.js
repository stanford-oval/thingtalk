...
class FunctionDef extends ExpressionSignature {
    constructor(functionType, name, args, is_list, is_monitorable, metadata, annotations) {
        if (arguments.length === 3) {
            ...
            metadata = {
                ...
                doc: new Ast.Value.String(manifest.doc || '')
            };
    ...
    get doc() {
        return getString(this.metadata.doc);
    }
        ...
        if (manifest.auth) {
            ...
            switch (manifest.auth.type) {
                ...
                case 'custom_oauth':
                    ...
                    break;
                case 'interactive':
                    this.imports.push(new ImportStmt.Mixin(['config'], 'org.thingpedia.config.interactive', params));
                    break;
                case 'discovery':
                    this.imports.push(new ImportStmt.Mixin(['config'], 'org.thingpedia.config.discovery', params));
                    break;
                case 'basic':
                    if (!(params.some((param) => param.name === 'extra_params')))
                        params.push(new Ast.InputParam('extra_params', argmap));
                    this.imports.push(new ImportStmt.Mixin(['config'], 'org.thingpedia.config.basic_auth', params));
                    break;
                case 'builtin':
                    this.imports.push(new ImportStmt.Mixin(['config'], 'org.thingpedia.config.builtin', params));
                    break;
                case 'none':
                    if (Object.keys(manifest.params).length > 0) {
                        ...
                        this.imports.push(new ImportStmt.Mixin(['config'], 'org.thingpedia.config.form', params));
                        break;
                    }
                default:
                    this.imports.push(new ImportStmt.Mixin(['config'], 'org.thingpedia.config.none', params));
            }
        }
    }
}
