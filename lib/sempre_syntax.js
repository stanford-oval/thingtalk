...
function parsePrimitive(schemaRetriever, primType, prim, getMeta = false, classes = []) {
    ...
    return Q.try(() => {
        ...
        var filter;
        if (filters.length > 0)
            filter = filters.reduce((lhs, rhs) => Ast.BooleanExpression.And(lhs, rhs));
        else
            filter = Ast.BooleanExpression.True;
        var ast = Ast.RulePart(sel, channel, inParams, filter, outParams);
        ast.schema = schema;
        return ast;
    });
}
...
function parseRule(schemaRetriever, json, getMeta = false) {
    ...
    return Q.all([parsePrimitive(schemaRetriever, 'triggers', json.trigger, getMeta, classes),
        ...
        if (action === null)
            action = Ast.RulePart(Ast.Selector.Builtin, 'notify', [], Ast.BooleanExpression.True, []);
