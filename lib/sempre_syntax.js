        ...
        prim.args.forEach((arg) => {
            ...
            if (arg.operator === 'is') {
                ...
                if (op === 'contains')
                    ...
                    op = 'contains';
                filters.push(Ast.BooleanExpression.Atom(argname, op, parseValue(arg)));
            } else {
                throw new TypeError('Invalid parameter name ' + argname);
            }
        });
...
function parseQuery(schemaRetriever, prim, getMeta = false) {
    return parsePrimitive(schemaRetriever, prim, 'queries', getMeta).then(([invocation, filter]) => {
        if (!invocation)
            return null;
        if (filter.isTrue)
            return new Ast.Table.Invocation(invocation, null);
        else
            return new Ast.Table.Filter(new Ast.Table.Invocation(invocation, null), filter, null);
    });
}
...
function parseRule(schemaRetriever, json, getMeta = false) {
    if (json.trigger)
        return Q.reject(new Error('Triggers do not exist any more, some manual processing is needed'));

    return Q.all([parseQuery(schemaRetriever, json.query, getMeta),
                  parseAction(schemaRetriever, json.action, getMeta)]).then(([query, action]) => {
        if (action === null)
            action = new Ast.Invocation(Ast.Selector.Builtin, 'notify', [], null);
        var rule = new Ast.Statement.Command(query, [action]);
