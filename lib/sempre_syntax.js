...
function parseTrigger(schemaRetriever, prim, getMeta = false) {
    return parsePrimitive(schemaRetriever, prim, 'queries', getMeta).then(([invocation, filter]) => {
        if (!invocation)
            return null;
        if (filter.isTrue)
            return new Ast.Stream.Monitor(new Ast.Table.Invocation(invocation, null), null);
        else
            return new Ast.Stream.Filter(new Ast.Stream.Monitor(new Ast.Table.Invocation(invocation, null), null), filter, null);
    });
}
...
function parseRule(schemaRetriever, json, getMeta = false) {
    return Q.all([parseTrigger(schemaRetriever, json.trigger, getMeta),
        ...
        if (trigger === null)
            ...
            rule = new Ast.Statement.Rule(new Ast.Stream.Join(trigger, query, null), [action]);
        else
