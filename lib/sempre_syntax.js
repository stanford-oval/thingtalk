...
function parsePrimitive(schemaRetriever, primType, prim) {
    if (!prim)
        return Q(null);

    let into = {};
    let [kind, channel] = handleSelector(prim.name);
    let principal = prim.person ? Ast.Value.Entity(prim.person, 'tt:contact_name') : null;
    ...
    return Q.try(() => {
        if (prim.dynamic_type)
            ...
            return Utils.getSchemaForSelector(schemaRetriever, sel.kind, channel, primType);
    }).then((schema) => {
...
function parseRule(schemaRetriever, json) {
    return Q.all([parsePrimitive(schemaRetriever, 'triggers', json.trigger),
                  parsePrimitive(schemaRetriever, 'queries', json.query),
                  parsePrimitive(schemaRetriever, 'actions', json.action)]).then(([trigger, query, action]) => {
        if (action === null)
...
function parseToplevel(schemaRetriever, json) {
    if (json.rule)
        return parseRule(schemaRetriever, json.rule);
    if (json.action || json.query || json.trigger)
        return parseRule(schemaRetriever, json);
