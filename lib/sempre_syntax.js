...
function parseTrigger(schemaRetriever, prim, getMeta = false) {
    return parsePrimitive(schemaRetriever, prim, 'queries', getMeta).then(([invocation, filter]) => {
        ...
        let stream = new Ast.Stream.Monitor(table, null);
        if (!filter.isTrue)
            stream = new Ast.Stream.Filter(stream, filter, null);
        return stream;
    });
}
    ...
    return Q.all([parseTrigger(schemaRetriever, json.trigger, getMeta),
        ...
        if (trigger === null) {
            ...
            while (!innermostQuery.isInvocation)
                innermostQuery = innermostQuery.query;
            let originalInParams = innermostQuery.invocation.in_params;
            innermostQuery.invocation.in_params = originalInParams.filter((p) => !p.value.isVarRef);
            let paramPassing = originalInParams.filter((p) => p.value.isVarRef);
