...
class SmtReduction {
    constructor(SolverClass, { allowUndefined = false, debug = false }) {
        ...
        let contactType = this._declareEntityType(new Type.Entity('tt:contact'));
        let contactGroupType = this._declareEntityType(new Type.Entity('tt:contact_group'));
    ...
    _entityToSmtValue(entityValue, entityType) {
        let smtType = this._declareEntityType(new Type.Entity(entityType));
    ...
    addGroups(principal, groups) {
        let lhs = smt.Predicate('Entity_tt_contact.getGroups', this._valueToSmtValue(principal, new Type.Entity('tt:contact')));
        let contactGroupType = this._declareEntityType(new Type.Entity('tt:contact_group'));
    ...
    addRule(principal, program, rule) {
        ...
        rule.actions.forEach((action, i) => {
            ...
            if (action.isInvocation && !isRemoteSend(action.invocation))
    ...
    _collectPrincipals(program, into) {
        for (let [, slot,,] of program.iterateSlots()) {
            if (slot instanceof Ast.Selector) {
                if (slot.principal && slot.principal.isEntity)
