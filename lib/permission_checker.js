...
class RuleTransformer {
    constructor(principal, program, rule, permissiondb, groupmap) {
    ...
    _isPermissionApplicable(permission) {
        ...
        if (filters.every((f) => this._mainReduction.isFilterTrue(f))) {
    ...
    _isFilterImplied(permission, permissionFunction, check) {
        ...
        if (filter.isFalse)
            return Q(false);
        if (!this._mainReduction.isFilterTrue(filter)) {
    ...
    _partiallyEvalFilter(expr, scope, inParamMap, previousPrimitiveDef = {}) {
        ...
        return (function recursiveHelper(expr) {
            ...
            if (lhs.isNull)
                throw new Error('Filter refers to unspecified parameter');
            if (lhs.isUndefined)
                throw new Error('Unexpected $undefined');
            if (lhs.isVarRef) {
                if (previousPrimitiveDef[lhs.name])
                    return Ast.BooleanExpression.Atom(Ast.Filter(previousPrimitiveDef[lhs.name], filter.operator, rhs));
                else
                    return Ast.BooleanExpression.Atom(Ast.Filter(lhs.name, filter.operator, rhs));
            } else {
        ...
        return promiseDoAll(this._relevantPermissions, (permission) => {
            return this._isPermissionApplicable(permission).then((isApplicable) => {
                ...
                return this._isFilterImplied(permission, permission.trigger, (reduction) => {
                    ...
                    if (isTriggerValid) {
                        permissionPredicate.push(Ast.BooleanExpression.True);
                        return;
                    }
                    ...
                    permissionPredicate.push(this._partiallyEvalFilter(permission.trigger.filter, scope, inParamMap));
                    ...
                    permissionPredicate.push(this._partiallyEvalFilter(permission.query.filter, scope, inParamMap));
                    ...
                    permissionPredicate.push(this._partiallyEvalFilter(permission.action.filter, scope, inParamMap, previousPrimitiveDef));
                }).then(() => {
                    newfilter.push(Ast.BooleanExpression.And(permissionPredicate));
                });
            });
        }).then(() => {
            //console.log(Ast.prettyprintFilterExpression(Ast.BooleanExpression.Or(newfilter)));
            let filter = optimizeFilter(Ast.BooleanExpression.Or(newfilter));
            if (filter.isFalse)
                return null;
            if (filter.isTrue)
            ...
            if (this._query)
                this._query.filter = Ast.BooleanExpression.And([this._query.filter, filter]);
            else if (this._trigger)
                this._trigger.filter = Ast.BooleanExpression.And([this._trigger.filter, filter]);
            else
                return null;
            return this._newrule;
        });
    ...
    transform() {
        ...
        return Q.try(() => {
            ...
            this._addProgram(this._mainReduction);

            let ors = [];
            for (let permission of this._relevantPermissions)
                ors.push(this._mainReduction.addPermission(permission));
            this._mainReduction.addAssert(smt.Not(smt.Or(...ors)));

            return this._mainReduction.checkSatisfiable(true).then((isSatisfiable) => {
                if (!isSatisfiable)
    ...
    check(principal, program) {
        return this._setProgram(principal, program).then(() => {
            ...
            return promiseDoAll(program.rules, (rule) => {
                let transformer = new RuleTransformer(principal, program, rule, this._permissiondb, this._groupmap);
