...
class SmtReduction {
    ...
    _booleanConstraintToSmt(fnvar, fndef, constr, isPrecondition) {
        ...
        if (filter.name === '__pi') {
            param = 'pi';
            if (filter.operator === 'group_member')
                valueType = Type.Entity('tt:contact_group');
            else
                valueType = Type.Entity('tt:contact');
        } else {
            param = 'param_' + fnvar + '_' + filter.name;
            ...
            if (filter.operator === 'contains')
                valueType = paramType.elem;
            else
                valueType = paramType;
        }
        let value = this._valueToSmtValue(filter.value, valueType);
    ...
    _addBuiltinNotify(ast, scope, allowedmap) {
        ...
        return this._addFunction(fn, 'actions', scope, ast, allowedmap);
    }
    ...
    _processFilter(ast, fnvar, schema, scope) {
        if (ast.isTrue)
            return 'true';
        if (ast.isFalse)
            return 'false';
        ...
        if (ast.isOr && ast.operands.length === 0)
            return 'false';
        if (ast.isAnd)
            return smt.And(...ast.operands.map((o) => this._processFilter(o, fnvar, schema, scope)));
        if (ast.isOr)
            return smt.Or(...ast.operands.map((o) => this._processFilter(o, fnvar, schema, scope)));
        if (ast.isNot)
            return smt.Not(this._processFilter(ast.expr, fnvar, schema, scope));
    ...
    _addFunction(fn, fnType, scope, originalFn, allowedmap) {
        ...
        this._asserts.push(this._processFilter(fn.filter, fnvar, fn.schema, scope));
...
module.exports = class PermissionChecker {
    ...
    check(principal, program) {
        return this._setProgram(principal, program).then(() => {
            ...
            return this._mainReduction.checkAllowed();
        }).then((isAllowed) => {
            ...
            })).then(() => optimizeProgram(this._newprogram));
        });
    }
    ...
    _adjustCommon(oldfn, newfn, newlastprimitive, lastPrimitiveDef, scope, newPrimitiveDef) {
        ...
        return promiseDoAll(allowedRules, (allowed) => {
            ...
            let postcondition = optimizeFilter(partiallyEvalFilter(Ast.BooleanExpression.And(postconditions), scope, inParamMap, this._groupmap, null));
            if (!precondition.isFalse && !precondition.isTrue && !newlastprimitive)
                throw new TypeError('??? Precondition is not statically true or false, but there is no primitive before?');
