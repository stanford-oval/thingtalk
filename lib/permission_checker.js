...
module.exports = class PermissionChecker {
            ...
            for (let i = 0; i < this._checkmap.length; i++) {
                ...
                for (let rule of rules) {
                    ...
                    if (assignment['constr_' + constridx] === true) {
                        ...
                        let constrrevmap = this._constrrevmap;
                        function checksub(sub) {
                            if (sub.isTrue || sub.isFalse)
                                return;
                            let constridx = constrrevmap.get(sub);
                            if (assignment['constr_' + constridx])
                                console.log('Sub constraint ' + sub + ' (' + constridx + ') is satisfied');
                            else
                                console.log('Sub constraint ' + sub + ' (' + constridx + ') is not satisfied');
                            if (!sub.isAtom) {
                                checksub(sub.lhs);
                                checksub(sub.rhs);
                            }
                        ...
                        if (!boolexpr.isAtom) {
                            checksub(boolexpr.lhs);
                            checksub(boolexpr.rhs);
                        }
                    } else {
    ...
    _addBuiltinNotify(scope) {
        return this._addFunction(Ast.RulePart(Ast.Selector.Device('builtin', null, null), 'notify', [], [], []), 'actions', scope);
    }
        ...
        return this._getSchema(fn, fnType).then((schema) => {
            ...
            for (let filter of fn.filters) {
                ...
                if (filter.value.isVarRef)
                    this._asserts.push(this._filterToSmt(filter.operator, pname, scope[filter.value.name]));
                else
                    this._asserts.push(this._filterToSmt(filter.operator, pname, this._valueToSmtValue(filter.value, ptype)));
            }
