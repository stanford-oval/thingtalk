...
module.exports = class PermissionChecker {
    ...
    _adjustRule(oldrule, newrule, assignment, constants) {
        if (oldrule.trigger)
            this._adjustTrigger(oldrule.trigger, newrule.trigger, assignment, constants);
        oldrule.actions.forEach((oldaction, i) => {
            this._adjustAction(oldaction, newrule.actions[i], newrule.trigger, assignment, constants);
        });
    }
    ...
    _adjustAction(oldaction, newaction, newlastprimitive, assignment, constants) {
        ...
        let rules = this._getRelevantRules(oldaction);
        console.log('Found ' + rules.length + ' relevant permission rules');
        if (rules.length === 0)
            throw new Error('No rule to make this function allowed'); // FIXME do something different
        ...
        for (let rule of rules)
            extraFilters.push(rule.constraint);
        let extraFilter = extraFilters.reduce((lhs, rhs) => Ast.BooleanExpression.Or(lhs, rhs));
    ...
    _adjustTrigger(oldtrigger, newtrigger, assignment, constants) {
        ...
        let rules = this._getRelevantRules(oldtrigger);
        console.log('Found ' + rules.length + ' relevant permission rules');
        if (rules.length === 0)
            throw new Error('No rule to make this function allowed'); // FIXME do something different
