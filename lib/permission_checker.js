    ...
    _partiallyEvalFilter(expr, inParamMap, previousPrimitiveDef) {
        ...
        return (function recursiveHelper(expr) {
            if (expr.isTrue || expr.isFalse)
                return expr;
            if (expr.isOr)
                return new Ast.BooleanExpression.Or(expr.operands.map(recursiveHelper));
            if (expr.isAnd)
                return new Ast.BooleanExpression.And(expr.operands.map(recursiveHelper));
            if (expr.isNot)
                return new Ast.BooleanExpression.Not(recursiveHelper(expr.expr));
        ...
        return promiseDoAll(this._relevantPermissions, (permission) => this._isPermissionApplicable(permission).then((isApplicable) => {
            ...
            return this._isFilterImplied(permission, permission.query, (reduction) => {
                ...
                for (let action of this._rule.actions) {
                    ...
                    querypredicate = new Ast.BooleanExpression.And([
            ...
            let queryfilter = new Ast.BooleanExpression.Or(newfilter).optimize();
