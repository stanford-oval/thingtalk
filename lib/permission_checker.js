...
module.exports = class PermissionChecker {
    ...
    check() {
        ...
        return this._mainReduction.checkAllowed().then((isAllowed) => {
            ...
            return Q.all(this._program.rules.map((oldrule, i) => {
                return this._adjustRule(oldrule, newprogram.rules[i]);
            })).then(() => newprogram);
        });
    }
    ...
    _adjustRule(oldrule, newrule) {
        ...
        return Q.try(() => {
            ...
            return Q.all(oldrule.actions.map((oldaction, i) => {
    ...
    _adjustCommon(oldfn, newfn, scope) {
        ...
        let isAllowed = this._mainReduction.isFunctionAllowed(oldfn);
        if (isAllowed)
        ...
        return Q.all(allowedRules.map((allowed) => {
            let isRulePossible = true; //
            let allowedPrecondition = isAllowed ? Ast.BooleanExpression.True : partiallyEvalFilter(allowed.precondition, scope, inParamMap);
            let allowedPostcondition = partiallyEvalFilter(Ast.BooleanExpression.Or([Ast.BooleanExpression.Not(allowed.precondition),
                allowed.postcondition]), scope, inParamMap);
            if (isRulePossible) {
                preconditions.push(allowedPrecondition);
                postconditions.push(allowedPostcondition);
            }
        })).then(() => {
            for (let outParam of newfn.out_params)
                scope[outParam.name] = outParam.value;

            return [optimizeFilter(Ast.BooleanExpression.Or(preconditions)),
                    optimizeFilter(Ast.BooleanExpression.And(postconditions))];
        });
    }
