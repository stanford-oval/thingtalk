...
module.exports = class PermissionChecker {
    ...
    _getRelevantRules(fn) {
        ...
        return rules.reduce((lhs, rhs) => Ast.BooleanExpression.Or(lhs, rhs), Ast.BooleanExpression.False);
    }
    ...
    _adjustAction(oldaction, newaction, newlastprimitive, scope, assignment, constants) {
        ...
        function partiallyEvalFilter(expr) {
            if (expr.isTrue || expr.isFalse)
                return expr;
            if (expr.isOr)
                return Ast.BooleanExpression.Or(partiallyEvalFilter(expr.lhs), partiallyEvalFilter(expr.rhs));
            if (expr.isAnd)
                return Ast.BooleanExpression.And(partiallyEvalFilter(expr.lhs), partiallyEvalFilter(expr.rhs));
            if (expr.isNot)
