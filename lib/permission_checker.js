...
class SmtReduction {
    ...
    allowed(allowed) {
        let fnvar = this._declareFunction(allowed.kind, allowed.channel, allowed.schema);
    ...
    _addRule(rule) {
        var scope = {};
        if (rule.trigger)
            this._addFunction(rule.trigger, 'triggers', scope, rule.trigger);
        for (let query of rule.queries)
            this._addFunction(query, 'queries', scope, query);
        for (let action of rule.actions) {
            if (action.selector.isBuiltin)
                this._addBuiltinNotify(action, scope);
            else
                this._addFunction(action, 'actions', scope, action);
        }
    }
    ...
    _addBuiltinNotify(ast, scope) {
        ...
        returnthis._addFunction(fn, 'actions', scope, ast);
    }
    ...
    _addFunction(fn, fnType, scope, originalFn) {
        var fnvar = this._declareFunction(fn.selector.kind, fn.channel, fn.schema);
...
module.exports = class PermissionChecker {
    ...
    check() {
        this._allowedmap.forEach((allowed) => {
            this._mainReduction.allowed(allowed);
        });
        this._mainReduction.addProgram(this._principal, this._program);
    ...
    _adjustCommon(oldfn, newfn, scope) {
        ...
        if (this._mainReduction.isFunctionAllowed(oldfn)) {
            console.log('Function ' + oldfn + ' is allowed');
            precondition = Ast.BooleanExpression.True;
        } else {
            console.log('Function ' + oldfn + ' is not (always) allowed');
