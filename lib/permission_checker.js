...
class SmtReduction {
    ...
    _addGetPredicate(ast, scope, scopeType) {
        ...
        let isList = !SINGLE_RESULT_FUNCTIONS.has(ast.selector.kind + ':' + ast.channel);
        if (isList)
    ...
    _declareUninterpretedFunction(kind, fn, def) {
        ...
        let isList = !SINGLE_RESULT_FUNCTIONS.has(kind + ':' + fn);
        let signames = [];
        let sigtypes = [];
        if (isList) {
    ...
    _addGet(prim, scope, scopeType, extraInParams) {
        ...
        let isList = !SINGLE_RESULT_FUNCTIONS.has(prim.selector.kind + ':' + prim.channel);
        if (isList)
...
class RuleTransformer {
    constructor(SolverClass, principal, program, rule, permissiondb, groupmap) {
        ...
        this._rule = rule;
        this._primKey = Array.from(Generate.iteratePrimitivesRule(rule));
        ...
        return promiseDoAll(this._relevantPermissions, (permission) => this._isPermissionApplicable(permission).then((isApplicable) => {
            ...
            return this._isFilterImplied(permission, permission.query, (reduction) => {
                ...
                if (this._newrule.stream) {
                    for (let [,in_param,,] of Generate.iterateSlotsStream(this._newrule.stream, {})) {
                    ...
                    for (let [,in_param,,] of Generate.iterateSlotsTable(this._newrule.table, {})) {
                        if (!(in_param instanceof Ast.InputParam))
                            continue;
                        if (in_param.name in inParamMap)
                            inParamMap[in_param.name] = undefined;
                ...
                querypredicate = this._partiallyEvalFilter(permission.query.filter, scope, inParamMap, {});
            }).then(() => {
                ...
                for (let action of this._rule.actions) {
                    ...
                    querypredicate = Ast.BooleanExpression.And([
                        querypredicate, this._partiallyEvalFilter(permission.action.filter, scope, inParamMap, previousPrimitiveDef.out)]);
                }
            }).then(() => {
                newfilter.push(querypredicate);
            });
        })).then(() => {
            let queryfilter = optimizeFilter(Ast.BooleanExpression.Or(newfilter));
...
module.exports = class PermissionChecker {
    ...
    _collectPrincipals(program, into) {
        for (let [, slot,,] of Generate.iterateSlots(program)) {
        ...
        return this._setProgram(principal, program).then(() => {
            ...
            return promiseDoAll(program.rules, (rule) => {
                ...
                if (newrules.length === 0)
                    return null;
                return optimizeProgram(new Ast.Program(program.classes, program.declarations, newrules, null));
            });
        });
    }
