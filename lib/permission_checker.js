...
module.exports = class PermissionChecker {
    ...
    _getRelevantRules(fn) {
        ...
        return Ast.BooleanExpression.Or(rules);
    }
    ...
    _adjustQuery(oldquery, newquery, newlastprimitive, scope, assignment, constants) {
        ...
        let inParamMap = {};
        for (let inParam of newquery.in_params)
            inParamMap[inParam.name] = inParam.value;

        let extraFilter = this._getRelevantRules(oldquery);

        // FINISHME
        newquery.filter = Ast.BooleanExpression.And([newquery.filter, extraFilter]);
    }
    ...
    _adjustAction(oldaction, newaction, newlastprimitive, scope, assignment, constants) {
        ...
        let inParamMap = {};
        for (let inParam of newaction.in_params)
            inParamMap[inParam.name] = inParam.value;

        let extraFilter = partiallyEvalFilter(this._getRelevantRules(oldaction));

        function partiallyEvalFilter(expr) {
            if (expr.isTrue || expr.isFalse)
                return expr;
            if (expr.isOr)
                return Ast.BooleanExpression.Or(expr.operands.map(partiallyEvalFilter));
            if (expr.isAnd)
                return Ast.BooleanExpression.And(expr.operands.map(partiallyEvalFilter));
            if (expr.isNot)
                return Ast.BooleanExpression.Not(partiallyEvalFilter(expr.expr));
        ...
        if (newlastprimitive === null)
            throw new Error('Cannot filter the action parameters with no trigger');
        newlastprimitive.filter = Ast.BooleanExpression.And([newlastprimitive.filter, extraFilter]);
    }
    ...
    _adjustTrigger(oldtrigger, newtrigger, scope, assignment, constants) {
        ...
        let extraFilter = this._getRelevantRules(oldtrigger);
        newtrigger.filter =  Ast.BooleanExpression.And([newtrigger.filter, extraFilter]);
    }
    ...
    _booleanConstraintToSmt(fn, fndef, constr) {
        ...
        if (constr.isAnd)
            return smt.Named('constr_' + name,
                smt.And(...constr.operands.map((o) => this._booleanConstraintToSmt(fn, fndef, o))));
        if (constr.isOr)
            return smt.Named('constr_' + name,
                smt.Or(...constr.operands.map((o) => this._booleanConstraintToSmt(fn, fndef, o))));
        if (constr.isNot)
            return smt.Named('constr_' + name,
    ...
    allowed(allowed) {
        return Utils.getSchemaForSelector(this._schemaRetriever,
            ...
            this._asserts.push(smt.Implies(
                this._booleanConstraintToSmt(fnvar, schema, optimizeFilter(allowed.constraint)),
