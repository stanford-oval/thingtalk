...
class SmtReduction {
    ...
    addFilterForFunction(fn, filter) {
        ...
        let fnvar = (fn.selector.isBuiltin ? 'builtin_notify' : fn.selector.kind.replace(/[^A-Za-z0-9_]/g, '_') + '_' + fn.channel) + '_' + name;
        let scope = {
            __pi: 'pi',
        };
        let scopeType = {
...
module.exports = class PermissionChecker {
    ...
    _isPostconditionImplied(allowed, newfn, postcondition) {
        ...
        reduction.addFilterForFunction(newfn,
            Ast.BooleanExpression.And([allowed.precondition, Ast.BooleanExpression.Not(allowed.postcondition)]));
        return reduction.checkValid();
    }
