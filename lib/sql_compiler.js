...
module.exports = class SqlCompiler {
    constructor(ast, scope) {
        this._ast = ast; // Ast.Stream or Ast.Table
        this._aliasIndex = 0;
        this._scope = scope;
    ...
    compile() {
        assert(this._ast instanceof Ast.Table || this._ast instanceof Ast.Stream);
        if (this._ast.isRule) return this._compileStream(this._ast);
        return this._compileTable(this._ast);
    }
    ...
    _compileInvocation(ast, cols='*', where='1') {
        ...
        let tablename = ast.invocation.schema.annotations['table'];
        let table = tablename ? tablename.value : `${ast.invocation.selector.kind}.${ast.invocation.channel}`;
