...
function prettyprintHistoryItem(ast, prefix) {
    let buffer = prettyprintStatement(ast.stmt, prefix, (ast.confirm || ast.results !== null) ? '\n' : ';\n');
    if (ast.results !== null)
        buffer += prettyprintHistoryResultList(ast.results, prefix) + ';\n';
    else if (ast.confirm)
        buffer += prefix + '#[confirm=true];\n';
    return buffer;
}
...
function prettyprintDialogueState(ast) {
    ...
    if (ast.dialogueActParam)
        buffer = `$dialogue @${ast.policy}.${ast.dialogueAct}(${ast.dialogueActParam});\n`;
    else
