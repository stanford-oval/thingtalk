...
function prettyprintAction(action) {
    if (action.isVarRef)
        ...
        return `{ ${prettyprintRule(action.rule, '')} }`;
    else
        throw new TypeError();
}
...
function prettyprintStream(stream) {
    if (stream.isVarRef)
        ...
        return `timer(base=${prettyprintValue(stream.in_params[0])}, interval=${prettyprintValue(stream.in_params[1])})`;
...
function prettyprintProgram(ast, short) {
    ...
    return prefix + '{\n' +
            ...
            ast.rules.map((rule) => prettyprintRule(rule)).join('') + '}';
}
