...
function prettyprintTable(table) {
    ...
    else if (table.isArgMinMax && table.limit === 1)
        return `aggregate ${table.operator} ${table.field} of ${prettyprintTable(table.table)}`;
    else if (table.isArgMinMax)
        return `aggregate ${table.operator} ${table.limit} ${table.field} of ${prettyprintTable(table.table)}`;
    else if (table.isJoin)
        return `(${prettyprintTable(table.lhs)} join ${prettyprintTable(table.rhs)})`;
    else if (table.isWindow)
        return `window from ${prettyprintScalarExpression(table.from)} to ${prettyprintScalarExpression(table.to)} of ${prettyprintStream(table.stream)}`;
    else if (table.isTimeSeries)
        return `timeseries from ${prettyprintScalarExpression(table.from)} to ${prettyprintScalarExpression(table.to)} of ${prettyprintStream(table.stream)}`;
    else
        throw new TypeError();
}
...
function prettyprintStream(stream) {
    if (stream.isVarRef)
        ...
        return `edge (${prettyprintTable(stream.table)}) on new`;
    else if (stream.isEdgeFilter)
        return `edge (${prettyprintTable(stream.table)}) on ${prettyprintFilterExpression(stream.filter)}`;
