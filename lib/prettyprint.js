...
function prettyprintAssignment(ast) {
    return `let result ${ast.name} := ${prettyprintTable(ast.value)};\n`;
}
...
function prettyprintAnnotations(ast, prefix = '  ') {
    let annotations = '';
...
function prettyprintOnInputChoice(ast) {
    let buffer = '    ';
    if (ast.table)
...
function prettyprintProgram(ast, short = true) {
    let buffer = '';
    ...
    for (let _class of ast.classes)
        buffer += prettyprintClassDef(_class);
    for (let decl of ast.declarations)
        buffer += prettyprintDeclaration(decl);
    for (let rule of ast.rules) {
        if (rule.isAssignment)
            buffer += prettyprintAssignment(rule);
        else
            buffer += prettyprintRule(rule);
    }
...
function prettyprintDataset(ast, prefix = '  ') {
    ...
    ast.examples.forEach((ex) => {
        examples += prettyprintExample(ex, '    ') + '\n';
    });
