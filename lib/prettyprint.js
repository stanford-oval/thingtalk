...
function prettyprintType(ast, prefix='') {
    if (ast.isTuple) {
        ...
        const fields = Object.keys(ast.fields)
            ...
            .map((f) => `${f}: ${prettyprintType(ast.fields[f].type, prefix + '  ')}`);
        return `{\n${prefix}  ${fields.join(',\n' + prefix + '  ')}\n${prefix}}`;
    } else {
        return ast.toString();
    }
...
function prettyprintFilterExpression(ast) {
    ...
    if (ast.isVarRef)
        return `${prettyprintSelector(ast.selector)}.${ast.name}(${ast.args.map(prettyprintInputParam).join(', ')})`;
...
function prettyprintScalarExpression(expr) {
    if (expr.isPrimary)
        ...
        return `${prettyprintSelector(expr.selector)}.${expr.name}(${expr.args.map(prettyprintInputParam).join(', ')})`;
    else
        throw new TypeError();
}
