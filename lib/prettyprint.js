...
function prettyprintPermissionRule(allowed) {
    return `source = ${allowed.principal === null ? '*' : prettyprintValue(allowed.principal)} : ${allowed.query.isBuiltin ? 'now' : prettyprintPermissionFunction(allowed.query)} => ${allowed.action.isBuiltin ? 'notify' : prettyprintPermissionFunction(allowed.action)};`;
}
