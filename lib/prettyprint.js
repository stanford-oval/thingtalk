...
${ast.imports.map((i) => prefix + prettyprintImportStmt(i) + '\n').join('')}${Object.keys(ast.queries).map((q) => ast.queries[q].toString(prefix + '  ') + '\n').join('')}${Object.keys(ast.actions).map((q) => ast.actions[q].toString(prefix + '  ') + '\n').join('')}${prefix}}\n`;
}
...
function prettyprintExample(ex, prefix = '') {
    ...
    if (ex.id >= 0)
        annotations += `\n${prefix}    #[id=${ex.id}]`;
    for (let key in ex.metadata)
        annotations += `\n${prefix}    #[${key}=${typeof ex.metadata[key] === 'string' ? stringEscape(ex.metadata[key]) : ex.metadata[key]}]`;

    return `${prefix}${ex.type} ${arg_decl} := ${value}${annotations};\n`;
}

function prettyprintDataset(ast) {
    let examples = '';
