...
function prettyprintDate(value) {
    ...
    } else if (value.isWeekDayDate) {
        if (value.time === null)
            return `new Date(${value.weekday})`;
        else
            return `new Date(${value.weekday}, ${value.time.value.hour}, ${value.time.value.minute}, ${value.time.value.second})`;
    } else {
...
function prettyprintValue(ast) {
    ...
    else if (ast.isRecurrentTimeSpecification)
...
function prettyprintClassDef(ast, prefix = '  ') {
    ...
    const queries = Object.keys(ast.queries).map((q) => ast.queries[q].toString(prefix + '  ') + '\n').join('\n');
    const actions = Object.keys(ast.actions).map((q) => ast.actions[q].toString(prefix + '  ') + '\n').join('\n');
