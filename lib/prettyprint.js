...
function prettyprintDate(value, offset) {
    let base;
    if (value === null)
        base = 'makeDate()';
    ...
    return base + (offset ? ` + delta(${prettyprintValue(offset)})` : '');
}
...
function prettyprintValue(ast) {
    if (ast.isVarRef)
        ...
        return prettyprintDate(ast.value, ast.offset);
    else if (ast.isTime)
...
function prettyprintInputParam(ast) {
    return ast.name + '=' + prettyprintScalarExpression(ast.value);
}
...
function prettyprintFilterExpression(ast) {
    ...
    if (INFIX_FILTERS.has(ast.operator))
        return `${prettyprintScalarExpression(ast.lhs)} ${ast.operator} ${prettyprintScalarExpression(ast.rhs)}`;

    return `${ast.operator}(${prettyprintScalarExpression(ast.lhs)}, ${prettyprintScalarExpression(ast.rhs)})`;
}
...
function prettyprintScalarExpression(expr) {
    if (expr.isPrimary)
        ...
        return prettyprintFilterExpression(expr.value);
    else
        throw new TypeError();
}
...
function prettyprintTable(table) {
    if (table.isVarRef)
        ...
        return `aggregate count of ${prettyprintTable(table.table)}`;
    else if (table.isAggregation)
        return `aggregate ${table.operator} ${table.field} of ${prettyprintTable(table.table)}`;
    else if (table.isArgMinMax)
        return `aggregate ${table.operator} ${prettyprintScalarExpression(table.base)}, ${prettyprintScalarExpression(table.limit)} ${table.field} of ${prettyprintTable(table.table)}`;
    else if (table.isJoin)
        return `(${prettyprintTable(table.lhs)} join ${prettyprintTable(table.rhs)})`;
    else if (table.isWindow)
        return `window ${prettyprintScalarExpression(table.base)}, ${prettyprintScalarExpression(table.delta)} of ${prettyprintStream(table.stream)}`;
    else if (table.isTimeSeries)
        return `timeseries ${prettyprintScalarExpression(table.base)}, ${prettyprintScalarExpression(table.delta)} of ${prettyprintStream(table.stream)}`;
    else if (table.isSequence)
        return `sequence ${prettyprintScalarExpression(table.base)}, ${prettyprintScalarExpression(table.delta)} of ${prettyprintTable(table.table)}`;
    else if (table.isHistory)
        return `history ${prettyprintScalarExpression(table.base)}, ${prettyprintScalarExpression(table.delta)} of ${prettyprintTable(table.table)}`;
    else
        throw new TypeError();
}
...
function prettyprintStream(stream) {
    if (stream.isVarRef)
        ...
        return `monitor (${prettyprintTable(stream.table)})`;
...
function prettyprintDeclaration(decl) {
    ...
    return `    let ${decl.type} ${decl.name} := ${arg_decl}${decl.type === 'stream' ? prettyprintStream(decl.value) : prettyprintTable(decl.value)};\n`;
}
