...
function prettyprintDate(value, offset) {
    ...
    return base + (offset ? ` + ${prettyprintValue(offset)}` : '');
}
...
function prettyprintTable(table) {
    if (table.isVarRef)
        ...
        return `compute ${prettyprintScalarExpression(table.expression)} ${table.alias !== null ? `as ${table.alias} ` : ''}of (${prettyprintTable(table.table)})`;
    else if (table.isAggregation && table.operator === 'count' && table.field === '*')
        return `aggregate count ${table.alias !== null ? `as ${table.alias} ` : ''}of ${prettyprintTable(table.table)}`;
    else if (table.isAggregation)
        return `aggregate ${table.operator} ${table.field} ${table.alias !== null ? `as ${table.alias} ` : ''}of ${prettyprintTable(table.table)}`;
...
function prettyprintStream(stream) {
    if (stream.isVarRef)
        ...
        return `compute ${prettyprintScalarExpression(stream.expression)} ${stream.alias !== null ? `as ${stream.alias} ` : ''}of (${prettyprintStream(stream.stream)})`;
