...
function prettyprintAggregationParam(ast) {
    ...
    if (aggregation.startsWith('argm')) {
        ...
        if (cols.length === 0) {
            ...
            names = '[' + ast.out_params.map((out) => out.name).join(', ') + ']';
            params = '[' + cols.join(', ') + ']';
        }
...
function prettyprintRulePart(ast) {
    if (ast.selector.isBuiltin) {
        if (ast.channel === 'log.get') {
            let query = ast.channel + '(' + ast.in_params.map(prettyprintInputParam).join(', ') + ')' +
                (!ast.filter.isTrue ? ', ' + prettyprintFilterExpression(ast.filter) : '');
            if (ast.aggregation)
                return query + prettyprintAggregationParam(ast);
            else
                return query + ast.out_params.map(prettyprintOutputParam).join('');
        }
        return ast.channel;
    }
