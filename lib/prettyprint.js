...
function prettyprintFilter(ast) {
    if (ast.operator === 'contains' || ast.operator === 'group_member')
...
function prettyprintAllowed(allowed) {
    return 'Allowed' + allowed.channelType[0].toUpperCase() + allowed.channelType.substr(1)
        + '(_, @' + allowed.kind + '.' + allowed.channel + ', ' +
        prettyprintFilterExpression(allowed.precondition) + ', ' +
        prettyprintFilterExpression(allowed.postcondition) + ')';
}
