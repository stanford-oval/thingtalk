...
function prettyprintExternalFilter(ast) {
    if (ast.selector.isBuiltin) {
        ...
        return `get_record(${ast.in_params.map(prettyprintInputParam).join(', ')}) { ${prettyprintFilterExpression(ast.filter)} }`;
    } else {
...
function prettyprintAggregationParam(ast) {
    let aggregation = ast.aggregation.type;
    let field = ast.aggregation.field;
    if (aggregation.startsWith('argm')) {
        let cols = ast.aggregation.cols;
        let names = '[' + ast.out_params.map((out) => out.name).join(', ') + ']';
        let count = ast.aggregation.count;
        let params = '[' + cols.join(', ') + ']';
        if (count)
            return ', ' + names + ' := ' + aggregation + '(' + field + ', ' + count + ', ' + params + ')';
        return ', ' + names + ' := ' + aggregation + '(' + field + ', ' + params + ')';
    } else {
        let name = ast.out_params[0].name;
...
function prettyprintRulePart(ast) {
    if (ast.selector.isBuiltin) {
        if (ast.channel === 'get_record' || ast.channel === 'new_record') {
            let part = ast.channel + '(' + ast.in_params.map(prettyprintInputParam).join(', ') + ')' +
                (!ast.filter.isTrue ? ', ' + prettyprintFilterExpression(ast.filter) : '');
            if (ast.aggregation)
                return part + prettyprintAggregationParam(ast);
            else
                return part + ast.out_params.map(prettyprintOutputParam).join('');
        } else {
            return ast.channel;
        }
    }
...
function prettyprintRule(ast) {
    if (ast.queries.length > 0) {
        return '    ' + (ast.table !== null ? ('table = ' + stringEscape(ast.table) + ': ') : '') + (ast.once ? 'once ' : '') + prettyprintTrigger(ast.trigger) + ' => ' + ast.queries.map(prettyprintRulePart).join(' => ') +
            ' => ' + prettyprintActions(ast.actions);
    } else {
        return '    ' + (ast.table !== null ? ('table = ' + stringEscape(ast.table) + ': ') : '') + (ast.once ? 'once ' : '') + prettyprintTrigger(ast.trigger) + ' => ' + prettyprintActions(ast.actions);
    }
}
