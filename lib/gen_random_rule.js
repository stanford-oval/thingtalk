...
function applyFilters(invocation, { applyHeuristics, allowUnsynthesizable, applyFiltersToInputs, actionArgConstantProbability, argConstantProbability, requiredArgConstantProbability, filterClauseProbability }, isAction) {
    ...
    function makeFilterClause() {
        ...
        args.forEach((argname) => {
            ...
            if ((!allowUnsynthesizable || !applyFiltersToInputs) && isInput)
                return;
    ...
    if (isAction) {
        ...
        if (allowUnsynthesizable) {
            while (coin(filterClauseProbability))
                filterClauses.push(makeFilterClause());
        }

        filter = filterClauses.length === 1 ? filterClauses[0] : Ast.BooleanExpression.Or(filterClauses);
