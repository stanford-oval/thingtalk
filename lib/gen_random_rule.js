...
function applyFilters(invocation, options, isAction) {
    ...
    if (invocation.kind === '$builtin')
        sel = Ast.Selector.Builtin;
    else
        sel = Ast.Selector.Device(invocation.kind, null, null);
    var ret= new Ast.RulePart(sel, invocation.name, [], filter, outParams, invocation);
    return ret;
}
...
function genOneRandomRule(schemaRetriever, schemas, options) {
    return chooseRule(schemaRetriever, schemas, options).then(([triggerMeta, queryMeta, actionMeta]) => {
        ...
        return optimizeProgram(new Ast.Program('AlmondGenerated', [], [], [rule], null));
    }).then((prog) => {
        if (!DEBUG)
            return prog;
