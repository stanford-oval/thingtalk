...
const InvocationProto = Ast.Invocation.prototype;
...
function notifyAction(what = 'notify') {
    return new Ast.Action.Invocation(null,
        new Ast.Invocation(null, Ast.Selector.Builtin, what, [], Builtin.Actions[what]),
...
function declarationLikeToProgram() {
    ...
    if (this.type === 'action') {
        ...
        program = new Ast.Program(null, [], [],
            [new Ast.Statement.Command(null, this.value.clone(), [notifyAction()])], null);
    } else if (this.type === 'stream') {
        program = new Ast.Program(null, [], [],
            [new Ast.Statement.Rule(null, this.value.clone(), [notifyAction()])], null);
    } else {
...
function convertPrimitiveToPermission(prim) {
    ...
    for (let inParam of prim.in_params) {
        if (inParam.value.isUndefined)
            continue;
        filter.push(new Ast.BooleanExpression.Atom(null, inParam.name, '==', inParam.value));
    }
    filter = new Ast.BooleanExpression.And(null, filter);
    return new Ast.PermissionFunction.Specified(null, prim.selector.kind, prim.channel,
        filter, prim.schema);
}
...
function convertTableToPermissionFunction(table) {
    ...
    if (table.isFilter) {
        ...
        if (!inner)
            return inner;
        return new Ast.PermissionFunction.Specified(null, inner.kind, inner.channel,
            new Ast.BooleanExpression.And(null, [inner.filter, table.filter]), inner.schema);
    }
...
function convertStreamToPermissionFunction(stream) {
    ...
    if (stream.isFilter || stream.isEdgeFilter) {
        ...
        if (!inner)
            return inner;
        return new Ast.PermissionFunction.Specified(null, inner.kind, inner.channel,
            Ast.BooleanExpression.And(null, [inner.filter, stream.filter]), inner.schema);
    }
...
ProgramProto.convertToPermissionRule = function(principal, contactName) {
    ...
    return new Ast.PermissionRule(null, new Ast.BooleanExpression.Atom(null,
        'source', '==',
        new Ast.Value.Entity(principal, 'tt:contact', contactName)
    ), query, action);
};
...
Ast.BooleanExpression.prototype.iteratePrimitives = function* iteratePrimitivesFilter() {
    ...
    if (this.isCompute) {
        if (this.lhs.isAggregation && this.lhs.list.filter)
            yield* this.lhs.list.filter.iteratePrimitives();
        return;
    }
    if (this.isVarRef) {
        // TODO
...
function makeSendSchema(sendFrom, secondSendFrom) {
    const args = [
        new Ast.ArgumentDef(null, Ast.ArgDirection.IN_REQ, '__principal', Type.Entity('tt:contact')),
        new Ast.ArgumentDef(null, Ast.ArgDirection.IN_REQ, '__program_id', Type.Entity('tt:program_id')),
        new Ast.ArgumentDef(null, Ast.ArgDirection.IN_REQ, '__flow', Type.Number),
        new Ast.ArgumentDef(null, Ast.ArgDirection.IN_REQ, '__kindChannel', Type.Entity('tt:function')),
        new Ast.ArgumentDef(null, Ast.ArgDirection.IN_OPT, '__response', Type.String)
    ];
    for (let argname in sendFrom.schema.out)
        args.push(new Ast.ArgumentDef(null, Ast.ArgDirection.IN_REQ, argname, sendFrom.schema.out[argname]));
    return new Ast.FunctionDef(null, 'action', null, 'send', [], {}, args, {});
}
function makeReceiveSchema(receiveFrom) {
    const args = [
        ...
        new Ast.ArgumentDef(null, Ast.ArgDirection.IN_REQ, '__program_id', Type.Entity('tt:program_id')),
        new Ast.ArgumentDef(null, Ast.ArgDirection.IN_REQ, '__flow', Type.Number),
        new Ast.ArgumentDef(null, Ast.ArgDirection.OUT, '__kindChannel', Type.Entity('tt:function')),
        new Ast.ArgumentDef(null, Ast.ArgDirection.OUT, '__response', Type.String)
    ];
    for (let argname in receiveFrom.schema.out)
        args.push(new Ast.ArgumentDef(null, Ast.ArgDirection.OUT, argname, receiveFrom.schema.out[argname]));

    return new Ast.FunctionDef(null, 'query', null, 'receive', [], { is_list: true, is_monitorable: true}, args, {});
}

function makeDynamicClass(classes, sendSchema, receiveSchema) {
    var classdef = new Ast.ClassDef(null, '__dyn_' + classes.length, ['org.thingpedia.builtin.thingengine.remote'],
        {}, {});
    if (sendSchema)
...
function lowerReturnAction(state, action, lastPrimitive, principal) {
    ...
    let sendInputs = [
        new Ast.InputParam(null, '__principal', getSelf(state.messaging, principal)),
        new Ast.InputParam(null, '__program_id', Ast.Value.Event('program_id')),
        new Ast.InputParam(null, '__flow',  Ast.Value.Number(token)),
        new Ast.InputParam(null, '__kindChannel', Ast.Value.Event('type'))
    ];
    for (let name in lastPrimitive.schema.out)
        sendInputs.push(new Ast.InputParam(null, name, new Ast.Value.VarRef(name)));
    action.invocation.selector = new Ast.Selector.Device(null, localClass.name, null, null);
    ...
    let receiveInputs = [
        new Ast.InputParam(null, '__principal', principal),
        new Ast.InputParam(null, '__program_id', Ast.Value.Event('program_id')),
        new Ast.InputParam(null, '__flow',  Ast.Value.Number(token))
    ];
    let receiveTrigger = new Ast.Stream.Monitor(
        ...
        new Ast.Table.Invocation(
            ...
            new Ast.Invocation(null,
                new Ast.Selector.Device(null, toSendClass.name, null, null),
                'receive', receiveInputs, receiveSchema),
        receiveSchema),
    null, receiveSchema);

    var ourrule = new Ast.Program(null, [toSendClass], [], [
        new Ast.Statement.Rule(null, receiveTrigger, [notifyAction()])
    ], null);
