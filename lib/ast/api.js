...
const PermissionRuleProto = Ast.PermissionRule.prototype;
...
const DatasetProto = Ast.Dataset.prototype;
...
const LibraryProto = Ast.Input.Library.prototype;
...
LibraryProto.iterateSlots = function* iterateSlots() {
    for (let dataset of this.datasets)
        yield* dataset.iterateSlots();
};

Ast.Statement.ClassDef.prototype.iterateSlots = function* iterateSlots() {};

Ast.Statement.Dataset.prototype.iterateSlots = function* iterateSlots() {
    for (let ex of this.examples)
        yield* ex.iterateSlots();
};

Ast.Statement.Rule.prototype.iterateSlots = function* iterateSlots() {
    let [,scope] = yield* this.stream.iterateSlots({});
    for (let action of this.actions)
        yield* action.iterateSlots(scope);
};

Ast.Statement.Command.prototype.iterateSlots = function* iterateSlots() {
    let scope = {};
    if (this.table)
...
Ast.Statement.Declaration.prototype.iterateSlots = function* iterateSlots() {
    yield* this.value.iterateSlots({});
}

Ast.Statement.OnInputChoice.prototype.iterateSlots = function* iterateSlots() {
    let scope = {};
    if (this.table)
        [,scope] = yield* this.table.iterateSlots({});
    for (let action of this.actions)
...
// *** primitive iteration ***
LibraryProto.iteratePrimitives = function* iterativePrimitives(includeVarRef) {
    for (let dataset of this.datasets)
...
ProgramProto.iteratePrimitives = function* iteratePrimitives(includeVarRef) {
    ...
    if (this.oninput)
        yield* this.oninput.iteratePrimitives(includeVarRef);
};

Ast.Statement.ClassDef.prototype.iteratePrimitives = function* () {
};

Ast.Statement.Dataset.prototype.iteratePrimitives = function* iteratePrimitives(includeVarRef) {
    for (let ex of this.examples)
        yield* ex.iteratePrimitives(includeVarRef);
};

Ast.Statement.Declaration.prototype.iteratePrimitives = function* iteratePrimitivesDeclaration(includeVarRef) {
    yield* this.value.iteratePrimitives(includeVarRef);
};

Ast.Statement.Rule.prototype.iteratePrimitives = function* iteratePrimitivesRule(includeVarRef) {
    if (this.isRule)
...
Ast.Statement.Command.prototype.iteratePrimitives = function* iteratePrimitivesCommand(includeVarRef) {
    if (this.table)
...
Ast.Statement.OnInputChoice.prototype.iteratePrimitives = function* iteratePrimitivesOnInputChoice(includeVarRef) {
    if (this.table)
        yield* this.table.iteratePrimitives(includeVarRef);
    for (let action of this.actions)
        yield* action.iteratePrimitives(includeVarRef);
