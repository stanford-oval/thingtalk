...
Ast.BooleanExpression.prototype.iterateSlots2 = function* iterateSlots2(schema, prim, scope) {
    ...
    if (this.isExternal) {
        yield this.selector;
        yield* iterateSlots2InputParams(this, scope);
        yield* this.filter.iterateSlots2(this.schema, prim, makeScope(this));
    } else {
...
Ast.Table.prototype.iterateSlots2 = function* iterateSlots2(scope) {
    if (this.isVarRef) {
        ...
        yield* recursiveYieldArraySlots(new FieldSlot(this, innerScope, Type.Number, this, 'index'));
        ...
        for (let i = 0; i < this.indices.length; i++)
            yield* recursiveYieldArraySlots(new ArrayIndexSlot(prim, innerScope, Type.Number, this.indices, i));
        return [prim, innerScope];
    } else if (this.isSlice) {
        const [prim, innerScope] = yield* this.table.iterateSlots2(scope);
        yield* recursiveYieldArraySlots(new FieldSlot(prim, innerScope, Type.Number, this, 'base'));
        yield* recursiveYieldArraySlots(new FieldSlot(prim, innerScope, Type.Number, this, 'limit'));
        return [prim, innerScope];
    } else if (isUnaryTableToTableOp(this)) {
...
Ast.Stream.prototype.iterateSlots2 = function* iterateSlots2(scope) {
    if (this.isVarRef) {
        ...
        // no primitive here
        yield* recursiveYieldArraySlots(new FieldSlot(null, scope, Type.Date, this, 'base'));
        yield* recursiveYieldArraySlots(new FieldSlot(null, scope, Type.Measure('ms'), this, 'interval'));
        return [null, {}];
    } else if (this.isAtTimer) {
        for (let i = 0; i < this.time.length; i++)
            yield* recursiveYieldArraySlots(new ArrayIndexSlot(null, scope, Type.Time, this.time, i));
        if (this.expiration_date !== null)
            yield* recursiveYieldArraySlots(new FieldSlot(null, scope, Type.Date, this, 'expiration_date'));
        return [null, {}];
    } else if (this.isWindow || this.isTimeSeries) {
        const [prim, innerScope] = this.stream.iterateSlots2(scope);
        yield* recursiveYieldArraySlots(new FieldSlot(prim, innerScope, this.isWindow ? Type.Number : Type.Date, this, 'base'));
        yield* recursiveYieldArraySlots(new FieldSlot(prim, innerScope, this.isWindow ? Type.Number : Type.Measure('ms'), this, 'delta'));
        return [prim, innerScope];
    } else if (this.isHistory || this.isSequence) {
        const [prim, innerScope] = this.table.iterateSlots2(scope);
        yield* recursiveYieldArraySlots(new FieldSlot(prim, innerScope, this.isSequence ? Type.Number : Type.Date, this, 'base'));
        yield* recursiveYieldArraySlots(new FieldSlot(prim, innerScope, this.isSequence ? Type.Number : Type.Measure('ms'), this, 'delta'));
        return [prim, innerScope];
    } else if (this.isFilter || this.isEdgeFilter) {
