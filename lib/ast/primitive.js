...
import { Invocation, Selector, BooleanExpression } from './expression';
...
export class Table extends Node {
    ...
    *iterateSlots2(scope) {
        if (this.isVarRef) {
            ...
            let [prim, innerScope] = yield* this.table.iterateSlots2(scope);
            yield* recursiveYieldArraySlots(new FieldSlot(prim, innerScope, this.type, this, 'compute', 'expression'));
            return [prim, innerScope];
        } else if (isUnaryTableToTableOp(this)) {
...
class NotifyAction extends Action {
     ...
     * @param {Ast.ExpressionSignature|null} schema - type signature of this action
     */
    constructor(location, name, schema) {
 ...
 * @abstract
 */
export class PermissionFunction extends Node {}
