...
function* recursiveYieldArraySlots(slot) {
    ...
    if (value.isArray) {
        ...
        assert(overload && overload.length === value.operands.length+1);
        for (let i = 0; i < value.operands.length; i++)
            yield* recursiveYieldArraySlots(new ComputationOperandSlot(slot.primitive, slot.scope, overload[i], value.op, value.operands, slot, i));
    }
}
