...
// in the future, org.thingpedia.database should have 3 different server option: local/almond-cloud/self-hosted
// local: hosted on user's device (where user's Almond is running)
// almond-cloud: hosted by us in a centralized database
// self-hosted: hosted by the developers on their own server
...
class FunctionDef extends ExpressionSignature {
    constructor(functionType, name, args, is_list, is_monitorable, metadata, annotations, parent = null) {
        let is_db = parent ? parent.imports.some((mixin) => mixin.module === 'org.thingpedia.database') : false;
        let db_server = is_db ? parent.name : null;
