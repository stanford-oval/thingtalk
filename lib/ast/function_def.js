...
function legacyAnnotationToValue(value) {
    ...
    if (typeof value === 'string')
        ...
        v = value.map((elem) => legacyAnnotationToValue(elem));
    return v;
}
...
class ExpressionSignature {
    constructor(functionType, args, is_list, is_monitorable, require_filter, default_projection) {
    ...
    _cloneInternal(args) {
        return new ExpressionSignature(this._functionType, args,
            this.is_list, this.is_monitorable, this.require_filter, this.default_projection);
    }
...
class FunctionDef extends ExpressionSignature {
    constructor(functionType, name, args, is_list, is_monitorable, metadata, annotations, parent = null) {
        ...
        if ('require_filter' in annotations) {
            assert(annotations.require_filter.isBoolean);
            require_filter = annotations.require_filter.value;
        } else {
            require_filter = false;
        }
        if ('default_projection' in annotations) {
            assert(annotations.default_projection.isArray);
