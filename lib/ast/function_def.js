...
class FunctionDef extends ExpressionSignature {
    ...
    _checkConflictArguments(functions) {
        const args = {};
        for (let f of functions) {
            for (let a of f.args) {
                ...
                const err_msg = `Parameter ${a} is defined multiple times in ${functions.map((f) => f.name).join(', ')}`;
                if (args[a].canonical !== f.getArgCanonical(a))
                    throw new TypeError(`${err_msg} with different canonical forms`);
                if (args[a].prettyprint() !== f._argmap[a].prettyprint())
                    throw new TypeError(`${err_msg} with different types or annotations`);
            }
        }
        for (let a of this.args) {
            if (a in args)
                throw new TypeError(`Parameter ${a} has been defined in ${functions.map((f) => f.name).join(', ')}`);
        }
    }

    extends(functions) {
        let metadata = {}, annotations = {}, args = [];

        // all the annotations from these functions will be dropped
        Object.assign(metadata, this._metadata);
        Object.assign(annotations, this._annotations);

        this._checkConflictArguments(functions);
        args = args.concat(this.args.map((a) => this._argmap[a]));
        functions.forEach((f) => {
            f.args.forEach((a) => {
                if (!this.args.includes(a))
                    args.push(f._argmap[a]);
            });
        });
