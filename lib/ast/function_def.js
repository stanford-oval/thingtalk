...
// is_db is used to indicate if it is a database function call, and db_server indicates where the database is hosted
// TODO: currently each device is considered to have a different server location
// in the future, org.thingpedia.database should have 3 different server option: local/almond-cloud/self-hosted(url)
// local: hosted on user's device (where user's Almond is running)
// almond-cloud: hosted by almond cloud
// self-hosted: hosted by the developers on their own server, in which case db_server will be the endpoint of the server
// all devices that use local(almond-cloud) should be considered using the same server
// db_server of a join with different server location will be marked as 'federated'
class ExpressionSignature {
    constructor(functionType, args, is_list, is_monitorable, is_db = false, db_server = null) {
    ...
    isFederated() {
        return this.db_server === 'federated';
    }
...
class FunctionDef extends ExpressionSignature {
    constructor(functionType, name, args, is_list, is_monitorable, metadata, annotations, parent = null) {
        let is_db = parent ? parent.isdb : false;
        let db_server = is_db ? parent.db_server : null;
        super(functionType, args, is_list, is_monitorable, is_db, db_server);
