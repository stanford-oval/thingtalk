...
class DialogueHistoryItem extends AstNode {
    constructor(location, stmt, results, confirm) {
        ...
        assert(results === null || confirm === true);
    ...
    _getFunctions() {
        ...
        const visitor = new class extends NodeVisitor {
            visitInvocation(invocation) {
                ...
                functions.add(invocation.selector.kind, invocation.channel);
            }
        };
    ...
    clone() {
        return new DialogueHistoryItem(this.location, this.stmt.clone(), this.results ? this.results.map((r) => r.clone()) : null, this.confirm);
    }
