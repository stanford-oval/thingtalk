...
class Declaration extends Statement {
    constructor(name, type, args, value) {
        super();

        assert(typeof name === 'string');
        this.name = name;

        assert(['stream', 'query', 'action'].indexOf(type) >= 0);
        this.type = type;

        assert(typeof args === 'object');
        this.args = args;

        assert(value instanceof Stream || value instanceof Table || value instanceof Action);
        this.value = value;
    }
...
class OnInput extends Statement {
    constructor(choices) {
        super();
    ...
    clone() {
        return new OnInput(this.choices.map((c) => c.clone()));
    }
}
OnInput.prototype.isOnInput = true;
Statement.OnInput = OnInput;
...
class Program extends Input {
    constructor(classes, declarations, rules, principal = null, oninput = null) {
        super();
        ...
        assert(oninput === null || oninput instanceof Statement.OnInput);
    ...
    clone() {
        return new Program(
            ...
            this.oninput !== null ? this.oninput.clone() : null);
    }
}
