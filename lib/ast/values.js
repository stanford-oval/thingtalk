...
Value.prototype.visit = async function visit(visitor) {
    // TODO more finegrained visit functions
    await visitor.enter(this);
    if (await visitor.visitValue(this)) {
        if (this.isArray || this.isCompoundMeasure) {
            for (let v of this.value)
                await v.visit(visitor);
        } else if (this.isObject) {
            for (let key in this.value)
                await this.value[key].visit(visitor);
        }
    }
...
Value.prototype.toJS = function toJS() {
    ...
    if (v.isVarRef || v.isEvent || v.isContextRef)
