...
class DatePiece {
    constructor(year, month, day, time) {
        assert((year !== null && year >= 0) || month > 0 || day > 0);
...
class WeekDayDate {
    ...
    equals(other) {
        return (other instanceof WeekDayDate && this.weekday === other.weekday && this.time === other.time);
    }
}
WeekDayDate.prototype.isWeekDayDate = true;
module.exports.WeekDayDate = WeekDayDate;
...
class RecurrentTimeSpecification extends Value {
    constructor() {
        super(null);
    }

    toString() {
        return `RecurrentTimeSpec()`;
    }

    clone() {
        return new RecurrentTimeSpecification();
    }

    equals(other) {
        return other.isRecurrentTimeSpecification;
    }
...
RecurrentTimeSpecification.prototype.isRecurrentTimeSpecification = true;
Value.ReccurentTimeSpecification = RecurrentTimeSpecification;
