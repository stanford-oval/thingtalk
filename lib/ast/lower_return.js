...
import { Stream, Table, Action } from './primitive';
import { ClassDef } from './class_def';
import { Program, Statement } from './program';
...
function makeDynamicClass(classes, sendSchema, receiveSchema) {
    let classdef = new ClassDef(null, '__dyn_' + classes.length, ['org.thingpedia.builtin.thingengine.remote'],
        {}, {});
    if (sendSchema)
...
function lowerReturnAction(state, action, lastPrimitive, principal) {
    ...
    let ourrule = new Program(null, [toSendClass], [], [
...
export default function lowerReturn(program, messaging) {
    let ourrules = [];
