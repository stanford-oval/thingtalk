...
class ClassDef extends Statement {
    ...
    async visit(visitor) {
        ...
        if (await visitor.visitClassDef(this)) {
            for (let import_ of this.imports)
                await import_.visit(visitor);
            for (let macro in this.macros)
                await this.macros[macro].visit(visitor);
            for (let query in this.queries)
                await this.queries[query].visit(visitor);
            for (let action in this.actions)
                await this.actions[action].visit(visitor);
        }
