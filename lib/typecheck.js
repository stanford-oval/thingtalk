...
function resolveListExpression(ast, schema, scope, schemas, classes, useMeta) {
    ...
    const args = [
        new Ast.ArgumentDef('out', 'value', paramType.elem, {}, {})
    ];
    const localschema = new Ast.ExpressionSignature('query', [], args, true, true);
    typeCheckFilter(ast.filter, localschema, new Scope(), schemas, classes, useMeta);
...
function resolveScalarExpression(ast, schema, scope, schemas, classes, useMeta) {
    ...
    if (ast.isAggregation) {
        ...
        const paramType = resolveListExpression(ast.list, schema, scope, schemas, classes, useMeta);
...
function typeCheckFilter(ast, schema, scope, schemas, classes, useMeta) {
    ...
    return (async function recursiveHelper(ast) {
        ...
        if (ast.isCompute) {
            let type_lhs = resolveScalarExpression(ast.lhs, schema, scope, schemas, classes, useMeta);
...
function resolveFilter(filter, schema) {
    ...
    schema.no_filter = (function recursiveHelper(ast) {
        ...
        if (ast.isAtom) {
            return !!schema.getArgument(ast.name).unique;
        } else {
            assert(ast.isExternal);
            return false;
        }
