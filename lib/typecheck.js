...
const ALLOWED_PRINCIPAL_TYPES = new Set([
    'tt:contact', 'tt:contact_name', 'tt:username', 'tt:contact_group', 'tt:contact_group_name'
]);
...
function typeCheckFilter(ast, schema, scope, schemas, classes, useMeta) {
    ...
    return (function recursiveHelper(ast) {
        ...
        if (ast.isAtom) {
            let name = ast.name;
            let type_lhs = schema.inReq[name] || schema.inOpt[name] || schema.out[name] || scope.get(name);
            if (!type_lhs)
