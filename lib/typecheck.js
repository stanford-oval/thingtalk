...
function pushInReq(schema, scope) {
    if (scope.hasInReq()) {
        for (let name in scope._inReq) {
            addInput(schema, name, scope._inReq[scope], true);
        }
        scope.clearInReq();
    }
}
...
function typeCheckTable(ast, schemas, scope, classes, useMeta = false) {
    ...
    if (ast.isJoin) {
        ...
        return Promise.resolve()
            ...
            .then(() => {
                scope_in_join.$has_event = true;
                return typeCheckTable(ast.rhs, schemas, scope_in_join, classes, useMeta)
            }).then(() => {
...
function typeCheckStream(ast, schemas, scope, classes, useMeta = false) {
    ...
    if (ast.isJoin) {
        ...
        return Promise.resolve()
            ...
            .then(() => {
                scope_in_join.$has_event = true;
                return typeCheckTable(ast.table, schemas, scope_in_join, classes, useMeta)
            }).then(() => {
