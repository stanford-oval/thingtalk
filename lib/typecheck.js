...
async function resolveScalarExpression(ast, schema, scope, schemas, classes, useMeta) {
    ...
    if (ast.isFilter)
        return resolveListExpression(ast.list, schema, scope, schemas, classes, useMeta);

    if (ast.isVarRef)
        return typeCheckMacro(ast, schema, scope, schemas, classes, useMeta);
...
function typeEqual(t1, t2) {
    ...
    if (t1.isCompound && t2.isCompound) {
        ...
        for (let f in t1.fields) {
            if (!(f in t2.fields))
                return false;
            if (!t1.fields[f].type.equals(t2.fields[f].type))
                return false;
        }
        return true;
    } else {
