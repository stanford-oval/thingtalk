...
function findPrincipal(prim) {
    ...
    if (!Type.isAssignable(principal.value.getType(), Type.Entity('tt:contact'), {}, true))
        throw new TypeError(`Invalid type for parameter 'principal' (got ${table.value.getType()}, expected Entity(tt:contact))`);
    return String(principal.value.toJS());
}
...
function ensureSchema(schemas, classes, prim, primType, useMeta) {
    ...
    if (prim.selector.isBuiltin && ['new_record', 'get_record'].indexOf(prim.channel) > -1) {
        ...
        return Utils.getMemorySchema(schemas, prim.channel, table, principal, useMeta).then((schema) => {
            if (!schema)
...
function genAutoTableName(rule) {
    ...
    function addInvocation(inv) {
        ...
        assert(fname);
        buf += '+' + fname + ':' + inv.out_params.map((op) => op.name + ':' + op.value).join(',');
    }
    if (rule.trigger)
