...
function typeCheckTable(ast, schemas, scope, classes, useMeta = false) {
    ...
    if (ast.isVarRef) {
        log('isVarRef');
        if (ast.name in scope && scope[ast.name].type === 'table') {
            ast.schema = scope[ast.name].value.schema;
            Object.assign(scope, ast.schema.out);
        }
        return ensureSchema(schemas, classes, ast, 'query', useMeta).then(() => {
            Object.assign(scope, ast.schema.out);
...
function typeCheckStream(ast, schemas, scope, classes, useMeta = false) {
    ...
    if (ast.isVarRef) {
        if (ast.name in scope && scope[ast.name].type === 'stream') {
            ast.schema = scope[ast.name].value.schema;
            Object.assign(scope, ast.schema.out);
        }
        return ensureSchema(schemas, classes, ast, 'query', useMeta).then(() => {
    ...
    if (ast.isJoin) {
        log('isJoin');
        return Promise.all([
            typeCheckStream(ast.stream, schemas, scope, classes, useMeta),
            typeCheckTable(ast.table, schemas, scope, classes, useMeta)
        ]).then(() => {
