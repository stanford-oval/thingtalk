...
async function typeCheckValue(value, scope, schemas, classes, useMeta) {
    ...
    if (value.isArray) {
        ...
        let elem = await typeCheckValue(value.value[0], scope, schemas, classes, useMeta);

        for (let v of value.value) {
            let vtype = await typeCheckValue(v, scope, schemas, classes, useMeta);

            // merge enum types if necessary
            if (vtype.isEnum && vtype.entries[vtype.entries.length-1] === '*' &&
                elem.isEnum && elem.entries[elem.entries.length-1] === '*') {
                for (let entry of vtype.entries) {
                    if (elem.entries.includes(entry))
                        continue;
                    elem.entries.splice(elem.entries.length-1, 0, entry);
                }
            } else if (!Type.isAssignable(vtype, elem, typeScope)) {
...
function resolveOverload(overloads, operator, argTypes, allowCast) {
    for (let overload of overloads.types) {
        ...
        if (overload[overload.length-1].isMeasure && typeScope['_unit'])
            return [overload, new Type.Measure(typeScope['_unit'])];
...
function resolveProjection(args, schema, scope) {
    ...
    const result = clone.filterArguments((a) => a.is_input || args.has(a.name));
...
async function typeCheckClass(klass, schemas, classes, useMeta, isLibrary) {
    ...
    for (let [, query] of Object.entries(klass.queries))
        await typeCheckFunctionDef('query', query, schemas, useMeta);
     for (let [, action] of Object.entries(klass.actions))
...
async function typeCheckFunctionDef(type, func, schemas, useMeta) {
    for (let argname of func.args) {
        ...
        for (let name in arg.impl_annotations) {
        ...
        for (let name of ['default', 'min_number', 'max_number']) {
            const value = arg.impl_annotations[name];
            if (!value)
                continue;
