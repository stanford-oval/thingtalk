...
class Scope {
    ...
    hasTable(name) {
        return (name in this._globalScope && this._globalScope[name].type === 'table');
    }

    hasStream(name) {
        return (name in this._globalScope && this._globalScope[name].type === 'stream');
    }

    getSchema(name) {
        if (this.hasTable(name) || this.hasStream(name))
            return this._globalScope[name].value.schema;
    }
    ...
    addGlobal(name, ast) {
        if (this.hasTable(name) || this.hasStream(name))
            throw new TypeError('Conflict on using ' + name + ' for ' + ast.type);
        this._globalScope[name] = ast;
    }
    ...
    assign(name_type_pairs) {
        for (let name in name_type_pairs)
            this.add(name, name_type_pairs[name]);
    }
...
function typeCheckStream(ast, schemas, scope, classes, useMeta = false) {
    ...
    if (ast.isEdgeFilter) {
        throw new Error('Not implemented yet: typeCheckStream - edgeFilter');
    }
    if (ast.isFilter) {
        log('isFilter');
