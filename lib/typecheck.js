...
function typeCheckMemoryInputArgs(ast, scope) {
...
function typeCheckMemoryFilter(ast, schema, scope) {
...
function ensureSchema(schemas, classes, prim, primType, useMeta) {
    ...
    if (prim.selector.isBuiltin && ['log.new', 'log.get', 'low.save'].indexOf(prim.channel) > -1) {
        let table = prim.in_params.find((e) => {
            return e.name === 'table'
        }).value;
        return Utils.getMemorySchema(schemas, table).then((schema) => {
            prim.schema = schema;
        });
    }
