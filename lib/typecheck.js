...
function ensureSchema(schemas, classes, prim, primType, useMeta) {
    ...
    if (prim.selector.isBuiltin && primType === 'query') {
        if (prim.channel === 'log.get')
            prim.schema = Builtin.Queries['log.get'];
        else
            throw new TypeError('Invalid builtin query ' + prim.channel);
        return Q();
    }
    if (prim.selector.principal !== null) {
        if (!ALLOWED_PRINCIPAL_TYPES.has(prim.selector.principal.type))
...
function typeCheckProgram(ast, schemas, useMeta = false) {
    ...
    if (ast.principal !== null) {
        if (!ALLOWED_PRINCIPAL_TYPES.has(ast.principal.type))
            throw new TypeError('Invalid principal, must be a contact or a group');
    }
