...
async function typeCheckClass(klass, schemas, isMeta = false) {
    if (isMeta) {
        ...
        if (!('name' in klass.metadata))
            throw new TypeError('Missing name for the class');
        if (!('description' in klass.metadata))
            throw new TypeError('Missing description for the class');
    }
        ...
        if (import_stmt.isMixin) {
            for (let facet of import_stmt.facets) {
                if (imported.has(facet))
                    throw new TypeError(`Multiple ${facet} are imported`);
                imported.add(facet);
            }
