...
function resolveProjection(args, schema, scope) {
    args.forEach((arg) => {
        ...
        scope[arg] = schema.types[schema.index[arg]];
        //TODO: better maintenance for scope, delete ones not projected
    });
    ...
    Object.keys(schema.out).forEach((arg) => {
        if (schema.args.indexOf(arg) === -1) delete schema.out[arg];
    });
}

function resolveJoin(ast) {
    ast.schema = Object.assign({}, ast.lhs.schema);
    ast.schema.args = ast.schema.args.concat(ast.rhs.schema.args.slice(2));
    ast.schema.types = ast.schema.types.concat(ast.rhs.schema.types.slice(2));
    ast.schema.index = ast.rhs.schema.args.slice(2).reduce((res, arg) => {
        res[arg] = Object.keys(res).length;
        return res;
    }, ast.lhs.schema.index);
    ast.schema.out = Object.assign(ast.schema.out, ast.rhs.schema.out);
}
