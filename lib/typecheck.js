...
function resolveProjection(args, schema, scope) {
    ...
    if (clone.annotations)
        clone.annotations.default_projection = Ast.Value.Array([]);
...
async function typeCheckDialogue(ast, schemas, useMeta = false) {
    for (let item of ast.history) {
        ...
        if (item.results !== null) {
            ...
            if (item.results.error && (!item.results.error.isConstant() ||
                !Type.isAssignable(await typeCheckValue(item.results.error, new Scope, schemas, {}, useMeta), Type.String, {})))
                throw new TypeError(`History annotation #[error] must be a constant of String type`);
