...
function addOutput(schema, name, type, scope) {
    scope.add(name, type);
    return schema.addArguments([new Ast.ArgumentDef(Ast.ArgDirection.OUT, name, type)]);
}
...
function typeCheckAggregation(ast, scope) {
    ...
    ast.schema = addOutput(cleanOutput(ast.table.schema, scope), name, type, scope);
}
    ...
    return Promise.all(Array.from(ast.iteratePrimitives(true)).map(async ([primType, prim]) => {
        ...
        if (prim.isVarRef) {
            if (scope.has(prim.name))
                ...
                schema = await Utils.getMemorySchema(schemas, prim.name, useMeta);
            if (schema === null)
...
function typeCheckFunctionDef(type, func) {
    ...
    func.args.forEach((name) => {
        if (!(func.isArgInput(name)) && 'prompt' in func.getArgMetadata(name))
            throw new TypeError(`Invalid annotation prompt for output argument ${name}.`);
    });

    if (type === 'query' && func.is_monitorable) {
        if (!('poll_interval' in func.annotations))
            throw new TypeError(`Missing poll_interval annotation for monitorable query ${func.name}.`);
...
async function typeCheckDeclaration(ast, schemas, scope, classes, useMeta) {
    if (ast.type === 'program') {
        ...
        await typeCheckProgram(ast.value, schemas, scope, classes, useMeta);
    } else {
        ...
        switch (ast.type) {
            case 'stream':
                addRequiredInputParamsStream(ast.value, null);
            ...
            case 'query':
                ...
                break;
            case 'action':
                addRequiredInputParamsAction(ast.value, null);
...
async function typeCheckRule(ast, schemas, scope, classes, useMeta = false) {
    ...
    if (ast.table !== undefined && ast.table !== null) {
        ...
        addRequiredInputParamsStream(ast.stream, null);
        await typeCheckStream(ast.stream, schemas, scope, classes, useMeta);
    }
    ...
    for (let prim of ast.actions)
        addRequiredInputParamsAction(prim, null);
    await Promise.all(
        ast.actions.map((action) => typeCheckAction(action, schemas, scope, classes, useMeta)));
}

async function typeCheckProgram(ast, schemas, useMeta = false) {
    const classes = {};
...
async function typeCheckDataset(dataset, schemas, getMeta = false) {
    for (let ex of dataset.examples) {
        await typeCheckDeclaration(ex, schemas, new Scope(), schemas, {}, getMeta);
...
async function typeCheckMeta(meta, schemas, getMeta = false) {
    ...
    for (let dataset of meta.datasets)
        await typeCheckDataset(dataset, schemas, getMeta);
}
