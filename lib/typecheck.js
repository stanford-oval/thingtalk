...
function resolveProjection(args, schema) {
    ...
    ['inReq', 'inOpt', 'out'].forEach((meta) => {
        Object.keys(schema[meta]).forEach((arg) => {
            if (schema.args.indexOf(arg) === -1) delete schema[meta][arg];
        });
    });
}
...
function typeCheckTable(ast, schemas, scope, classes, useMeta = false) {
    ...
    if (ast.isProjection) {
        ...
        return typeCheckTable(ast.table, schemas, scope, classes, useMeta).then(() => {
            ast.schema = ast.table.schema;
            resolveProjection(ast.args, ast.schema);
            return Q();
        });
    }
    if (ast.isAlias) {
