...
function optimizeRule(rule) {
    ...
    // projection is only allowed when the actions include notify/return
...
function postProcessProgram(program) {
    ...
    program.rules.forEach((rule) => {
        if (rule.isAssignment) {
            ...
            let newrule = postProcessRule(rule);
            rules.push(newrule);
        }
    });
    program.rules = rules;
...
function postProcessRule(rule) {
    let ast = rule.stream ? rule.stream : rule.table;
    // only apply when there is default projection in stream or table
    // which means returning all output parameters is inefficient for some of the functions
    if (ast.schema && ast.schema.default_projection.length > 0) {
        let projection = [];
        rule.actions.forEach((action) => {
            if (action.isInvocation && action.invocation.selector.isBuiltin) {
                projection = projection.concat(ast.schema.default_projection);
            } else {
                action.invocation.in_params.forEach((p) => {
                    if (p.value.isVarRef)
                        projection.push(p.value.name);
                });
            }
        });
        rule.stream ?
            rule.stream = postProcessStream(ast, projection) :
            rule.table = postProcessTable(ast, projection);
    }

    return rule;
}

function postProcessStream(stream, projection) {
    if (stream.isProjection) {
        projection = projection.concat(stream.args);
        stream = stream.stream;
    }

    projection = [...new Set(projection)];
    return new Ast.Stream.Projection(stream, projection, stream.schema);

}

function postProcessTable(table, projection) {
    if (table.isProjection) {
        projection = projection.concat(table.args);
        table = table.stream;
    }

    projection = [...new Set(projection)];
    return new Ast.Table.Projection(table, projection, table.schema);
}
