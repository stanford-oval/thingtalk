...
function optimizeStream(stream, allow_projection=true) {
    ...
    if (stream.isProjection) {
        if (!allow_projection)
            ...
            return new Ast.Stream.Projection(optimizeStream(stream.stream), stream.args, stream.schema);
    }

    if (stream.isMonitor) {
        let table = optimizeTable(stream.table, allow_projection);
        if (!table)
            return null;
        stream.table = table;
        return stream;
...
function optimizeTable(table, allow_projection=true) {
    ...
    if (table.isProjection) {
        ...
        if (table.table.isAggregation)
            return optimizeTable(table.table);
        return new Ast.Table.Projection(optimizeTable(table.table, allow_projection),
            table.args,
            table.schema);
    }
