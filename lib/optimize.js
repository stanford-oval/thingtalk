...
function optimizeFilter(expr) {
    ...
    if (expr.isOr) {
        ...
        if (operands.length === 1)
            return operands[0];
        return new Ast.BooleanExpression.Or(operands);
    }
    if (expr.isNot) {
        ...
        if (subexpr.isFalse)
            return Ast.BooleanExpression.True;
        return new Ast.BooleanExpression.Not(subexpr);
    }
    if (expr.isExternal) {
...
function optimizeTable(table, allow_projection=true) {
    ...
    if (table.isFilter) {
        ...
        if (table.table.isFilter) {
            table.filter = optimizeFilter(new Ast.BooleanExpression.And([table.filter, table.table.filter]));
