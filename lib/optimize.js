...
function optimizeStream(stream, allow_projection=true) {
    ...
    if (stream.isFilter) {
        ...
        if (stream.stream.isCompute || stream.stream.isJoin) {
            stream.stream = optimizeStream(stream.stream, allow_projection);
            return stream;
        }

        // for everything else that has a stream field push down filters to the stream
        else if (stream.stream.stream) {
            stream.stream.stream = optimizeStream(new Ast.Stream.Filter(
                stream.stream.stream,
                stream.filter,
                null
            ), allow_projection);
        ...
        // do nothing for the rest (invocation, varref, timer, etc)
        return stream;
