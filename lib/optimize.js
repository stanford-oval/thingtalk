...
function optimizeTable(table) {
    ...
    if (table.isSlice && table.limit.isNumber && table.limit.value === 1)
        return optimizeTable(new Ast.Table.Index(table.table, [table.base], table.table.schema));
...
function optimizeProgram(program) {
    ...
    program.rules = rules;
    if (program.rules.length === 0 && program.declarations.length === 0 && Object.keys(program.classes).length === 0)
        return null;
    else
        return program;
}
