    ...
    next() {
        ...
        if (/^[A-Z]/.test(next)) {
            ...
            if (!(next in this._entities))
                throw new SyntaxError('Invalid entity ' + next + ', have ' + Object.keys(this._entities));
            ...
            if (entityType.startsWith('GENERIC_ENTITY_')) {
                next = new TokenWrapper('GENERIC_ENTITY', {
                    value: this._entities[next].value,
                    display: this._entities[next].display,
...
function constantToNN(constant) {
    let measure = /__const_(NUMBER_[0-9]+)_([a-z0-9A-Z]+)/.exec(constant);
    if (measure !== null)
        return List.concat(measure[1], measure[2]);
    return List.singleton(constant.substring('__const_'.length));
}
...
function findEntity(entityType, value, entities, { ignoreNotFound = false, ignoreMultiple = true } = {}) {
    ...
    for (let what in entities) {
        ...
        case 'DATE':
            ...
            break;
        case 'LOCATION':
            if (entities[what].latitude === value.latitude &&
                entities[what].longitude === value.longitude)
                candidates.push(what);
            break;
        }
    }
    ...
    if (candidates.length === 0) {
        ...
        if (reuse !== null)
            ...
            throw new Error('Cannot find entity ' + value + ' of type ' + entityType);
    } else {
        if (!entities.$used)
...
function valueToNN(value, entities) {
    if (value.isArray) {
        return List.concat('[', ...value.value.map((v) => valueToNN(v, entities)), ']');
        ...
        if (value.offset === null)
            ...
            offset = List.Cons('+', valueToNN(value.offset));
...
function cnfFilterToNN(filter, entities) {
    let result = List.Nil;
    for (let and of filter.operands) {
        ...
        for (let or of and.operands) {
            ...
            let orclause;
            if (or.isAtom)
                orclause = List.concat('param:' + or.name, or.operator, valueToNN(or.value, entities));
            else
...
function streamToNN(stream, entities) {
    if (stream.isVarRef) {
        ...
        return List.concat('monitor', '(', tableToNN(stream.table, entities), ')');
    } else if (stream.isEdgeNew) {
        return List.concat('edge', '(', streamToNN(stream.stream), ')', 'on', 'new');
        ...
        if (optimized.isTrue)
            ...
            return List.concat('edge', '(', streamToNN(stream.stream, entities), ')', 'on', cnfFilterToNN(optimized, entities));
        ...
        if (stream.args.length > 1) {
            let list = 'param:' + stream.args[0];
            for (let i = 1; i < stream.args.length; i++)
                list = List.concat(list, ',', 'param:' + stream.args[i]);
            return List.concat('[', list, ']',
                'of', '(', streamToNN(stream.stream, 1), ')');
        } else {
            return List.concat('param:' + stream.args[0], 'of',
                '(', streamToNN(stream.stream, 1), ')');
        }
        ...
        for (let inParam of stream.in_params)
            param_passing = List.concat(param_passing, 'on', 'param:' + inParam.name, '=', 'param:' + inParam.value.name);
        return List.concat('(', streamToNN(stream.stream, entities), ')',
...
function tableToNN(table, entities) {
    if (table.isVarRef) {
        ...
        for (let inParam of table.invocation.in_params)
            params = List.concat(params, 'param:' + inParam.name, '=', valueToNN(inParam.value, entities));
        ...
        if (table.args.length > 1) {
            let list = 'param:' + table.args[0];
            for (let i = 1; i < table.args.length; i++)
                list = List.concat(list, ',', 'param:' + table.args[i]);
            return List.concat('[', list, ']',
                'of', '(', tableToNN(table.table, 1), ')');
        } else {
            return List.concat('param:' + table.args[0], 'of',
                '(', tableToNN(table.table, 1), ')');
        }
...
function actionToNN(action, entities) {
    ...
    for (let inParam of action.in_params) {
        let value = valueToNN(inParam.value, entities);
        if (typeof value === 'string' &&
            (value.startsWith('param:') || value === 'event'))
            param_passing = List.concat(param_passing, 'on', 'param:' + inParam.name, '=', value);
        else
            const_param = List.concat(const_param, 'param:' + inParam.name, '=', value);
    }
