...
class SequenceLexer {
    ...
    next() {
        ...
        if (/^[A-Z]/.test(next)) {
            ...
            let unit = null;
            if (this._i < this._sequence.length-1 && this._sequence[this._i+1].startsWith('unit:'))
                unit = this._sequence[this._i+1].substring('unit:'.length);
            ...
            this._lastparam = next.substring('param:'.length);
            next = new TokenWrapper('PARAM_NAME', this._lastparam);
