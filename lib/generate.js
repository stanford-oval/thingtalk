...
function* iterateSlotsInputParams(invocation) {
    for (let in_param of invocation.in_params)
        yield [invocation.schema, in_param];
}

function* iterateSlotsFilter(schema, expr) {
    if (expr.isTrue || expr.isFalse)
        return;
    if (expr.isAnd || expr.isOr) {
        for (let op of expr.operands)
            yield* iterateSlotsFilter(schema, op);
        return;
    }
    if (expr.isNot) {
        yield* iterateSlotsFilter(schema, expr.expr);
        return;
    }
    if (expr.isExternal) {
        yield* expr.in_params;
        yield* iterateSlotsFilter(expr.schema, expr.filter);
    } else {
        yield [schema, expr];
    }
}

function* iterateSlotsTable(table) {
    if (table.isVarRef) {
        // this will be handled when we visit the declaration
    } else if (table.isInvocation) {
        yield* iterateSlotsInputParams(table.invocation);
    } else if (table.isFilter) {
        yield* iterateSlotsTable(table.table);
        yield* iterateSlotsFilter(table.table.schema, table.filter);
    } else if (isUnaryTableToTableOp(table)) {
        yield* iterateSlotsTable(table.table);
    } else if (isUnaryStreamToTableOp(table)) {
        yield* iterateSlotsStream(table.stream);
    } else if (table.isJoin) {
        yield* iterateSlotsTable(table.lhs);
        yield* iterateSlotsTable(table.rhs);
    } else {
        throw new TypeError("Can't handle " + table);
    }
}

function* iterateSlotsStream(stream) {
    if (stream.isVarRef) {
        // this will be handled when we visit the declaration
    } else if (stream.isTimer || stream.isAtTimer) {
        // no primitive here
    } else if (stream.isFilter) {
        yield* iterateSlotsStream(stream.stream);
        yield* iterateSlotsFilter(stream.stream.schema, stream.filter);
        ...
        yield* iterateSlotsTable(stream.table);
    } else if (stream.isJoin) {
        yield* iterateSlotsStream(stream.stream);
        yield* iterateSlotsTable(stream.table);
    } else {
...
function* iterateSlots(program) {
    for (let decl in program.declarations) {
        if (decl.type === 'table')
            yield* iterateSlotsTable(decl.value);
        else if (decl.type === 'stream')
            yield* iterateSlotsStream(decl.value);
    }
    for (let rule of program.rules) {
        if (rule.isRule)
            yield* iterateSlotsStream(rule.stream);
        else if (rule.isCommand && rule.table)
            yield* iterateSlotsTable(rule.table);
        for (let action of rule.actions)
            yield* iterateSlotsInputParams(action);
    }
}
