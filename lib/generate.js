...
function factorRule(messaging, rule) {
    ...
    if (isRemote(trigger)) {
        ...
        trigger.resolved_args = [Ast.Value.Entity(trigger.principal, 'tt:contact', null),
            Ast.Value.Entity(token, 'tt:flow_token', null),
            Ast.Value.Entity(oldKindChannel, 'tt:function', null)]
    ...
    if (isRemote(query)) {
        ...
        var queryPrincipal = Ast.Value.Entity(query.principal, 'tt:contact', query.owner.display);
        ...
        newrules.push({
            trigger: {
                ...
                resolved_args: [Ast.Value.Entity(query.principal, 'tt:contact', null),
                    Ast.Value.Entity(token2, 'tt:flow_token', null),
                    Ast.Value.Entity('query:' + query.kind + ':' + query.channel, 'tt:function', null)]
    ...
    if (isRemote(action)) {
        ...
        var actionPrincipal = Ast.Value.Entity(action.principal, 'tt:contact', action.owner.display);
        ...
        if (query) {
            ...
            action.resolved_args = [Ast.Value.Entity(action.principal, 'tt:contact', null),
                Ast.Value.Entity(token, 'tt:flow_token', null)]
