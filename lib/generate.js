...
function codegenTrigger(trigger, uses) {
    var triggerParams = [];
    ...
    var triggerParams = trigger.resolved_args.map(function(arg, i) {
        if (arg === undefined) {
            if (uses.has(trigger.schema.args[i])) {
...
function codegenRule(trigger, query, action, once) {
    ...
    var triggerAst = null;
    if (trigger !== null)
        triggerAst = codegenTrigger(trigger, triggerUses);
    var queryAst = null;
    if (query !== null)
        queryAst = codegenTrigger(query, queryUses);
...
function assignSlots(slots, prefilled, values, comparisons, fillAll, mustFill, scope, toFill, remoteSlots) {
    ...
    slots.forEach((slot, i) => {
        if (remoteSlots && remoteSlots.has(slot.name))
            return;
...
function factorRule(messaging, rule) {
    ...
    if (trigger) trigger.remoteSlots = new Set();
    if (query) query.remoteSlots = new Set();
    if (action) action.remoteSlots = new Set();
    ...
    if (!trigger && !query && isRemote(action)) {
        ...
        action.schema.schema.forEach((type, i) => {
            if (action.resolved_args[i] !== undefined) {
                ...
                // TODO: check if arg is in remote slots, still throw error otherwise
