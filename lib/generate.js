...
function codegenInvocation(invocation, params) {
    var sel, part;
    if (invocation.kind_type === 'app' ||
        (invocation.kind === 'builtin' && !invocation.principal)) {
...
function codegenRule(trigger, query, action) {
    var triggerAst = null;
    if (trigger !== null)
...
function factorRule(messaging, rule) {
    ...
    if (isRemote(trigger)) {
        ...
        var toSend = {
            ...
            action: {
                ...
                person: messaging.type + '-account:' + messaging.account,
                dynamic_type: {
                    types: ['String'].concat(trigger.schema.schema.map(String)),
                    args: ['__token'].concat(trigger.schema.args),
                    required: [true],
                },
                args: [{
        ...
        trigger.resolved_args = [Ast.Value.String(token)].concat(trigger.resolved_args);
    }
    if (isRemote(query)) {
        ...
        toSend.action = {
            ...
            person: messaging.type + '-account:' + messaging.account,
            dynamic_type: {
                types: ['String'].concat(query.schema.schema.map(String)),
                args: ['__token'].concat(query.schema.args),
                required: [true],
            },
            args: [{
                name: { id: 'tt:param.__token' },
        ...
        newrules.push({
            trigger: {
                ...
                principal: query.principal,
                schema: {
                    schema: [Type.String].concat(query.schema.schema),
                    args: ['__token'].concat(query.schema.args),
                    required: [true]
                },
                resolved_args: [Ast.Value.String(token2)].concat(query.schema.args.map((a) => Ast.Value.VarRef(a))),
                resolved_conditions: []
            },
            query: null,
            action: action
    ...
    if (isRemote(action)) {
        ...
        if (query) {
            ...
            action.resolved_args = [Ast.Value.String(token)].concat(query.schema.args.map((a, i) => {
                if (query.resolved_args[i] !== undefined)
            ...
            if (isRemote(trigger)) {
                ...
                action.resolved_args = trigger.schema.args.map((a, i) => {
                    if (trigger.resolved_args[i] !== undefined)
                        return trigger.resolved_args[i];
                    else
                        return Ast.Value.VarRef(a);
                ...
                action.resolved_args = [Ast.Value.String(token)].concat(trigger.schema.args.map((a, i) => {
                    if (trigger.resolved_args[i] !== undefined)
                        return trigger.resolved_args[i];
                    else
                        return Ast.Value.VarRef(a);
