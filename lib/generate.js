...
function codegenTrigger(trigger, uses, constants) {
    ...
    var triggerParams = trigger.resolved_args.map(function(arg, i) {
        if (arg === undefined) {
            ...
            if (arg.isVarRef && arg.name in constants)
                return constants[arg.name];
            return constants[trigger.schema.args[i]] = codegenValue(arg);
        }
    }, this);
...
function codegenRule(trigger, query, action, once) {
    ...
    var actionAst;
    if (action !== null) {
        actionAst = codegenActionPart(action, constants);
    } else {
