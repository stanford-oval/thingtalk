...
function factorRule(messaging, rule) {
    ...
    if (!trigger && !query && isRemote(action)) {
        ...
        sendrules.push([action.principal, toSend]);
    ...
    if (isRemote(trigger)) {
        ...
        var toSend = {
            ...
            action: {
                ...
                dynamic_type: {
                    types: ['String', 'String'].concat(trigger.schema.schema.map(String)),
        ...
        sendrules.push([trigger.principal, {rule: toSend}]);
        ...
        trigger.schema = {
            schema: [Type.String, Type.String, Type.String].concat(trigger.schema.schema),
            args: ['__principal', '__token', '__kindChannel'].concat(trigger.schema.args),
            required: [true],
        };
        trigger.resolved_args = [Ast.Value.String(trigger.principal), Ast.Value.String(token), Ast.Value.String(oldKindChannel)]
    ...
    if (isRemote(query)) {
        ...
        if (trigger) {
            ...
            if (trigger.kind === 'remote' && trigger.channel === 'receive') {
                ...
                toSend.trigger.dynamic_type = {
                    types: ['String', 'String', 'String'].concat(trigger.schema.schema.map(String)),
                    args: ['__principal', '__token', '__kindChannel'].concat(trigger.schema.args),
        ...
        toSend.action = {
            ...
            dynamic_type: {
                types: ['String', 'String'].concat(query.schema.schema.map(String)),
        ...
        sendrules.push([query.principal, {rule: toSend}]);
        ...
        if (trigger) {
            newrules.push({
                ...
                action: {
                    ...
                    principal: null,
                    schema: {
                        schema: [Type.String, Type.String].concat(trigger.schema.schema),
                        args: ['__principal', '__token'].concat(trigger.schema.args),
                        required: [true]
                    },
                    resolved_args: [Ast.Value.String(token1)].concat(trigger.schema.args.map((a, i) => {
                        if (trigger.resolved_args[i] !== undefined)
        ...
        newrules.push({
            trigger: {
                ...
                principal: null,
                schema: {
                    schema: [Type.String, Type.String, Type.String].concat(query.schema.schema),
                    args: ['__principal', '__token', '__kindChannel'].concat(query.schema.args),
                    required: [true, true, true]
                },
                resolved_args: [Ast.Value.String(query.principal), Ast.Value.String(token2),
                    Ast.Value.String('query:' + query.kind + ':' + query.channel)].concat(query.schema.args.map((a) => Ast.Value.VarRef(a))),
                resolved_conditions: []
            },
            query: null,
            action: action
    ...
    if (isRemote(action)) {
        ...
        sendrules.push([action.principal, {rule: toSend}]);
        action.schema.schema.forEach((type, i) => {
            if (action.resolved_args[i] !== undefined) {
                var [jsonType, jsonValue] = valueToJSON(type, action.resolved_args[i]);
                toSend.action.args.push({
        ...
        if (query) {
            toSend.trigger.dynamic_type = {
                types: ['String', 'String', 'String'].concat(query.schema.schema.map(String)),
                args: ['__principal', '__token', '__kindChannel'].concat(query.schema.args),
                required: [true, true, true]
            };
            action.schema = {
                schema: [Type.String, Type.String].concat(query.schema.schema),
                args: ['__principal', '__token'].concat(query.schema.args),
                required: [true, true],
            };
            action.resolved_args = [Ast.Value.String(action.principal), Ast.Value.String(token)].concat(query.schema.args.map((a, i) => {
                if (query.resolved_args[i] !== undefined)
            ...
            if (trigger.kind === 'remote' && trigger.channel === 'receive') {
                ...
                action.schema = {
                    schema: [Type.String, Type.String].concat(trigger.schema.schema.slice(3)),
                    args: ['__principal', '__token'].concat(trigger.schema.args.slice(3)),
                    required: [true, true],
                };
                action.resolved_args = [Ast.Value.String(action.principal), Ast.Value.String(token)]
                ...
                action.schema = {
                    schema: [Type.String].concat(trigger.schema.schema),
                    args: ['__principal', '__token'].concat(trigger.schema.args),
                    required: [true],
                };
                action.resolved_args = [Ast.Value.String(action.principal), Ast.Value.String(token)].concat(trigger.schema.args.map((a, i) => {
                    if (trigger.resolved_args[i] !== undefined)
                        return trigger.resolved_args[i];
                    else
                        return Ast.Value.VarRef(a);
