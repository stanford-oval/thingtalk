...
module.exports = function(TERMINAL_IDS, RULE_NON_TERMINALS, ARITY, GOTO, PARSER_ACTION, SEMANTIC_ACTION) {
    return class ShiftReduceParser {
        ...
        async _helper(sequence, applySemanticAction) {
            ...
            const $ = {
                error(msg) {
                    throw new ThingTalkTypeError(msg, currentLocation);
                }
            };
            ...
            for(;;) {
                ...
                if (action === 1) { // shift
                    ...
                    if (applySemanticAction) {
                        ...
                        results.push(await action($, ...args));
                    } else {
                        results.push(null);
                    }
                }
        ...
        async parse(sequence) {
            let [, ast] = await this._helper(sequence, true);
            return ast;
        }

        async getReduceSequence(sequence) {
            let [reduces, ] = await this._helper(sequence, false);
            return reduces;
        }
    };
};
