...
module.exports = class Formatter extends FormatUtils {
    ...
    async _displayKey(key, outputType) {
        if (outputType === null)
            return clean(key);
    ...
    async _formatFallback(outputValue, outputType) {
        ...
        for (let key in outputValue) {
            const keyDisplay = await this._displayKey(key, outputType);
            const valueDisplay = this._displayValue(outputValue[key]);
            result.push(this._(`${keyDisplay}: ${valueDisplay}`));
        }
        return result;
    }
    ...
    async _formatAggregation(outputValue, operator, outputType) {
        ...
        let keyDisplay = await this._displayKey(key, outputType);
        ...
        case 'min':
            return [this._("The minimum %s is %s.").format(keyDisplay, this._displayValue(outputValue[key]))];

        case 'max':
            return [this._("The maximum %s is %s.").format(keyDisplay, this._displayValue(outputValue[key]))];

        case 'avg':
            return [this._("The average %s is %s.").format(keyDisplay, this._displayValue(outputValue[key]))];

        case 'sum':
            return [this._("The total %s is %s.").format(keyDisplay, this._displayValue(outputValue[key]))];
    ...
    async formatForType(outputType, outputValue, hint) {
        ...
        if (outputType === null)
            return await this._formatFallback(outputValue);
        ...
        return this._schemas.getFormatMetadata(kind, function_name).then((metadata) => {
            if (metadata.length)
                ...
                return this._formatFallback(outputValue, outputType);
        });
    }
