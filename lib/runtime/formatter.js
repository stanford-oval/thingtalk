...
class Formatter extends interpolate.Formatter {
    ...
    constructor(locale, timezone, schemaRetriever, gettext = I18n.get(locale)) {
        ...
        if (gettext) {
            ...
            this.ngettext = gettext.dngettext.bind(gettext, 'thingtalk');
        } else {
            this._ = (x) => x;
            this.ngettext = (x, x2, n) => n === 1 ? x : x2;
        }
    }
    ...
    _displayValue(value) {
        ...
        if (Array.isArray(value))
            return this.listToString(value.map((v) => this._displayValue(v)));
    ...
    _displayKey(key, functionDef) {
        ...
        for (let part of split) {
            if (first) {
                ...
                buf = this._("%s of %s").format(this._displayKeyPart(part, functionDef), buf);
            }
        }
    ...
    _formatFallbackValue(key, value, functionDef, outputPrefix, importanceOffset) {
        ...
        if (isPlainObject(value)) {
            ...
            result.push({ formatted: outputPrefix + this._("Here are the %s:").format(keyDisplay), isPrefix: true, importance: importance + 0.1 });
            ...
            const importance = importanceOffset + this._getArgImportance(key, functionDef);
            const valueDisplay = this._displayValue(value);

            if (key === 'id') {
                // id is special
                return [{ formatted: valueDisplay, importance }];
            }
            if (arg) {
            ...
            return [{ formatted: outputPrefix + this._("The %s is %s.").format(keyDisplay, valueDisplay), importance }];
        }
    }
    ...
    async _formatAggregation(outputValue, operator, outputType) {
        if (operator === 'count' && Object.prototype.hasOwnProperty.call(outputValue, 'count'))
            return [this.ngettext("I found %d result.", "I found %d results.", outputValue.count).format(outputValue.count)];
        ...
        switch (operator) {
        case 'count':
            return [this.ngettext("I found only one value of %2$s.", "I found %d distinct values of %s.", outputValue[key]).format(outputValue[key], keyDisplay)];

        case 'min':
            return [this._("The minimum %s is %s.").format(keyDisplay, this._displayValue(outputValue[key], '', outputType, key))];

        case 'max':
            return [this._("The maximum %s is %s.").format(keyDisplay, this._displayValue(outputValue[key], '', outputType, key))];

        case 'avg':
            return [this._("The average %s is %s.").format(keyDisplay, this._displayValue(outputValue[key], '', outputType, key))];

        case 'sum':
            return [this._("The total %s is %s.").format(keyDisplay, this._displayValue(outputValue[key], '', outputType, key))];
