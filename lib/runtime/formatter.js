...
module.exports = class Formatter extends FormatUtils {
    ...
    _formatFallbackValue(key, value, functionDef, outputPrefix) {
        if (value === null || value === undefined)
            return [];

        if (isPlainObject(value)) {
            // if it's a plain object, recurse format fallback
            return this._formatFallback(value, functionDef, key + '.', outputPrefix);
            ...
            const result = [];
            result.push(`${outputPrefix}${keyDisplay}:`);
            for (let element of value)
                result.push(...this._formatFallbackValue(key, element, functionDef, outputPrefix + '\t'));
            return result;
        } else {
            // otherwise, it's a scalar value, and we just display it
            const keyDisplay = this._displayKey(key, functionDef);
            const valueDisplay = this._displayValue(value);
            return [`${outputPrefix}${keyDisplay}: ${valueDisplay}`];
        }
    }

    _formatFallback(outputValue, functionDef, keyPrefix = '', outputPrefix = '') {
        const result = [];
        for (let key in outputValue)
            result.push(...this._formatFallbackValue(keyPrefix + key, outputValue[key], functionDef, outputPrefix));
        return result;
    }
    ...
    async _formatAggregation(outputValue, operator, outputType) {
        ...
        switch (operator) {
        case 'count':
            return [this._("I found %d distinct values of %s.").format(outputValue[key], keyDisplay)];
    ...
    async formatForType(outputType, outputValue, hint) {
        ...
        if (metadata.length) {
            ...
            const functionDef = await this._getFunctionDef(outputType);
            return this._formatFallback(outputValue, functionDef);
        }
    }
