...
module.exports = class Formatter extends FormatUtils {
    ...
    async _displayKey(key, outputType) {
        ...
        const functionDef = await this._schemas.getSchemaAndNames(kind, 'query', function_name);
    ...
    _formatFallback(outputValue, outputType) {
        return Object.keys(outputValue).map((key) => {
            return `${this._displayKey(key, outputType)}: ${this._displayValue(outputValue[key])}`;
        });
    }
